...
Follow only references whose trigger governs a changed file, behavior, boundary, artifact type, or
validation step. Do not bulk-read neighboring architecture, contexts, completed specifications, or
historical evidence. For user-facing work, include the applicable frontend, accessibility,
responsive, design-system, and runtime guidance. Resolve conflicts according to `AGENTS.md` and
report any conflict that requires a developer decision.
