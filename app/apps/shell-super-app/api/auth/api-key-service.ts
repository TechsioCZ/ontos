...
export interface ApiKeyServiceContract {
  ...
  readonly verify: (
    rawKey: string
  ) => Effect.Effect<VerifiedApiKey, ApiKeyProviderError>;
}
...
const IssuerPrincipalIdSchema = Schema.String.pipe(
  Schema.brand('ApiKeyIssuerPrincipalId')
);
const TenantIdSchema = Schema.String.pipe(Schema.brand('ApiKeyTenantId'));
const LifecycleOperationIdSchema = Schema.String.pipe(
  Schema.brand('ApiKeyLifecycleOperationId')
);
...
const PendingBindingScopeJson = Schema.fromJsonString(
  PendingBindingScopeSchema
);
const PendingBindingMarkerJson = Schema.fromJsonString(
  PendingBindingMarkerSchema
);
const makeIssuerPrincipalId =
  Brand.nominal<Schema.Schema.Type<typeof IssuerPrincipalIdSchema>>();
...
export const classifyPendingApiKeyCleanup = (
  ...
  const staleBefore =
    input.nowEpochMillis - Duration.toMillis(PENDING_BINDING_LEASE);
  return records.flatMap((record) => {
    ...
    return marker !== undefined &&
      ...
      DateTime.toEpochMillis(DateTime.makeUnsafe(record.createdAt)) <=
        staleBefore
...
const toSafe = (value: {
  ...
  expiresAt:
    value.expiresAt === null
      ? null
      : DateTime.formatIso(DateTime.makeUnsafe(value.expiresAt)),
  name: value.name,
...
const apiKeyExternalTimeout = Effect.timeoutOrElse({
  ...
  orElse: () =>
    Effect.fail(unavailable('The API key provider operation timed out')),
});

export const makeApiKeyService = Effect.fn('ApiKeyService.make')(
  function* makeService() {
    const configuration = yield* AuthConfig;
    const { adapter: databaseAdapter, executor: database } =
      yield* AuthDatabase;
      ...
      database
        ...
        .pipe(
          ...
          Effect.flatMap(([record]) =>
            record === undefined
              ? Effect.fail(inconsistent())
              : Effect.succeed(toSafe(record))
          )
        );
    const service: ApiKeyServiceContract = {
      ...
      metadata,
      pendingCleanup: Effect.fn('ApiKeyService.pendingCleanup')(
        function* pendingCleanup(input) {
          const nowEpochMillis =
            input.nowEpochMillis ?? (yield* Clock.currentTimeMillis);
          const staleBefore = DateTime.makeUnsafe(nowEpochMillis).pipe(
            DateTime.subtractDuration(PENDING_BINDING_LEASE),
            DateTime.toDateUtc
          );
          const encodedScope = yield* Schema.encodeEffect(
            PendingBindingScopeJson
          )(pendingBindingScope(input)).pipe(Effect.mapError(unavailable));
      ...
      verify: (rawKey) =>
        Effect.tryPromise({
          ...
          apiKeyExternalTimeout,
          Effect.flatMap(
            (result): Effect.Effect<VerifiedApiKey, ApiKeyProviderError> => {
