...
type GatewayIssuerFailureCause = Schema.Schema.Type<
  typeof GatewayIssuerFailureCauseSchema
>;

const gatewayIssuerErrorFields = {
  code: Schema.Literals([
    'gateway_audience_invalid',
    'gateway_issuer_unavailable',
  ]),
...
export type GatewayIssuerError = Schema.Schema.Type<
  typeof GatewayIssuerErrorSchema
>;

export interface IssueGatewayAssertionInput<
  Principal = GatewayTrustedPrincipalContext,
> {
...
export class GatewayIssuer extends Context.Service<
  GatewayIssuer,
  GatewayIssuerService
...
const makeGatewayIssuer = Effect.fn('GatewayIssuer.make')(
  function* gatewayIssuerService(options: GatewayIssuerLayerOptions) {
    const configurationCache = yield* Cache.makeWith(
      (_key: 'configuration') =>
        options.loadConfig.pipe(
          Effect.mapError((failureCause) =>
            unavailable('configuration', failureCause)
          )
        ),
      {
        capacity: 1,
        ...
        Effect.gen(function* loadSigningKeySlot() {
          ...
          if (Predicate.isUint8Array(key)) {
            return yield* unavailable(
              'signing',
              'Private key import returned a symmetric key'
            );
          }
    ...
    const issue = Effect.fn('GatewayIssuer.issue')(
      function* issueGatewayAssertion<Principal>(
        input: IssueGatewayAssertionInput<Principal>
      ) {
        const principal = yield* Schema.decodeUnknownEffect(
          ...
          Effect.mapError((failureCause) =>
            unavailable('principal', failureCause)
          )
        );
        const audiences = yield* options.loadAudiences.pipe(
          Effect.mapError((failureCause) =>
            unavailable('audience', failureCause)
          )
        );
        if (!audiences.has(input.audience)) {
          return yield* new GatewayIssuerErrorConstructor({
            ...
            reason:
              'The requested gateway audience is not a generated MicroVertical',
            stage: 'audience',
          });
        }

        const configuration = yield* Cache.get(
          configurationCache,
          'configuration'
        );
...
export const GatewayIssuerLive = makeGatewayIssuerLayer(
  gatewayIssuerLiveOptions
);

export const issueGatewayContextAssertion = Effect.fn(
  'GatewayIssuer.issueGatewayContextAssertion'
