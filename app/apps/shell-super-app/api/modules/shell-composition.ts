...
export type ShellCompositionUnavailableError =
  typeof ShellCompositionUnavailableErrorSchema.Type;
...
const loadCatalog = (
  ...
  sources.catalog.pipe(
    Effect.mapError(
      (cause) => new ShellCompositionUnavailableErrorConstructor({ cause })
    )
  );
...
const resolveContributionPage = (
  ...
  return landing === undefined
    ...
    : pages.find(
        ({ contributionKey }) =>
          String(contributionKey) === String(landing.pageKey)
      );
};

const resolveTargetPermission = Effect.fn(
  'ShellComposition.resolveTargetPermission'
  ...
  if (
    ...
    permission.key !== context.moduleId
  ) {
...
export const makeShellComposition = (sources: ShellCompositionSources) => {
  const compose = Effect.fn('makeShellComposition.compose')(
    function* composeShellEffect(context: ShellCompositionContext) {
      ...
      const states = new Map(
        records.map(({ moduleKey, state }) => [moduleKey, state])
      );
      ...
      const permissionByModule = new Map(
        decisions.map(({ decision, key }) => [key, decision])
      );
      ...
      const navigation = catalog.contracts.flatMap((contract) => {
        ...
        return contract.manifest.publicSurface.shellContributions.navigation.map(
          (contribution) => {
            ...
            const unavailable =
              permission === 'unavailable' || page === undefined;
      ...
      const composition = yield* Schema.decodeEffect(ShellCompositionSchema)({
        ...
        state: 'available',
        unavailableDeployments: catalog.deploymentStatuses.flatMap(
          (deployment) =>
  ...
  const resolveModuleTarget = Effect.fn(
    'makeShellComposition.resolveModuleTarget'
    ...
    const state =
      record?.moduleKey === input.moduleId ? record.state : undefined;
    if (state === undefined) {
      return { outcome: 'not_found' } as const;
    }
