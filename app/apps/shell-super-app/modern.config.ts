    ...
    decodeUnknownResult(
      OptionFromUndefinedOr(
        NumberFromString.pipe(
          check(isInt(), isBetween({ maximum: 65_535, minimum: 1 }))
        )
      )
...
const moduleFederationDevServerOrigin =
  getOptionalBuildConfig('ULTRAMODERN_MF_DEV_ORIGIN') ??
  'http://localhost:3020';
      ...
      'deploy',
      {
        worker: {
          ...
          services: [
            {
              binding:
                getOptionalBuildConfig(
                  'VERTICAL_PARTY_REGISTRY_WORKER_BINDING'
