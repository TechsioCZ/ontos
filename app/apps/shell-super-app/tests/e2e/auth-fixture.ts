...
const provisionContactsAccess = Effect.fn('provisionContactsAccess')(
  function* provisionContactsAccessEffect(e2eTenants: FixtureTenants) {
          ...
          await client.promises.writeRelationships(
            v1.WriteRelationshipsRequest.create({
              updates: relationships
                .flat()
                .map((relationship) => ({ operation, relationship })),
            })
          )
      );
    ...
    return yield* update(v1.RelationshipUpdate_Operation.TOUCH).pipe(
      Effect.uninterruptible
    );
  }
);

export const createAuthenticationFixture = Effect.fn(
  ...
  const corePool = yield* acquirePoolResource(
    () => new Pool(corePoolConfiguration)
  );
  ...
  const cleanup = Effect.fn('cleanupAuthenticationFixture')(
    function* cleanupAuthenticationFixtureEffect() {
      const tenantIds = Object.values(e2eTenants).map(
        ({ tenantId }) => tenantId
      );
...
export type AuthenticationFixture = Effect.Success<
  ReturnType<typeof createAuthenticationFixture>
>;
