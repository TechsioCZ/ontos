...
const hydratedLoginForm = (page: Page) =>
  page.locator('form[data-e2e-hydrated-login="true"]');
...
const waitForInteractiveAccountMenu = async (page: Page) => {
  await page.waitForFunction(() => {
    ...
    const content = positioner?.querySelector(
...
const gotoHydratedLogin = async (page: Page, language: 'cs' | 'en') => {
  ...
  await page.waitForFunction(() => {
    ...
    ].filter((form) => form.querySelector('input[name="login"]') !== null);
...
const login = async (
  page: Page,
  language: 'cs' | 'en',
  credentials: AuthenticationFixture['credentials']
) => {
  ...
  await form
    .getByRole('textbox', { name: labels.login })
    .fill(credentials.email);
  await form.getByLabel(labels.password).fill(credentials.password);
...
test('renders the exact anonymous English and Czech home states', async ({
  page,
}) => {
...
test('shows one generic error for invalid English credentials', async ({
  authentication,
  page,
}) =>
  await gotoHydratedLogin(page, 'en')
    ...
    .then(
      async () =>
        await Promise.all([
          expect(
            page.getByText('The email address or password is invalid.')
          ).toHaveCount(1),
          expect(
            page.getByRole('textbox', { name: /^Login\s*\*$/u })
          ).toBeFocused(),
        ])
    ));
...
test('logs a user in without any server-error response', async ({
  ...
  const signInResponsePromise = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname ===
        shellAuthenticationApiContract.signInPath &&
  ...
  expect(
    ...
    'The sign-in endpoint should accept valid credentials'
  ).toBe(200);
  ...
  await expect(
    page.getByRole('complementary', { name: 'Dashboard sidebar' })
  ).toBeVisible();
  await expect(
    page.locator('header[aria-label="Dashboard header"]')
  ).toBeVisible();
  expect(
    serverErrors,
    'Login and the authenticated page must not return HTTP 5xx'
  ).toEqual([]);
});
...
test('loads localized English and Czech Contacts pages only after login', async ({
  ...
  await expect(
    page.getByText('Nasazení modulu je dočasně nedostupné.')
  ).toHaveCount(0);
  ...
  const expectContacts = async (content: {
    ...
    await expect(
      page.getByRole('heading', { name: content.heading })
    ).toBeVisible();
    await expect(
      page.getByRole('heading', { name: content.module })
    ).toHaveCount(0);
  ...
  await expect(
    page.getByRole('complementary', { name: 'Postranní panel přehledu' })
  ).toBeVisible();
  ...
  await expect(
    page.getByText('The module is temporarily unavailable. Try again.')
  ).toHaveCount(0);
  const dashboardSidebar = page.getByRole('complementary', {
...
test('keeps authenticated Shell chrome on search and guarded direct-target routes', async ({
  ...
  await login(page, 'en', authentication.credentials);
...
test('persists an English session, logs out, clears the cookie, and stays anonymous', async ({
  ...
  await gotoHydratedLogin(page, 'en')
    ...
    .then(
      async () =>
        await Promise.all([
          ...
          expect(
            page.getByText(authentication.credentials.email)
          ).toBeVisible(),
    ...
    .then(
      async () =>
        await expect(
          page.getByRole('button', { name: 'E2E user' })
        ).toBeVisible()
    )
    .then(
      async () => await page.getByRole('button', { name: 'E2E user' }).click()
    )
    .then(
      async () => await page.getByRole('menuitem', { name: 'Logout' }).click()
    )
...
test('switches tenant by pointer, fully reloads, and persists the selected context', async ({
  ...
  await expect(
    page.getByText(authentication.tenants.first.tenantId)
  ).toBeVisible();
  await tenant.click();
  ...
  await Promise.all([
    ...
    page
      .getByRole('option', { name: authentication.tenants.second.name })
      .click(),
  ]);
  const switchResponse = await switchResponsePromise;
  expect(switchResponse.status()).toBe(200);
  await expect(
    page.getByRole('combobox', { name: 'Current tenant' })
  ).toContainText(authentication.tenants.second.name);
  await expect(
    page.getByRole('button', { name: 'E2E user second tenant' })
  ).toBeVisible();
  await expect(
    page.getByText(authentication.tenants.second.principalId)
  ).toBeVisible();
  await expect(
    page.getByText(authentication.tenants.second.tenantId)
  ).toBeVisible();
...
test('retains Czech tenant context after one failed switch and supports keyboard retry', async ({
  ...
  await page
    ...
    .click();
  await expect(
    page.getByText('Přepnutí tenantu selhalo. Zkuste to znovu.')
  ).toBeVisible();
  await expect(tenant).toContainText(authentication.tenants.first.name);
  await expect(
    page.getByText(authentication.tenants.first.tenantId)
  ...
  const secondTenantOption = page.getByRole('option', {
    name: authentication.tenants.second.name,
  });
  ...
  await expect(tenantListbox).toHaveAttribute(
    'aria-activedescendant',
    secondTenantOptionId ?? ''
  );
  await Promise.all([
    page.waitForEvent('framenavigated', {
      predicate: (frame) => frame === page.mainFrame(),
  ...
  await expect(
    page.getByRole('combobox', { name: 'Aktuální tenant' })
  ).toContainText(authentication.tenants.second.name);
});
...
test('keeps keyboard logout operable after a Czech failure and succeeds on retry', async ({
  ...
  await gotoHydratedLogin(page, 'cs')
    ...
    .then(
      async () =>
        await hydratedLoginForm(page)
          .getByRole('button', { name: 'Přihlásit se' })
          .click()
    )
    ...
    .then(
      async () =>
        await page.route(
          '**/shell-super-app-api/auth/sign-out',
          async (route) => {
            if (failLogout) {
              failLogout = false;
              await route.abort('failed');
    ...
    .then(
      async () => await page.getByRole('button', { name: 'E2E user' }).focus()
    )
    ...
    .then(
      async () =>
        await Promise.all([
          expect(page.getByRole('button', { name: 'E2E user' })).toBeVisible(),
          expect(
            page.getByText('Odhlášení selhalo. Zkuste to znovu.')
          ).toBeVisible(),
...
test('keeps the login form keyboard- and mobile-usable', async ({ page }) => {
  ...
  await gotoHydratedLogin(page, 'cs');
  const form = hydratedLoginForm(page);
  const loginInput = form.getByRole('textbox', {
    name: /^Přihlašovací jméno\s*\*$/u,
  });
  await expect(async () => {
    await loginInput.fill('hydration-probe');
    await form.getByRole('button', { name: 'Přihlásit se' }).click();
    await expect(page.getByText('Zadejte heslo.')).toBeInViewport({
      timeout: 1000,
    });
  }).toPass({ timeout: 5000 });
...
test('keeps the authenticated dashboard reachable without horizontal overflow at 375px', async ({
  ...
  await expect(
    page.getByRole('region', { name: 'Authenticated identity' })
  ).toBeInViewport();
  ...
  await expect(
    page.getByText('Tenant switching failed. Try again.')
  ).toBeInViewport();
  await expect(tenant).toContainText(authentication.tenants.first.name);
  expect(
    await page.evaluate(
