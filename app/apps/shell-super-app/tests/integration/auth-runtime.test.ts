...
const SessionResponseSchema = Schema.Struct({
  identity: Schema.optional(
    Schema.Struct({ principalId: Schema.optional(PrincipalIdSchema) })
  ),
});
...
const legalEntitySelectionOptions = {
  contextAccess: {
    ...
    tenants: ({ tenantIds }: { readonly tenantIds: readonly string[] }) =>
      Effect.succeed(
        tenantIds.map((key) => ({ decision: 'allowed' as const, key }))
      ),
  },
  legalEntityContext: {
    listActiveForTenant: () =>
  ...
  Effect.fnUntraced(function* runIntegration1() {
    ...
    const cleanup = Effect.fnUntraced(function* runIntegration2() {
      ...
      yield* purgeFixtureRows([
        ...
        coreDatabase
          .delete(principals)
          .where(eq(principals.principalId, principalId)),
        coreDatabase
          ...
          current.identity === null
            ...
            : Effect.succeed({
                ...
                principal: {
                  ...
                  authContextRef:
                    'better-auth-session:45000000-0000-4000-8000-000000000001',
    ...
    const cleanup = Effect.fnUntraced(function* runIntegration7() {
      ...
      if (existingUserIds.length > 0) {
        yield* purgeFixtureRows([
          coreDatabase
            .delete(principalAuthBindings)
            .where(
              inArray(principalAuthBindings.providerSubjectId, existingUserIds)
            ),
          authDatabase
            .delete(session)
            .where(inArray(session.userId, existingUserIds)),
          authDatabase
            .delete(account)
      ...
      yield* purgeFixtureRows([
        ...
        coreDatabase
          .delete(legalEntities)
          .where(
            inArray(legalEntities.legalEntityId, [
              firstLegalEntityId,
              secondLegalEntityId,
            ])
          ),
