...
const OwnerVerifierSchema = Schema.declare<GeneratedOwnerModules['verifyOperationPrincipal']>(
  (value): value is GeneratedOwnerModules['verifyOperationPrincipal'] =>
...
const loadGeneratedOwner = Effect.fnUntraced(function* runIntegration1(
  ...
  return {
    ...
    verifyOperationPrincipal: Schema.decodeUnknownSync(OwnerVerifierSchema)(
      verifier['verifyOperationPrincipal'],
    ),
    wiring: {
      action: true,
      detailClient:
...
>(response: Response, schema: ResponseSchema) {
  return Schema.decodeUnknownSync(schema)(yield* Effect.tryPromise(() => response.json()));
});
...
it.live(
  ...
  Effect.fnUntraced(function* runIntegration9() {
    ...
    expect(generatedServerSource).toMatch(/verifyOperationPrincipal/u);
    expect(generatedServerSource).toMatch(/yield\* ReadRuntime/u);
  }),
);
