...
export const actionCoreFailure = (
...
export const makeActionRuntimeDouble = (
  outcomes: readonly TestActionOutcome[]
) => {
  let invocation = 0;
  const payloads: unknown[] = [];
  const runtime: ActionRuntimeService = {
    resolveActionCommit: () =>
      Effect.die('resolveActionCommit is not configured in this test'),
    runAction: (input) => {
      ...
      if (outcome === undefined) {
        return Effect.die(
          `Action invocation ${invocation} has no configured outcome`
        );
      }
      ...
      if (outcome.kind === 'domain-failure') {
        ...
        return Schema.is(schema)(outcome.error)
          ...
          : Effect.die(
              'Configured action domain failure does not match registration schema'
            );
      }
      return Effect.sync(() =>
        Schema.decodeUnknownSync(input.registration.descriptor.resultSchema)(
          outcome.value
        )
      );
    },
