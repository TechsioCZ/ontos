...
const validEnvironment = {
  ...
  BETTER_AUTH_TRUSTED_ORIGINS:
    'http://localhost:3020,https://preview.example.test',
...
test('parses trusted origins and derives local cookie security', async () =>
  await runEffectTestPromise(parseAuthConfig(validEnvironment)).then(
    (configuration) => {
...
test('keeps gateway signing configuration independent from Better Auth configuration', async () => {
  const authentication = await runEffectTestPromise(
    parseAuthConfig(validEnvironment)
  );
  const gatewayError = await runEffectTestPromise(
    Effect.flip(parseGatewayIssuerConfig({}))
  );
