...
rs.mock('../../api/auth/environment-file-provider.ts', () => ({
  loadEnvironmentFileProvider: rs.fn(() =>
    Effect.succeed(ConfigProvider.fromEnvRecord({}))
  ),
}));

it.effect(
  'explicit environment overrides file values and absent keys fall back to the file',
  () =>
    Effect.gen(function* explicitEnvironment() {
      ...
      expect(loadEnvironmentFileProvider).toHaveBeenCalledWith(
        'fixture-path',
        expect.any(Function)
      );
    })
...
const read = (
  options: {
    readonly environment?: Readonly<
      Partial<Record<'ONTOS_PROVIDER_TEST', string>>
    >;
  } = {}
) =>
  loadConfigurationProvider(options, () => 'unreadable').pipe(
    Effect.flatMap((provider) =>
      Config.string('ONTOS_PROVIDER_TEST').parse(provider)
    )
  );

it.effect(
  'an explicit empty environment does not fall through to process values',
  () =>
    Effect.gen(function* emptyEnvironment() {
      ...
      rs.mocked(loadEnvironmentFileProvider).mockReturnValue(
        Effect.succeed(
          ConfigProvider.fromEnvRecord({ ONTOS_PROVIDER_TEST: 'file-value' })
        )
      );
...
it.effect(
  'file loading failures retain the parser-specific typed error and safe reason',
  () =>
    Effect.gen(function* fileFailure() {
      rs.mocked(loadEnvironmentFileProvider).mockImplementation(
        (_path, failure) => Effect.fail(failure())
      );
      const authFailure = yield* Effect.flip(
        loadAuthConfig({ environment: {} })
      );
      const gatewayFailure = yield* Effect.flip(
        loadGatewayIssuerConfig({ environment: {} })
      );
      expect(authFailure.reason).toBe(
        'Unable to load the root authentication environment'
      );
      expect(gatewayFailure.reason).toBe(
        'Unable to load the Shell gateway signing environment'
      );
    })
);
