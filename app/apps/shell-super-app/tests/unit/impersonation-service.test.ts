...
it.effect(
  'preserves definite requested-checkpoint errors for their declared HTTP mapping',
  () =>
...
it.effect(
  'removes the provider session and recovery when started evidence cannot commit',
  () =>
...
it.effect(
  'completes every pending checkpoint correlated to the restored session',
  () =>
  ...
  () =>
    Effect.gen(function* testProgram6() {
      ...
      const service = makeService({
        ...
        provider: makeSupportAuthProviderDouble({
          getSession: () =>
            Promise.resolve({ headers: new Headers(), response: null }),
        }),
        resolver: makePrincipalResolverDouble(),
        store: makeSupportImpersonationStoreDouble({
          deleteRecovery: () =>
    ...
    Effect.gen(function* testProgram10() {
      ...
      const service = makeService({
        ...
        provider: makeSupportAuthProviderDouble({
          ...provider(true).api,
          stopImpersonating: () =>
            Promise.reject(new Error('admin session expired')),
        }),
        resolver: makePrincipalResolverDouble(),
        store: makeSupportImpersonationStoreDouble({
          deleteSession: () =>
