...
const allowlist = (
  entries: DeploymentAllowlist['entries']
): DeploymentAllowlist =>
  Object.freeze({
...
it.effect(
  'loads two independent deployment contracts once and preserves both identities',
  () =>
    Effect.gen(function* verifyCase1() {
      ...
      const [first, concurrent, cached] = yield* Effect.all(
      ...
      expect(first.moduleIds).toEqual([
        'documents.center',
        'property.registry',
      ]);
      expect(
...
it.effect(
  'keeps a healthy deployment available on cold start when another is unreachable',
  () =>
...
const unavailableResponses = [
  ...
  [
    ...
    () =>
      Promise.resolve(
        response(contract('documents-center', 'property.registry'))
      ),
    'incompatible',
  ],
] as const;
...
it.live(
  'classifies oversized, timed-out, and duplicate-module deployments without caching failures',
  () =>
...
it.effect(
  'recovers a deployment on a later read and caches only the fully healthy result',
  () =>
    Effect.gen(function* verifyCase5() {
      ...
      const loader = makeInstalledModuleCatalogLoader(
        ...
        () => {
          ...
          return Promise.resolve(
            response(contract('property-registry', 'property.registry'))
          );
        }
      );
...
it.effect(
  'recreates the complete cache by constructing a new deployment-revision Layer',
  () =>
