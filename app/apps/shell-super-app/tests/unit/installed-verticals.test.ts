...
it.effect(
  ...
  () =>
    Effect.gen(function* verifyCase1() {
      ...
      expect([...(yield* installedVerticalIds)]).toEqual([
...
it.effect(
  'rejects malformed, non-vertical, invalid, and duplicate installed entries',
  () =>
    Effect.gen(function* verifyCase2() {
      ...
      expect(errors.every(Schema.is(InstalledVerticalTopologyError))).toBe(
        true
      );
    })
);
