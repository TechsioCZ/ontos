...
it.effect(
  'resolves trusted context before returning one serializable composition',
  () =>
    Effect.gen(function* verifyCase1() {
      ...
      expect(currentSessionMock.mock.invocationCallOrder[0]).toBeLessThan(
        shellCompositionMock.mock.invocationCallOrder[0] ??
...
it.effect(
  'does not invent a selected legal entity while a tenant session requires selection',
  () =>
...
it.effect(
  'uses the configured HTTPS origin for the server-side session request',
  () =>
    Effect.gen(function* verifyCase4() {
      ...
      expect(
        currentSessionMock.mock.calls.at(-1)?.[0]?.baseUrl.toString()
      ).toBe('https://shell.stage.example.test/shell-super-app-api');
    })
);

it.effect(
  'keeps the configured local HTTP origin for the server-side session request',
  () =>
    Effect.gen(function* verifyCase6() {
      ...
      yield* withBetterAuthUrl('http://localhost:3020', () =>
        Effect.gen(function* verifyCase7() {
          return yield* loadModel({
            request: new Request('http://localhost:3020/en'),
          });
        })
      );
      ...
      expect(
        currentSessionMock.mock.calls.at(-1)?.[0]?.baseUrl.toString()
      ).toBe('http://localhost:3020/shell-super-app-api');
    })
);

it.effect(
  'maps composition failure to unavailable without discarding verified context',
  () =>
...
it.effect(
  'maps tenant failure to the current-tenant fallback without discarding composition',
  () =>
...
it.effect(
  'tears down stale authenticated data when tenant context requires authentication',
  () =>
