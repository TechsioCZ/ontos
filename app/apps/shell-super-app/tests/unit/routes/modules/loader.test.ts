...
import { loader as contactDetailLoader } from '../../../../src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/page.data.ts';
import { loader as contactEditLoader } from '../../../../src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/edit/page.data.ts';
import { loader as customerDetailLoader } from '../../../../src/routes/[lang]/projects/customers/[id]/page.data.ts';
import { loader as customerCreateLoader } from '../../../../src/routes/[lang]/projects/customers/[id]/new/page.data.ts';
import { loader as customerEditLoader } from '../../../../src/routes/[lang]/projects/customers/[id]/edit/page.data.ts';
import { loader as contactCreateLoader } from '../../../../src/routes/[lang]/projects/customers/[id]/contacts/new/page.data.ts';
import { loader as customersListLoader } from '../../../../src/routes/[lang]/projects/customers/page.data.ts';
...
const request = () => {
  const value = new Request('https://shell.example.test/en/projects/customers');
...
beforeEach(() => {
  ...
  resolveModuleTargetMock.mockReturnValue(
    Effect.succeed({
      appId: 'projects',
      componentKey: 'projects.core.page-customers',
...
test('forwards an exact generated page entrypoint through the authenticated client', async () => {
  ...
  await expect(customersListLoader({ request: request() })).resolves.toMatchObject({
    state: 'resolved',
    target: { componentKey: 'projects.core.page-customers-list' },
  });
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.customers-list', moduleId: 'projects.core' },
    {
      baseUrl: new URL('http://localhost:3020/shell-super-app-api'),
...
test('forwards only the declared Customer ID after exact Customer-detail resolution', async () => {
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.customer-detail', moduleId: 'projects.core' },
    expect.any(Object),
  );
});
...
test('gates Contact detail exactly and forwards only its two bounded hierarchical IDs', async () => {
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.contact-detail', moduleId: 'projects.core' },
    expect.any(Object),
  );
});
...
test('gates ContactEdit exactly and forwards only its ordered bounded hierarchical IDs', async () => {
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.contact-edit', moduleId: 'projects.core' },
    expect.any(Object),
  );
...
test('gates CustomerEdit exactly and carries only its declared bounded Customer ID', async () => {
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.customer-edit', moduleId: 'projects.core' },
    expect.any(Object),
  );
});
...
test('gates CustomerCreate exactly and carries only its declared bounded route ID', async () => {
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.customer-create', moduleId: 'projects.core' },
    expect.any(Object),
  );
...
test('gates ContactCreate exactly and carries only its declared bounded Customer ID', async () => {
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.contact-create', moduleId: 'projects.core' },
    expect.any(Object),
  );
});
...
test('retains only declared bounded route parameters outside the resolved target identity', async () => {
  await expect(
    loader({
      params: { entrypointKey: 'projects.core.page.customers', moduleId: 'projects.core' },
      request: request(),
      routeParams: { id: 'customer-1' },
    }),
  ).resolves.toMatchObject({
  ...
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { entrypointKey: 'projects.core.page.customers', moduleId: 'projects.core' },
    expect.any(Object),
  );
});
...
test('retains module landing behavior when no exact page entrypoint is supplied', async () => {
  await expect(
    loader({ params: { moduleId: 'projects.core' }, request: request() }),
  ).resolves.toMatchObject({ routeParams: {} });
  expect(resolveModuleTargetMock).toHaveBeenCalledWith(
    { moduleId: 'projects.core' },
    expect.any(Object),
