...
it.effect(
  'retains module landing behavior when no exact page entrypoint is supplied',
  () =>
...
it.effect(
  'does not request or load a private target before authentication',
  () =>
    Effect.gen(function* doesNotRequestOrLoadA() {
      loadHomePageModelMock.mockReturnValueOnce(
        Effect.succeed({ state: 'anonymous' })
      );
      expect(
        yield* moduleModel({
          params: {
...
it.effect(
  'fails with the typed timeout instead of hanging on an unresponsive shell read',
  () =>
