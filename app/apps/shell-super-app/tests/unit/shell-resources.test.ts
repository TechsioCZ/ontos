  ...
  resources: ({ permission = 'read', resources }) =>
    Effect.succeed(
      resources.map(({ moduleId: owner, resourceId, resourceType: type }) => ({
        decision:
          permission === 'write' ? resourceWriteDecision : resourceDecision,
      ...
      const result = yield* makeShellSearch(
        {
          ...
          contextAccess: {
            ...
            resources: () =>
              Effect.die(
                'empty results must not authorize an empty resource batch'
              ),
          },
        },
...
it.effect(
  'search filters resource denials and reports partial provider failure',
  () =>
    Effect.gen(function* searchFiltersResourceDenialsAndReports() {
      const result = yield* makeShellSearch(
        dependencies('active', 'allowed', 'denied'),
        {
      ...
      if (contract === undefined) {
        throw new TypeError(
          'The search fixture must install its module contract'
        );
      }
...
it.effect(
  ...
  () =>
    Effect.gen(function* tenantScopedPartySearchNeedsNo() {
      ...
      const [partyResourceDescriptor] =
        contract.manifest.publicSurface.resourceTypes;
      ...
      const result = yield* makeShellSearch(
        {
          ...
          contextAccess: {
            ...baseline.contextAccess,
            modules: () =>
              Effect.die('tenant-scoped search must not require module access'),
            resources: () =>
              Effect.die(
                'tenant-scoped search must not require resource access'
              ),
...
it.effect('search fails only when every eligible provider fails', () =>
  Effect.gen(function* searchFailsOnlyWhenEveryEligible() {
    ...
    expect(
      Schema.is(ShellProviderUnavailableError)(yield* Effect.flip(effect))
    ).toBe(true);
  })
);
  ...
  () =>
    Effect.gen(function* CounterpartySearchPreservesBothIdentitiesSelected() {
      ...
      const filteredCatalog = buildInstalledModuleCatalog([
        {
          contract: {
            ...
            manifest: {
              ...
              publicSurface: {
                ...contract.manifest.publicSurface,
                search: contract.manifest.publicSurface.search.map(
                  (descriptor) => ({
                    ...descriptor,
      ...
      const collision = {
        counterpartyRefs: [
          counterpartyRef,
          { ...counterpartyRef, resourceId: 'unit-2' },
        ],
  ...
  () =>
    Effect.gen(function* treatsAMissingTenantModuleState() {
      ...
      const gateway = {
        ...
        timeline: () =>
          Effect.succeed({ entries: [], projectionLagging: false }),
      };
      expect(
        yield* makeShellResourceDetail(hiddenDependencies, gateway).resolve(
          context,
          ref
        )
...
it.effect(
  'search fails closed for module or resource authorization uncertainty',
  () =>
...
it.effect(
  'resource detail sorts an authorized timeline and exposes projection lag',
  () =>
    Effect.gen(function* resourceDetailSortsAnAuthorizedTimeline() {
      ...
      expect(
        yield* Schema.encodeEffect(Schema.Array(ShellTimelineEntrySchema))(
          result.timeline
        )
      ).toEqual([
        {
...
it.effect(
  'media affordance remains unavailable until a generated Action exists',
  () =>
    Effect.gen(function* mediaAffordanceRemainsUnavailableUntilA() {
      ...
      expect(
        yield* makeShellResourceDetail(dependencies(), provider).resolve(
          context,
          ref
        )
      ).toMatchObject({ media: { enabled: false, reason: 'unavailable' } });
...
it.effect(
  'acquires a fresh audience-scoped assertion for each provider attempt',
  () =>
