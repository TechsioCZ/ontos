...
test('search treats empty input as empty without touching providers', async () => {
  ...
  await expect(
    runEffectTestPromise(search.search(context, '   '))
  ).resolves.toEqual({
    partial: false,
    results: [],
  });
...
test('search filters resource denials and reports partial provider failure', async () => {
  ...
  await expect(
    runEffectTestPromise(partial.search(context, 'unit'))
  ).resolves.toEqual({
    partial: true,
    ...
    makeShellSearch(
      {
        ...
        contextAccess: {
          ...
          resources: () =>
            Effect.die('tenant-scoped search must not require resource access'),
...
test('search fails only when every eligible provider fails', async () => {
  ...
  await expect(runEffectTestPromise(effect)).rejects.toBeInstanceOf(
    ShellProviderUnavailableError
  );
});
...
test('Counterparty search preserves both identities, selected scope, roles and collision metadata', async () => {
  ...
  expect(
    await runEffectTestPromise(search.search(tenantContext, 'canonical'))
  ).toEqual({
    partial: false,
    results: [],
  });
...
test('treats a missing tenant module-state record as hidden rather than authorization uncertainty', async () => {
  ...
  await expect(
    runEffectTestPromise(attachShellMedia(context, ref))
  ).resolves.toEqual({
    outcome: 'unavailable',
  });
  expect(calls).toBe(0);
...
test('resource detail applies catalog, state, module and resource gates before providers', async () => {
  ...
  expect(
    await runEffectTestPromise(
      makeShellResourceDetail(dependencies('inactive'), provider).resolve(
        context,
        ref
      )
    )
...
test('resource detail sorts an authorized timeline and exposes projection lag', async () => {
  ...
  await expect(
    runEffectTestPromise(
      Schema.encodeEffect(Schema.Array(ShellTimelineEntrySchema))(
        result.timeline
      )
    )
  ).resolves.toEqual([
...
test('media affordance remains unavailable until a generated Action exists', async () => {
  ...
  await expect(
    runEffectTestPromise(
      makeShellResourceDetail(dependencies('read_only'), provider).resolve(
        context,
        ref
      )
    )
  ...
  await expect(
    runEffectTestPromise(
      makeShellResourceDetail(dependencies(), provider).resolve(context, ref)
    )
  ).resolves.toMatchObject({
    media: { enabled: false, reason: 'unavailable' },
  });
