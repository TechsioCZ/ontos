...
it.effect(
  'defines both exact stage accounts without storing their passwords',
  () =>
    Effect.sync(() => {
...
it.effect(
  'refuses to provision outside stage or without an operator-supplied password',
  () =>
...
it.effect(
  'treats an exact record as idempotent and rejects conflicting state',
  () =>
    Effect.gen(function* treatsAnExactRecordAsIdempotent() {
      ...
      expect(
        yield* classifyExactStageDemoRecord('tenant', undefined, expected)
      ).toBe('create');
      expect(
        yield* classifyExactStageDemoRecord('tenant', expected, expected)
      ).toBe('existing');
      expect(
        yield* Effect.flip(
          classifyExactStageDemoRecord(
...
it.live(
  'keeps the demo bootstrap operator-invoked and excludes its password from source',
  () =>
