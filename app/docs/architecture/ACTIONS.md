...
10. Open the Core-owned transaction, lock and recheck the invocation, then lock the tenant and authoritatively recheck tenant `write` access. Only then may Core create the collector, resolve the private handler, and execute it. Competing requests may repeat read-only gates, but their handlers must never run concurrently.
