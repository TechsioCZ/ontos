...
views into RLS policies under the effective view execution identity; follows recursive foreign-key
...
the identity can set `session_replication_role`; and matches `UPDATE OF` columns to direct or
cascaded column changes;
...
| Privileged routines                          | Audit inventories routines in every non-system schema and flags `SECURITY DEFINER` routines that the runtime or an assumable role can invoke directly, through applicable RLS policies, column defaults, generated columns, table or domain checks, index expressions, direct or nested selectable view rewrites, enabled DML triggers, or enabled database event triggers matching reachable DDL tags. It follows automatically writable views to stored expressions and base-relation triggers on the actual rewrite target, selectable owner-context views into RLS policies under the effective execution identity, partition-routed expressions and row triggers—including row movement caused by `UPDATE`—recursive foreign-key referential writes into stored expressions and triggers, and reachable replica-mode triggers. Table checks conservatively apply to every update; generated columns do so when a `BEFORE ROW UPDATE` trigger disables PostgreSQL's dependency optimization. | Every future privileged routine, policy, stored-expression, and trigger path needs a narrow contract and hardening review; production must be audited rather than inferred. |
