...
grants; inventories extension, foreign-data-wrapper, and foreign-server ownership and user-defined base,
...
| Privileged routines                          | Audit inventories routines in every non-system schema and flags `SECURITY DEFINER` routines that the runtime or an assumable role can invoke directly, through applicable RLS policies, column defaults, generated columns, table or domain checks, index expressions, direct or nested selectable view rewrites, or enabled DML triggers including partition-routed leaf triggers. Table checks conservatively apply to every update; the baseline has none. | Every future privileged routine, policy, stored-expression, and trigger path needs a narrow contract and hardening review; production must be audited rather than inferred. |
