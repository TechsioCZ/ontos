...
`SECURITY DEFINER` routines that are directly executable, referenced by an applicable RLS policy,
...
through relation DML and an enabled user trigger or through an applicable database event trigger. It
matches event-trigger command tags to DDL the identity can issue, maps leaf stored expressions and
row triggers back to accessible partition ancestors, follows recursive foreign-key `CASCADE`, `SET
NULL`, and `SET DEFAULT` writes into stored expressions and user triggers, excludes leaf statement
triggers when DML names only a partition ancestor, includes replica-enabled triggers when the
identity can set `session_replication_role`, and matches `UPDATE OF` columns to direct or cascaded
column changes;
generated columns follow referenced-column updates unless any `BEFORE ROW UPDATE` trigger forces
PostgreSQL to recompute them, while table checks conservatively apply to every update; inventories
extension, foreign-data-wrapper, and foreign-server ownership and user-defined base,
...
| Malformed transaction scope                  | The scoped-transaction unit test rejects a mocked post-installation context mismatch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | No PostgreSQL integration test currently installs malformed `ontos.tenant_id` or `ontos.legal_entity_id` values.                                                            |
...
| Privileged routines                          | Audit inventories routines in every non-system schema and flags `SECURITY DEFINER` routines that the runtime or an assumable role can invoke directly, through applicable RLS policies, column defaults, generated columns, table or domain checks, index expressions, direct or nested selectable view rewrites, enabled DML triggers, or enabled database event triggers matching reachable DDL tags. It follows partition-routed expressions and row triggers, recursive foreign-key referential writes into stored expressions and triggers, and reachable replica-mode triggers. Table checks conservatively apply to every update; generated columns do so when a `BEFORE ROW UPDATE` trigger disables PostgreSQL's dependency optimization. | Every future privileged routine, policy, stored-expression, and trigger path needs a narrow contract and hardening review; production must be audited rather than inferred. |
| DDL and role escalation                      | Audit checks database-level `CREATE`, every non-system schema, extension/foreign-data-wrapper/foreign-server/relation/routine/type ownership, predefined roles, parameter ACLs, and cluster/database/schema/object authority for every reachable role; the baseline has no DDL authority or membership.                                                                                                                                                                                                                                                                                                                                                                                                                                            | There is no deployed-environment evidence until the audit is run there; executable denial probes for `SET ROLE` and representative DDL could be added to a pilot.           |
| Grant authority                              | Audit inventories effective current-object and creator-default grant options for the runtime and every `SET`/`ADMIN`-reachable role across all supported object classes; inheritance-only grant options are not classified as exercisable; the baseline has none.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Production must be audited rather than inferred from local grants.                                                                                                          |
| Privileged owner-context views               | Audit records `security_invoker`, recursively follows view dependencies, and flags selectable views whose outer or nested owner context is administrative, `BYPASSRLS`, superuser, or owns a referenced unforced-RLS table—even through a security-invoker boundary; the baseline has no views.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | A future owner-context view requires an explicit tenant-isolation review even when its owner is not otherwise privileged.                                                   |
