...
through relation DML and an enabled user trigger. It maps leaf stored expressions and triggers back
to accessible partition ancestors, follows recursive foreign-key `CASCADE`, `SET NULL`, and
`SET DEFAULT` writes into user triggers, includes replica-enabled triggers when the identity can set
`session_replication_role`, and matches `UPDATE OF` columns to direct or cascaded column changes;
