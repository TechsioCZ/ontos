...
`SECURITY DEFINER` routines that are directly executable or invocable through relation DML and an
enabled user trigger, following accessible partition ancestors when rows route to leaf triggers;
inventories extension ownership; and inventories user-defined base,
...
| Privileged routines                          | Audit inventories routines in every non-system schema and flags `SECURITY DEFINER` routines that the runtime or an assumable role can invoke directly or through enabled DML triggers, including leaf triggers reached through partition routing; the baseline has none. | Every future privileged routine and trigger path needs a narrow contract and hardening review; production must be audited rather than inferred.                   |
