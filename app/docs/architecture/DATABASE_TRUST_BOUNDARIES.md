...
through relation DML and an enabled user trigger, following accessible
partition ancestors when rows route to leaf triggers, including replica-enabled triggers when the
identity can set `session_replication_role`, and matching `UPDATE OF` columns to column grants;
