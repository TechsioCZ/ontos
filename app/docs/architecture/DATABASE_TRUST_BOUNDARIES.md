...
enabled user trigger, plus user-defined base,
...
| Privileged routines                          | Audit inventories routines in every non-system schema and flags `SECURITY DEFINER` routines that the runtime or an assumable role can invoke directly or through enabled DML triggers; the baseline has none.                                                     | Every future privileged routine and trigger path needs a narrow contract and hardening review; production must be audited rather than inferred.                   |
