...
or invocable through relation DML and an enabled user trigger, following accessible partition
ancestors when rows route to leaf triggers and matching `UPDATE OF` columns to column grants;
inventories extension ownership and user-defined base,
...
| Privileged routines                          | Audit inventories routines in every non-system schema and flags `SECURITY DEFINER` routines that the runtime or an assumable role can invoke directly, through applicable RLS policies, or through enabled DML triggers including partition-routed leaf triggers; the baseline has none.        | Every future privileged routine, policy, and trigger path needs a narrow contract and hardening review; production must be audited rather than inferred.          |
| DDL and role escalation                      | Audit checks database-level `CREATE`, every non-system schema, extension/relation/routine/type ownership, predefined roles, parameter ACLs, and cluster/database/schema/object authority for every reachable role; the baseline has no DDL authority or membership.                             | There is no deployed-environment evidence until the audit is run there; executable denial probes for `SET ROLE` and representative DDL could be added to a pilot. |
