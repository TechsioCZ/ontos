...
3. Map only `name`, `ico`, `dic`, `legalFormCode`, `establishedOn`, and `dissolvedOn` into the flat
   Projects Customer prefill contract.
...
## Current OntOS Projects implementation profile

The implemented Projects adapter deliberately uses only the consolidated subject endpoint. It does not
...
The current Projects adapter therefore rejects shorter input instead of guessing or padding it. See the
...
IČO input
  ...
  -> generated Projects governed Read
