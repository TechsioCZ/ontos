...
- A baseline collection schema initially exposes Title; Task Property Definitions extend the shared schema. All creation paths—blank canvas, manual creation, duplication, import, automation, and system work—converge on one Task-creation operation. It assigns intrinsic facts and ID internally; callers cannot supply or override them.
- CoreSDK authorization derives the collection-scoped resource from validated action input. Schema authorization and Task-value authorization are separate checks. No Ticketing handler bypasses the governed action boundary with ad hoc authorization.
...
- Durable blank Task creation atomically assigns server Task identity, Created time, valid Actor, Last edited time equal to Created time, and Last edited by equal to the creator. Creation is idempotent. Title/content are not prerequisites.
