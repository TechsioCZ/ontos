...
const logInvocationPersistenceFailure = (
  ...
  logActionInvocationPersistenceFailureCause(failure, annotations);
...
const isCommitAcknowledgementFailure = (failure: SqlError): boolean =>
  ...
  (Option.isNone(findPostgresFailure(failure)) &&
    (Schema.is(ConnectionError)(failure.reason) ||
      Schema.is(UnknownError)(failure.reason)));
...
const requestHashConflict = () =>
  new ActionRequestHashConflict({
    ...
    reason:
      'This idempotency key was already used for a different Action request',
  });
...
const resolveActionTenantPermission = <Payload>(
  payload: Payload,
  resolver:
    | ((payload: Payload) => ActionTenantPermission | undefined)
    | undefined
): Effect.Effect<
  Option.Option<ActionTenantPermission>,
  ActionPermissionCheckError
> =>
  Effect.try({
    catch: (cause) =>
...
const ActionResourcePermissionTargetSchema = Schema.Struct({
  ...
  resource: Schema.Struct({
    moduleId: Schema.String.check(
      Schema.isMinLength(1),
      Schema.isMaxLength(300)
    ).pipe(Schema.brand('ActionTargetModuleId')),
    resourceId: Schema.String.check(
      Schema.isMinLength(1),
      Schema.isMaxLength(300)
    ).pipe(Schema.brand('ActionTargetResourceId')),
    resourceType: Schema.String.check(
      Schema.isMinLength(1),
      Schema.isMaxLength(300)
    ).pipe(Schema.brand('ActionTargetResourceType')),
  }),
});
...
const resolveActionResourcePermissionTarget = <Payload>(
  ...
  Option.Option<ActionResourcePermissionTarget>,
  ActionPermissionCheckError
> =>
  Effect.suspend(() => {
    ...
    return Effect.try({
        ...
        attachFailureCause(
          new ActionPermissionCheckError({
            code: 'action_permission_check_failed',
            reason:
              'The declared Action permission target could not be resolved safely',
          }),
          cause
        ),
      ...
      Effect.flatMap(
        Schema.decodeUnknownEffect(ActionResourcePermissionTargetSchema)
      ),
  ...
  if (invocation.status === 'indeterminate') {
    return Effect.fail(
      new ActionCommitIndeterminate({
        ...
        reason:
          'This invocation requires commit resolution before it can execute',
      })
    );
  }
  if (
...
const validateInvocationId = <Input>(
  ...
  typeof ActionInvocationIdSchema.Type,
  ActionPayloadValidationError
> =>
...
export const makeActionRuntime = (
  ...
  const [database, repository, permission, operationalScopeResolver, options] =
    construction;
  ...
  const resolveServiceFactory =
    options.resolveServiceFactory ?? getActionServiceFactory;
  ...
  const runAction: ActionRuntimeService['runAction'] = Effect.fn(
  ...
  >(
    ...
    const principal = yield* validatePrincipal(
      input.principal,
      input.registration,
      payload
    );
    ...
    const moduleStateSnapshot = yield* moduleEntrypointGateway.prepareSnapshot(
      scope,
    ...
    yield* requireIdempotencyKey(
      input.registration.descriptor.idempotency,
      transport
    );
    ...
    const resourcePermissionTarget =
      yield* resolveActionResourcePermissionTarget(
        payload,
        scope,
    ...
    )(function* logPermissionTransactionFailureEffect(
    ...
    const rejectPermission = () =>
      repository
        ...
        .pipe(
          Effect.tapErrorTag(
            'ActionInvocationPersistenceError',
            logPermissionInvocationFailure
          ),
          Effect.tapErrorTag(
            'ActionTransactionError',
            logPermissionTransactionFailure
          ),
          Effect.flatMap(() =>
            Effect.fail(
    ...
    const legalEntityPermissionDecision =
      yield* checkActionLegalEntityPermission(scope, legalEntityPermission);
    ...
    const policyInput: ActionPolicyEvaluatorInput<typeof payload> =
      Object.freeze({
    ...
    const evaluateActionPolicy = Effect.fn('ActionRuntime.evaluatePolicy')(
      function* evaluatePolicy(policy: ActionPolicy<typeof payload, Owner>) {
        const policyExit = yield* Effect.exit(
          Effect.suspend(() => policy.evaluate(policyInput))
        );
        if (Exit.isSuccess(policyExit)) {
          return policyEvidence(policy);
        }

        const failureReasons = policyExit.cause.reasons.filter(
          Cause.isFailReason
        );
    ...
    const transactionBodyExit = yield* Ref.make<Exit.Exit<
      unknown,
      unknown
    > | null>(null);
    const transactionProgram = Effect.fn('ActionRuntime.transaction')(
      function* executeTransaction(drizzleTransaction: CoreTransaction) {
        ...
        const scopedTransaction = yield* installScope(
          drizzleTransaction,
          scope
        );
        ...
        if (Exit.isFailure(handlerExit)) {
          const failureReasons = handlerExit.cause.reasons.filter(
            Cause.isFailReason
          );
          const [failureReason] = failureReasons;
          if (
            ...
            const decodedDomainError = yield* Effect.option(
              Schema.decodeUnknownEffect(
                input.registration.descriptor.domainErrorSchema
              )(failureReason.error)
            );
  ...
  const resolveActionCommit: ActionRuntimeService['resolveActionCommit'] =
    Effect.fn('ActionRuntime.resolveActionCommit')(
            ...
            Effect.mapError((error) =>
              Schema.is(ActionInvocationPersistenceError)(error)
                ? new ActionCommitIndeterminate({
                    ...
                    invocationId,
                    reason:
                      'The database cannot confirm the Action commit state yet',
                  })
                : error
            )
          );
...
> =>
  ActionRuntime.pipe(
    Effect.flatMap((runtime) => runtime.resolveActionCommit(input))
  );
