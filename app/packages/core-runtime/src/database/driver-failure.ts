...
export const DatabaseDriverFailureKindSchema = Schema.Literals([
  'socket',
  'sqlstate',
]);
export type DatabaseDriverFailureKind = Schema.Schema.Type<
  typeof DatabaseDriverFailureKindSchema
>;
...
type DatabaseTransactionFailureSelf = Schema.Schema.Type<
  typeof DatabaseTransactionFailureContract
>;
export const DatabaseTransactionFailure =
  Schema.TaggedError<DatabaseTransactionFailureSelf>()(
    'DatabaseTransactionFailure',
    driverFailureFields
  );
...
export type DatabaseDriverFailure = Schema.Schema.Type<
  typeof DatabaseDriverFailureSchema
>;
...
const unavailableSqlStateClasses = new Set<string>([
  '08',
  '40',
  '53',
  '55',
...
const decodeDriverCodeFailure = (
  code: string
): Option.Option<DatabaseDriverFailure> => {
  const sqlStateClass = code.slice(0, 2);
  if (
    sqlStateClass === connectionSqlStateClass ||
    code === administrativeShutdownSqlState
  ) {
    return Option.some(
      new DatabaseCommitAcknowledgementAmbiguous({ code, kind: 'sqlstate' })
    );
  }
  if (commitAcknowledgementSocketCodes.has(code)) {
    return Option.some(
      new DatabaseCommitAcknowledgementAmbiguous({ code, kind: 'socket' })
    );
  }
  if (sqlStateClass === transactionSqlStateClass) {
    return Option.some(
      new DatabaseTransactionFailure({ code, kind: 'sqlstate' })
    );
  }
  if (unavailableSqlStateClasses.has(sqlStateClass)) {
    return Option.some(
      new DatabaseDriverUnavailableFailure({ code, kind: 'sqlstate' })
    );
  }
  return unavailableSocketCodes.has(code)
    ? Option.some(
        new DatabaseDriverUnavailableFailure({ code, kind: 'socket' })
      )
    : Option.none();
};

const isUnavailableDriverCode = (code: string): boolean =>
  unavailableSqlStateClasses.has(code.slice(0, 2)) ||
  unavailableSocketCodes.has(code);

export const decodeDatabaseDriverFailure = (
  input: DatabaseDriverFailureInput
): Option.Option<DatabaseDriverFailure> =>
  Option.flatMap(
    findPostgresFailure(input, ({ code }) =>
      Option.isSome(decodeDriverCodeFailure(code))
    ),
    ({ code }) => decodeDriverCodeFailure(code)
  );

export const isDatabaseUnavailableFailure = (
  input: DatabaseDriverFailureInput
): boolean =>
  Option.exists(decodeDatabaseDriverFailure(input), ({ code }) =>
    isUnavailableDriverCode(code)
  );
