...
export type ScopedRoutineInvocationErrorCode =
  (typeof SCOPED_ROUTINE_INVOCATION_ERROR_CODES)[number];

const OwnerModuleKeySchema = Schema.String.check(
  Schema.isPattern(/^[a-z][a-z0-9]*(?:[.-][a-z0-9][a-z0-9-]*)*$/u),
).pipe(Schema.brand('ScopedRoutineOwnerModuleKey'), Schema.decodeTo(Schema.String));
const RoutineKeySchema = Schema.String.check(
  Schema.isPattern(/^[a-z][a-z0-9]*(?:[._-][a-z0-9]+)*$/u),
).pipe(Schema.brand('ScopedRoutineKey'), Schema.decodeTo(Schema.String));
