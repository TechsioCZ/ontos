...
const isAppWorkspace = Effect.fn('WorkspaceEnvironment.isAppWorkspace')(function* isAppWorkspaceEffect(
  candidate: string,
) {
...
const workspaceCandidates = (path: Path.Path, candidate: string): readonly string[] => {
  ...
  return parent === candidate ? [candidate, nestedApp] : [candidate, nestedApp, ...workspaceCandidates(path, parent)];
};
 ...
 */
export const resolveAppWorkspaceRootEffect = Effect.fn('WorkspaceEnvironment.resolveAppWorkspaceRootEffect')(
