...
export const ACTION_AUTHORIZATION_DENIED_PRINCIPAL_ID =
...
const hasInvalidActions = (
  ...
  return (
    actions.length === 0 ||
    actionKeys.some(
      (actionKey) => actionKey.length === 0 || actionKey.length > 256
    ) ||
...
const hasInvalidContexts = (
  contexts: readonly ActionAuthorizationContext[]
): boolean =>
  contexts.length === 0 ||
  contexts.some(
    ({ principalId, tenantId }) =>
      principalId.length === 0 || tenantId.length === 0
  ) ||
...
const isInvalidExplicitAssertionSet = (
  ...
  assertions.length < 2 ||
  new Set(assertions.map(({ principalId }) => principalId)).size !==
    assertions.length ||
  assertions.some(
    ({ expected, principalId }) =>
      principalId.length === 0 ||
      (expected !== 'allowed' && expected !== 'denied')
  ) ||
...
const assertProvisioningInput = (
  ...
  const deniedPrincipalId =
    input.deniedPrincipalId ?? ACTION_AUTHORIZATION_DENIED_PRINCIPAL_ID;
  if (
  ...
  const explicitActionAssertions = (
    input.explicitActionAssertions ?? []
  ).toSorted((left, right) => left.actionKey.localeCompare(right.actionKey));
  if (
    hasInvalidExplicitAssertions(explicitActionAssertions, explicitActionKeys)
  ) {
    throw failure(
      'action_authorization_input_invalid',
...
export const provisionActionAuthorization = Effect.fn(
  ...
  ActionAuthorizationProvisioningError
> {
  const { actions, contexts, deniedPrincipalId, explicitActionAssertions } =
    yield* Effect.try({
      catch: (error) =>
  ...
  const relationships = buildActionAuthorizationRelationships(
    defaultActionKeys,
    contexts
  );
  yield* callClient(
    ...
    (actionKey) =>
      Effect.forEach(
        ...
        Effect.andThen(
          checkNoPermission(
            client,
            actionExecuteRequest(actionKey, deniedPrincipalId)
          )
        )
      ),
    { concurrency: 1, discard: true }
