...
type ModuleStateDeniedOutcomeCode = Extract<
  ModuleStateAccessDecision,
  { readonly _tag: 'Denied' }
>['outcomeCode'];
...
export const checkModuleStateAccess = async ({
  ...
  const outcomeCodeByAccessKind = {
    load: 'module_state_load_blocked',
    mutate: 'module_state_mutate_blocked',
    read: 'module_state_read_blocked',
  } as const satisfies Record<ModuleStateAccessKind, ModuleStateDeniedOutcomeCode>;

  return {
    _tag: 'Denied',
    accessKind,
    moduleKey,
    outcomeCode: outcomeCodeByAccessKind[accessKind],
    state,
  };
};
