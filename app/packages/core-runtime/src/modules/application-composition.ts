...
const version = Schema.String.check(
  Schema.isPattern(/^[0-9]+(?:\.[0-9]+){0,2}$/u)
);
...
export const ApplicationCompositionModuleSchema = Schema.Struct({
  ...
  requiredCoreCapabilities: Schema.Array(
...
export type ApplicationCompositionModule =
  typeof ApplicationCompositionModuleSchema.Type;
...
const observedContractSchema = Schema.Struct({
  ...
  contributionKeys:
    ApplicationCompositionModuleSchema.fields.allowedContributions,
  deployment: OntosDeploymentIdentitySchema,
  federationExposes:
    ApplicationCompositionModuleSchema.fields.federation.fields.exposes,
...
export type ObservedApplicationCompositionContract =
  typeof observedContractSchema.Type;
export type ObservedModuleFederationManifest =
  typeof observedFederationSchema.Type;
export type ApplicationCompositionCandidateEvidence =
  typeof candidateEvidenceSchema.Type;
...
const identityKey = (
  identity: ApplicationCompositionVersionedIdentity
): string => `${identity.id}@${identity.version}`;
...
const sameVersionClaims = <Value extends { readonly version: string }>(
  ...
  const leftVersions = new Map(
    left.map((value) => [key(value), value.version])
  );
  const rightVersions = new Map(
    right.map((value) => [key(value), value.version])
  );
  return (
    ...
    [...leftVersions].every(
      ([claim, claimVersion]) => rightVersions.get(claim) === claimVersion
    )
  );
};
...
const assertAcyclicDependencies = Effect.fnUntraced(function* checkCycles(
  ...
  const dependencies = new Map(
    modules.map((module) => [module.moduleId, module.dependencies])
  );
  const visiting = new Set<string>();
  const visited = new Set<string>();
  const visit: (
    moduleId: string
  ) => Effect.Effect<void, ApplicationCompositionValidationError> = Effect.fn(
  ...
  yield* Effect.forEach(
    ...
    Effect.fnUntraced(function* checkDependency(dependency) {
      yield* claim(
        dependencies,
        dependency,
        `dependency in module ${module.moduleId}`
      );
...
const assertShellCompatibility = Effect.fnUntraced(function* checkCompatibility(
  ...
  if (
    identityKey(module.requiredShellAbi) !== identityKey(shell.contributionAbi)
  ) {
    return yield* new ApplicationCompositionValidationError({
  ...
  yield* Effect.forEach(
    ...
    Effect.fnUntraced(function* checkSingleton(singleton) {
      ...
      if (
        availableSingletons.get(singleton.packageName) !== singleton.version
      ) {
        return yield* new ApplicationCompositionValidationError({
...
const assertObservedFederationManifest = Effect.fnUntraced(
  function* checkFederation(
    module: ApplicationCompositionModule,
...
const assertObservedRuntime = Effect.fnUntraced(function* checkRuntime(
  ...
  if (
    identityKey(shell.contributionAbi) !==
      identityKey(runtime.contributionAbi) ||
    !sameVersionClaims(
      shell.coreCapabilities,
      runtime.coreCapabilities,
      ({ id }) => id
    ) ||
    !sameVersionClaims(
    ...
    return yield* new ApplicationCompositionValidationError({
      reason:
        'Shell and Core claims do not match the observed runtime contract',
    });
  }
  return yield* Effect.void;
});

const compositionJsonSchema = Schema.fromJsonString(
...
export const canonicalizeApplicationComposition = (
  ...
  encodeCompositionJson({
    ...
    modules: composition.modules
      .map((module) => ({
        ...
        requiredCoreCapabilities:
          module.requiredCoreCapabilities.toSorted(identityOrder),
    ...
    shell: {
      ...composition.shell,
      coreCapabilities:
        composition.shell.coreCapabilities.toSorted(identityOrder),
      sharedSingletons:
        composition.shell.sharedSingletons.toSorted(singletonOrder),
    },
  });

export const validateApplicationCompositionCandidate = Effect.fnUntraced(
  function* validate<Input>(
      ...
      Effect.catchTag('SchemaError', () =>
        Effect.fail(
          new ApplicationCompositionValidationError({
            reason:
              'candidate does not match the supported Application Composition schema',
          })
        )
      )
    );
    const observed = yield* Schema.decodeEffect(candidateEvidenceSchema)(
      evidence
    ).pipe(
      Effect.catchTag('SchemaError', () =>
        Effect.fail(
          new ApplicationCompositionValidationError({
            reason:
              'candidate evidence does not match the supported observation schema',
          })
        )
      )
    );
    const moduleIds = new Set(
      composition.modules.map(({ moduleId }) => moduleId)
    );
    ...
    const shellCapabilities = new Set(
      composition.shell.coreCapabilities.map(identityKey)
    );
    const shellSingletons = new Map(
      composition.shell.sharedSingletons.map(
        ({ packageName, version: singletonVersion }) => [
          packageName,
          singletonVersion,
        ]
      )
    ...
    yield* Effect.forEach(
      ...
      ({ packageName }) =>
        claim(shellSingletonPackages, packageName, 'shared singleton'),
      { concurrency: 1, discard: true }
    );
    ...
    yield* Effect.forEach(
      ...
      Effect.fnUntraced(function* validateModule(module) {
        ...
        if (
          ...
          [module.contract.url, manifestUrl].some(
        ...
        yield* claim(
          artifactUrls,
          new URL(module.contract.url).href,
          'artifact URL'
        );
        ...
        yield* assertObservedDeployment(
          module,
          observed.contracts[module.deployment.appId]
        );
        yield* assertObservedFederationManifest(
          module,
          observed.federationManifests[manifestUrl]
        );
        return yield* Effect.void;
      }),
      { concurrency: 1, discard: true }
