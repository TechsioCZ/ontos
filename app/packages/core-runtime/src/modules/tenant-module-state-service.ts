...
export type TenantModuleState = Schema.Schema.Type<
  typeof TenantModuleStateSchema
>;
...
export type ActiveTenantModule = Schema.Schema.Type<
  typeof ActiveTenantModuleSchema
>;
...
export type TenantModuleStateRecord = Schema.Schema.Type<
  typeof TenantModuleStateRecordSchema
>;

const TenantModuleStateChangeSourceSchema = Schema.Literals([
  'support',
  'system',
  'user',
]);
export type TenantModuleStateChangeSource =
  typeof TenantModuleStateChangeSourceSchema.Type;
...
export const resolveTenantModuleStateChangeSource = (
  ...
  TenantModuleStateChangeSource,
  TenantModuleStateUnsupportedChangeSourceError
> =>
  Match.value(authMethod).pipe(
    Match.when('session', () => Effect.succeed('user' as const)),
    Match.when('support_impersonation', () =>
      Effect.succeed('support' as const)
    ),
    ...
    Match.when('api_key', () =>
      Effect.fail(
        new TenantModuleStateUnsupportedChangeSourceError({
          ...
          reason:
            'This authentication method cannot change tenant module state',
        })
      )
    ),
    Match.exhaustive
...
export interface TenantModuleStateServiceContract {
  readonly getTenantModuleStates: (
    ...
    readonly TenantModuleStateRecord[],
    TenantModuleStateReadUnavailableError
  >;
  readonly listActiveTenantModules: (
    tenantId: string
  ) => Effect.Effect<
    readonly ActiveTenantModule[],
    TenantModuleStateReadUnavailableError
  >;
...
export const makeTenantModuleStateService = (database: {
  ...
  const decodeRows = (
    rows: readonly { readonly moduleKey: string; readonly state: unknown }[]
  ) =>
    Effect.forEach(
      rows,
      (row: (typeof rows)[number]) =>
        Schema.decodeUnknownEffect(TenantModuleStateSchema)(row.state).pipe(
          Effect.map((state) =>
            Object.freeze({ moduleKey: row.moduleKey, state })
          ),
    ...
    listActiveTenantModules: (tenantId) =>
      listTenantModuleStates(tenantId).pipe(
        Effect.map((rows) =>
          rows.flatMap((row) =>
            row.state === 'active'
              ? [{ moduleKey: row.moduleKey, state: 'active' as const }]
              : []
          )
        )
      ),
...
> {
  const { changeSource, tenantRows } =
    yield* resolveTenantModuleStateChangeSource(input.authMethod).pipe(
  ...
  if (
    ...
    input.expectedState !== effectivePreviousState
  ) {
  ...
  if (current === undefined) {
    ...
    const updated = yield* transaction
      ...
      .where(
        eq(tenantModuleStates.tenantModuleStateId, current.tenantModuleStateId)
      )
      .returning({
        tenantModuleStateId: tenantModuleStates.tenantModuleStateId,
      })
