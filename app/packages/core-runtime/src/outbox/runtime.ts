  ...
  >(
    ...
    Effect.gen(function* runOutboxCycleEffect() {
      ...
      const subscriptionsByKey = new Map(
        deployedSubscriptions.map((subscription) => [subscription.workerKey, subscription]),
      );
      for (const registration of validated.registrations) {
        const subscription = subscriptionsByKey.get(registration.descriptor.workerKey);
        if (
      ...
      if (deployedSubscriptions.length !== validated.registrations.length) {
        return yield* descriptorFailure(
