...
const resourceId = Schema.String.check(
  Schema.isMinLength(1),
  Schema.isMaxLength(300)
...
const tenantId = Schema.String.check(Schema.isUUID()).pipe(
  Schema.brand('TenantId')
);
const projectionVersion = Schema.String.check(
...
export const CoreSearchProjectionDocumentSchema = Schema.Struct({
  ...
  metadata: Schema.Array(CoreSearchMetadataFieldSchema).check(
...
export type CoreSearchProjectionDocument =
  typeof CoreSearchProjectionDocumentSchema.Type;
...
export const CoreSearchQuerySchema = Schema.Struct({
  ...
  facets: Schema.optionalKey(
    Schema.Array(CoreSearchFacetSchema).check(Schema.isMaxLength(20))
  ),
...
export const CoreSearchProjectionReplacementSchema = Schema.Struct({
  documents: Schema.Array(CoreSearchProjectionDocumentSchema).check(
...
export type CoreSearchProjectionReplacement =
...
export type CoreSearchProjectionMutation =
  typeof CoreSearchProjectionMutationSchema.Type;
...
const encodeProjectionUnitKey = Schema.encodeUnknownResult(
  projectionUnitKeyCodec
);
const projectionUnitKey = (
  tenant: string,
  module: string,
  type: string
): string => Result.getOrThrow(encodeProjectionUnitKey([tenant, module, type]));
const documentKey = ({ ref }: CoreSearchProjectionDocument): string =>
  ref.resourceId;
...
const invalidPeriod = ({
  ...
  return (
    from === undefined ||
    (validTo !== undefined && (to === undefined || to <= from))
  );
};
...
const hasInvalidDocumentFacets = (
  document: CoreSearchProjectionDocument
): boolean =>
...
const hasInvalidDocumentPeriods = (
  document: CoreSearchProjectionDocument
): boolean =>
...
const validateDocument = (
  ...
  if (
    ...
    return Result.fail(
      invalid('Core Search replacement contains an inconsistent document')
    );
  }
...
> => {
  ...
  for (const document of input.documents) {
    ...
    if (seen.has(key)) {
      return Result.fail(
        invalid('Core Search replacement contains a duplicate resource')
      );
    }
    seen.add(key);
  }
...
const encodeJsonValue = Schema.encodeUnknownResult(
...
const stableJson = (value: UnparsedCoreSearchInput): string => {
  ...
  if (Predicate.isObject(value)) {
    return `{${Object.entries(value)
      ...
      .map(
        ([key, entry]) =>
          `${Result.getOrThrow(encodeJsonValue(key))}:${stableJson(entry)}`
      )
      .join(',')}}`;
  }
...
const projectionDocumentEquivalence = Schema.toEquivalence(
...
const decodeReplacementEffect = (input: UnparsedCoreSearchInput) =>
  Schema.decodeUnknownEffect(CoreSearchProjectionReplacementSchema, {
    ...
    Effect.flatMap((replacement) =>
      Effect.fromResult(validateReplacement(replacement))
    )
  );
...
const sameStoredDocument = (
  ...
  current.document === undefined
    ? next === undefined
    : next !== undefined &&
      projectionDocumentEquivalence(current.document, next);
...
const shouldApplyMutation = (
  ...
  const order =
    BigInt(next.projectionVersion) - BigInt(current.projectionVersion);
  if (order < 0n) {
  ...
  return sameStoredDocument(current, next.document)
    ...
    : Result.fail(
        invalid('Core Search mutation reuses a version for different content')
      );
};
...
const shouldReplaceProjection = (
  ...
  return fingerprint === prior.fingerprint
    ? Result.succeed(false)
    : Result.fail(
        invalid('Core Search rebuild reuses a version for different content')
      );
};
...
export const makeInMemoryCoreSearchProjectionStore =
  (): CoreSearchProjectionStoreService => {
    const units = new Map<string, Map<string, Stored>>();
    const rebuilds = new Map<
      string,
      { readonly fingerprint: string; readonly version: bigint }
    >();
    ...
    const replace: CoreSearchProjectionStoreService['replace'] = Effect.fn(
      ...
      yield* Effect.fromResult(
        mergeReplacementDocuments(current, replacement.documents)
      );
...
const isEffectiveTemporalFacet = (
  ...
  const to =
    temporal.validTo === undefined
      ? undefined
      : toEpochMillis(temporal.validTo);
  return (
    temporal.key === key &&
    from !== undefined &&
    from <= effectiveAt &&
...
const matchesFacets = (
  ...
  requested.every(({ key, values }) => {
    const available = new Set(
      document.facets.find((candidate) => candidate.key === key)?.values
    );
...
type MutableProjectionHit = {
  -readonly [
    Key in keyof CoreSearchProjectionHit
  ]: CoreSearchProjectionHit[Key];
};
...
const matchDocument = (
  ...
  const alias = document.aliases?.find((candidate) =>
    matches([
      ...
      ...activeValues(candidate.temporalSearchableText),
    ])
  );
  if (alias === undefined) {
    return undefined;
...
> = Effect.gen(function* createCoreSearchQueryRuntimeService() {
  ...
  const search: CoreSearchQueryRuntimeService['search'] = Effect.fn(
    ...
    const query = yield* Schema.decodeUnknownEffect(CoreSearchQuerySchema)(
      input
    ).pipe(
      Effect.mapError((cause) =>
