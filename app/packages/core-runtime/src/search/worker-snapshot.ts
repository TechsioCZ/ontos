...
type SnapshotError =
  | CoreSearchProjectionInvalid
  | CoreSearchProjectionUnavailableError;
  ...
  read: <Value, Error>(
    ...
    return backend
      .run(context, (scope, executor, install) => {
        ...
        return read(snapshot.view).pipe(
...
export const makePostgresCoreSearchSnapshotBackend = (
  ...
  const run: CoreSearchSnapshotBackend['run'] = Effect.fn(
    'CoreSearchSnapshotBackend.runPostgres'
    ...
    const transactionProgram = Effect.fn(
      'CoreSearchSnapshotBackend.transaction'
        ...
        readSnapshot(
          {
            ...
            legalEntityIds: Object.freeze(
              entities.map(({ legalEntityId }) => legalEntityId)
            ),
      ...
      database.executor
        .transaction(
          Effect.fn('snapshotTransactionEffect')(
            function* snapshotTransactionEffect(transaction: CoreTransaction) {
...
export const CoreSearchWorkerSnapshotLive = Layer.effect(
  ...
  Effect.gen(function* makeCoreSearchWorkerSnapshotLive() {
    ...
    return makeCoreSearchWorkerSnapshot(
      makePostgresCoreSearchSnapshotBackend(database)
    );
  })
);
