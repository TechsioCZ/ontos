...
const TestPersistenceErrorContract = Schema.TaggedStruct(
...
const TestDomainRejected = Schema.TaggedError<TestDomainRejectedSelf>()(
  'TestDomainRejected',
  {
    reason: Schema.String,
  }
...
const ActionPolicyDeniedFailureSchema = Schema.TaggedStruct(
  'ActionPolicyDenied',
  {
...
const decodeActionPolicyDeniedFailure = Schema.decodeUnknownOption(
  ActionPolicyDeniedFailureSchema
);
  ...
  const transactionOverride = {
    transaction: (transactionBody) =>
      database.executor.transaction((transaction) => {
        ...
        const operation =
          stage === 'invocation-success' ? 'update' : 'insert into';
...
const liveModuleStateOptions = (database: ContextServiceContract) => {
  const moduleStateGate = makeModuleStateGate(
  ...
  withDatabase(
    Effect.fn(function* integrationProgram2(database) {
      yield* Effect.forEach(
        ...
        (table) =>
          database.executor.delete(table).where(eq(table.tenantId, tenantId)),
        { discard: true }
      );
    })
  ))();
...
type TestActionContext = ActionHandlerContext<
  typeof TestDomainEvents,
  TestActionServices
>;
...
const makeRegistration = ({
  ...
  defineAction(
    ...
    Effect.fn(function* integrationHandler(
      payload,
...
const hasFailure = <Error>(
  exit: Exit.Exit<unknown, Error>,
  tag: string
): boolean =>
  Exit.isFailure(exit) &&
  Option.exists(Cause.findErrorOption(exit.cause), Predicate.isTagged(tag));
  ...
  withDatabase(
    Effect.fn(function* integrationProgram3(database) {
      ...
      const [openInvocation] = yield* database.executor
        ...
        .where(
          eq(
            actionInvocations.idempotencyKey,
            'business-module-concurrent-gate'
          )
        );
      ...
      const deniedEvidence = yield* database.executor
        ...
        .from(auditEvents)
        .where(
          eq(auditEvents.actionInvocationId, openInvocation.actionInvocationId)
        );
...
const testProgram2 = Effect.fn(function* integrationProgram4() {
  ...
  yield* withDatabase(
    Effect.fn(function* integrationProgram5(database) {
      ...
      const [states, invocations, audits, accesses, events, messages] =
        yield* Effect.all([
          database.executor
            .select()
      ...
      expect(
        audits.filter(
          (row) => row.actionInvocationId === invocations[0]?.actionInvocationId
        ).length
      ).toBe(1);
      expect(
        accesses.filter(
      ...
      expect(
        messages.filter((row) => row.domainEventId === events[0]?.domainEventId)
          .length
      ).toBe(1);
...
const testProgram3 = Effect.fn(function* integrationProgram6() {
  ...
  yield* withDatabase(
    Effect.fn(function* integrationProgram7(database) {
      ...
      const policyAudit = audits.find(
        (row) => row.eventType === 'action.policy_checked'
      );
      const executionAudit = audits.find(
        (row) => row.eventType === 'action.executed'
      );
      ...
      expect(encodeJson(policyAudit?.evidenceJson).includes('committed')).toBe(
        false
      );
      expect(
        encodeJson(policyAudit?.evidenceJson).includes(policy.policyKey)
      ).toBe(true);
    })
  );
});
...
const testProgram4 = Effect.fn(function* integrationProgram8() {
  const scenarios = [
        ...
        const policy = defineGlobalPolicy<{ readonly value: string }>({
          evaluate: () =>
            Effect.fail(
              denyPolicy(
                'tenant_suspended',
                'This tenant is suspended — contact support'
              )
            ),
    ...
    },
    {
      ...
      makeRegistration(handler: () => void) {
        const policy = defineMicroverticalPolicy<
          { readonly value: string },
          'inventory.stock'
        >({
          evaluate: () =>
            Effect.fail(
              denyPolicy('stock_locked', 'Stock is locked for reconciliation')
            ),
          owningModuleKey: 'inventory.stock',
        ...
        Effect.fn(function* integrationProgram10(scenario) {
          ...
          const failure = Exit.isFailure(exit)
            ? Option.flatMap(
                ...
                decodeActionPolicyDeniedFailure
              )
            : Option.none();
          ...
          const [audits, accesses, events, states] = yield* Effect.all([
            ...
            database.executor
              ...
              .where(
                eq(
                  domainEvents.actionInvocationId,
                  invocation.actionInvocationId
                )
              ),
            database.executor
...
const testProgram5 = Effect.fn(function* integrationProgram11() {
  const policy = defineGlobalPolicy<{ readonly value: string }>({
    evaluate: () =>
      Effect.fail(denyPolicy('blocked', 'This operation is blocked')),
    ...
    Effect.fn(function* integrationProgram12(database) {
      ...
      yield* Effect.forEach(
        ...
        Effect.fn(function* integrationProgram13(stage) {
          ...
          const { audits, invocation } = yield* invocationEvidence(
            database,
            key
          );
    ...
    Effect.fn(function* integrationProgram18(database) {
      ...
      const policy = defineGlobalPolicy<{ readonly value: string }>({
        evaluate: () => {
          evaluations += 1;
          return Effect.fail(
            denyPolicy('terminal_rejection', 'This rejection is terminal')
          );
        },
      ...
      const concurrentAudits = yield* database.executor
        ...
        .where(
          eq(
            auditEvents.actionInvocationId,
            concurrentInvocation.actionInvocationId
          )
        );
  ...
  withDatabase(
    Effect.fn(function* integrationProgram19(database) {
      ...
      const denial = defineGlobalPolicy<{ readonly value: string }>({
        evaluate: () =>
          Deferred.succeed(denialEvaluated, null).pipe(
            Effect.andThen(
              Effect.fail(
                denyPolicy('late_denial', 'This denial arrived too late')
              )
            )
          ),
      ...
      const rejected = yield* Effect.forkScoped(
        Effect.exit(
          deniedRuntime.runAction({ ...sharedInput, registration: denied })
        )
      );
      ...
      expect(hasFailure(rejectedExit, 'ActionInvocationPersistenceError')).toBe(
        true
      );
      expect(invocation.status).toBe('succeeded');
      expect(
        audits.filter((row) => row.eventType === 'action.rejected').length
      ).toBe(0);
    })
  );
  ...
  withDatabase(
    Effect.fn(function* concurrencyProof(database) {
      ...
      const firstAttempt = yield* Effect.exit(
        runtime.runAction(concurrentInput)
  ...
  withDatabase(
    Effect.fn(function* integrationProgram20(database) {
      ...
      const firstEvent = events.find(
        (event) => event.subjectResourceId === firstModule
      );
      const secondEvent = events.find(
        (event) => event.subjectResourceId === secondModule
      );
      ...
      expect(
        firstEvent.tenantSequenceNo < secondEvent.tenantSequenceNo
      ).toBeTruthy();
    })
  );
...
const testProgram12 = () =>
  withDatabase(
    Effect.fn(function* integrationProgram21(database) {
      ...
      expect(hasFailure(committedResolution, 'ActionAlreadyCommitted')).toBe(
        true
      );
      expect(
        hasFailure(unauthorizedResolution, 'ActionInvocationNotFound')
      ).toBe(true);
      expect(
        hasFailure(unavailableResolution, 'ActionCommitIndeterminate')
      ).toBe(true);
          ...
          database.executor
            .transaction((transaction) =>
              transactionBody(transaction).pipe(
                Effect.andThen(
                  Effect.die(
                    new Error('force rollback after the transaction body')
                  )
                )
              )
            )
...
it.layer(
  Layer.effectDiscard(
    Effect.acquireRelease(prepare, () => cleanup.pipe(Effect.orDie))
  ),
  {
  ...
  suite.effect(
    'rolls back every denied-Policy finalization persistence failure',
    testProgram5
  );
  ...
  suite.effect(
    'rolls back every individual success-evidence persistence failure',
    testProgram7
  );
  ...
  suite.effect(
    'serializes Domain Event allocation by tenant commit order',
    testProgram11
  );
