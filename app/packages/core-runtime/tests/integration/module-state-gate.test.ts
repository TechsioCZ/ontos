...
it.live(
  ...
  () =>
    Effect.gen(function* moduleStateGate1() {
      ...
      const stateModuleKey = (
        state: (typeof TENANT_MODULE_STATES)[number]
      ): string => `${moduleKey}.${state.replaceAll('_', '-')}`;
      ...
      let selects = 0;
      const countingExecutor: DatabaseService['executor'] = Object.create(
        database.executor
      );
      ...
      const snapshot = yield* gate.prepareSnapshot(tenantOne, [
        read,
        read,
        write,
      ]);
      ...
      for (const [index, state] of TENANT_MODULE_STATES.entries()) {
        ...
        expect(descriptor).toBeDefined();
        const exit = Option.getOrThrow(
          Option.fromNullishOr(persistedStateExits[index])
        );
      ...
      const quarantined = yield* Effect.flip(
        gate.check(tenantTwoSnapshot, read)
      );
      expect(Predicate.isTagged(quarantined, 'ModuleStateDeniedError')).toBe(
        true
      );
      ...
      const missingSnapshot = yield* gate.prepareSnapshot(tenantOne, [
        missingDescriptor,
      ]);
      const missing = yield* Effect.flip(
        gate.check(missingSnapshot, missingDescriptor)
      );
      ...
      expect(Predicate.isTagged(lockedDenial, 'ModuleStateDeniedError')).toBe(
        true
      );

      const unavailable = yield* Effect.flip(
        makeModuleStateGate(
          unavailableStateService('secret db failure')
        ).prepareSnapshot(tenantOne, [read])
      );
      expect(
        Predicate.isTagged(unavailable, 'ModuleStateCheckUnavailableError')
      ).toBe(true);
      expect(unavailable.reason).not.toMatch(/secret|db failure/u);

      const malformed = yield* Effect.flip(
        makeModuleStateGate(
          unavailableStateService('corrupt-storage-value')
        ).prepareSnapshot(tenantOne, [read])
      );
      expect(
        Predicate.isTagged(malformed, 'ModuleStateCheckUnavailableError')
      ).toBe(true);
