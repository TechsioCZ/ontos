...
const matchedWorker = Effect.fn(function* matchedWorker(
  ...
  const messages = yield* Effect.forEach(
    Array.from({ length: options.messages ?? 1 }),
...
it.live(
  'matches zero, one, or multiple exact workers once without historical backfill',
  () =>
    Effect.gen(function* matchesZeroOneOrMultiple() {
      ...
      const repository = makeOutboxRepository(database);
      const workers = [
        makeWorker('consumer.alpha'),
        makeWorker('consumer.beta'),
      ];
      ...
      ).toEqual(['consumer.alpha', 'consumer.beta']);
...
it.live(
  'matches the complete subscription catalog before owner-local processes claim work',
  () =>
    Effect.gen(function* matchesTheCompleteSubscriptionCatalog() {
      ...
      const repository = makeOutboxRepository(database);
      const subscriptions = [consumerWorker, reportingWorker].map(
        subscriptionOf
      );
...
it.live(
  ...
  () =>
    Effect.gen(function* gatesClaimsOnEveryNonactive() {
      ...
      expect(
        Option.getOrNull(
          yield* repository.claimNext([registration], 'runtime-a', claimAt)
        )
      ).toBe(null);
...
it.live(
  ...
  () =>
    Effect.gen(function* reclaimsOnlyExpiredLeasesAbandons() {
      ...
      expect(Option.isSome(Option.fromNullishOr(attempts[0]?.finishedAt))).toBe(
        true
      );
...
it.live(
  'finishes an abandoned final attempt before dead-lettering its expired delivery',
  () =>
    Effect.gen(function* finishesAnAbandonedFinalAttempt() {
      ...
      expect(attempt?.errorMessage).toBe(
        'Outbox Worker lease expired before completion'
      );
      expect(Option.isSome(Option.fromNullishOr(attempt?.finishedAt))).toBe(
        true
      );
    })
);
it.live(
  'finalizes success atomically and advances only through contiguous done deliveries',
  () =>
    Effect.gen(function* finalizesSuccessAtomicallyAndAdvances() {
      ...
      const secondMessage = Option.getOrThrow(
        Option.fromNullishOr(messages[1])
      );
      const first = Option.getOrThrow(
        yield* repository.claimNext([registration], 'runtime-a', now)
      );
      const second = Option.getOrThrow(
        yield* repository.claimNext([registration], 'runtime-b', now)
      );
      expect(first).toBeDefined();
      expect(second).toBeDefined();
      ...
      expect(checkpoint.lastTenantSequenceNo).toBe(
        secondMessage.tenantSequenceNo
      );
      expect(
        Option.getOrThrow(
          Option.fromNullishOr(checkpoint.lastTenantSequenceNo)
