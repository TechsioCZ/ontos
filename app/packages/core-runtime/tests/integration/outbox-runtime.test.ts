...
/**
 * Every scenario owns one throwaway tenant and must drop it even when an assertion fails, so the
...
const withTenant = async (
  ...
  await withDatabase(async (database) => {
    ...
    try {
      await scenario(database, tenantId);
    } finally {
...
/**
 * The claim scenarios all start from a registered worker whose pending messages are already
 * matched, and they all claim against a clock advanced past the seeded rows.
 */
const matchedWorker = async (
  database: CoreDatabaseExecutor,
  tenantId: string,
  workerKey: string,
  ...
  readonly now: Date;
  readonly registration: AnyOutboxWorkerRegistration;
  readonly repository: OutboxRepositoryService;
}> => {
  await runEffectTestPromise(activateConsumer(database, tenantId));
  const messages: Awaited<ReturnType<typeof insertMessage>>[] = [];
...
void test('matches zero, one, or multiple exact workers once without historical backfill', async () => {
  await withTenant(async (database, tenantId) => {
...
void test('schedules bounded retry, dead-letters exhaustion, stores safe errors, and never checkpoints failure', async () => {
  await withTenant(async (database, tenantId) => {
    const { now, registration, repository } = await matchedWorker(
      database,
      tenantId,
