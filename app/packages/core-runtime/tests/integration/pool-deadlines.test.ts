...
const hasTimeoutMessage = (error: PostgresError): boolean =>
...
void test('applies PostgreSQL pool connection and statement deadlines', async () => {
  ...
  const { databaseConfiguration, poolConfiguration } = await Effect.runPromise(
    Effect.gen(function* loadPoolConfiguration() {
      ...
      const pool = yield* configureDatabasePool(
        Redacted.make(database.connectionString),
        {
  ...
  try {
    ...
    try {
      const settings = await client.query<{ statement_timeout: string }>(
