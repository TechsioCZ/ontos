...
it.live(
  ...
  () =>
    Effect.gen(function* searchPersistenceIntegration() {
      ...
      expect(yield* encodeJson(partyHits)).not.toMatch(
      ...
      expect(
        Predicate.isTagged(divergence, 'CoreSearchProjectionInvalid')
      ).toBe(true);
      ...
      expect(counterpartyHits.map(({ ref }) => ref.resourceId)).toEqual([
        counterpartyId,
      ]);
      expect(
        yield* search.search({
