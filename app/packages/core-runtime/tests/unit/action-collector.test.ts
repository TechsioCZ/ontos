...
it.effect('does not expose externally mutable collector arrays or captured payloads', () =>
...
it.effect('captures one immutable JSON audit-evidence object and rejects invalid repeats', () =>
  Effect.gen(function* capturesOneImmutableJSONAuditevidenceObjectAndRejects() {
    const collector = makeCollector();
    ...
    expect(Predicate.isTagged(undeclared, 'ActionCollectorError')).toBe(true);
    expect(Predicate.isTagged(missingSchema, 'ActionCollectorError')).toBe(true);
  }),
);

it.effect('applies descriptor evidence policy and rejects incompatible evidence', () =>
  Effect.gen(function* appliesDescriptorEvidencePolicyAndRejectsIncompatibleEvidence() {
...
it.effect('rejects an Outbox producer that differs from its registered Domain Event', () =>
...
it.effect('enforces Action-declared event payloads and producer ownership', () =>
  Effect.gen(function* enforcesActiondeclaredEventPayloadsAndProducerOwnership() {
    const collector = makeCollector();
    ...
    expect(Predicate.isTagged(invalidPayload, 'ActionCollectorError')).toBe(true);
    expect(Predicate.isTagged(invalidProducer, 'ActionCollectorError')).toBe(true);
    expect(Predicate.isTagged(undeclared, 'ActionCollectorError')).toBe(true);
    expect(Predicate.isTagged(inheritedName, 'ActionCollectorError')).toBe(true);
