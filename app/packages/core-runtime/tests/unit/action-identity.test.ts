...
it('identity Actions are generated, sensitive, idempotent, and owned by Core identity', () => {
  for (const registration of registrations) {
    expect(registration.descriptor.actionKey.startsWith('core.identity.')).toBe(
      true
    );
    ...
    expect(registration.descriptor.accessEvidencePolicy.captureMode).toBe(
      'metadata_only'
    );
...
it.effect(
  'identity administration and support starts declare independent tenant permissions',
  () =>
    Effect.gen(function* identityScenario2() {
      ...
      const managedPermissions = [
        bindManagedApiKeyAction.descriptor.tenantPermission?.(
          yield* Schema.decodeEffect(
            bindManagedApiKeyAction.descriptor.payloadSchema
          )({
            principalId,
        ...
        changePrincipalStatusAction.descriptor.tenantPermission?.(
          yield* Schema.decodeEffect(
            changePrincipalStatusAction.descriptor.payloadSchema
          )({
        ...
        createNonHumanPrincipalAction.descriptor.tenantPermission?.(
          yield* Schema.decodeEffect(
            createNonHumanPrincipalAction.descriptor.payloadSchema
          )({
      ...
      expect(setSelfApiKeyBindingStatusAction.descriptor.tenantPermission).toBe(
        undefined
      );
...
it('identity status schemas require reasons for disabling, archiving, and revoking', () => {
  ...
  expect(() =>
    Schema.decodeUnknownSync(
      setSelfApiKeyBindingStatusAction.descriptor.payloadSchema
    )({
      authBindingId,
      expectedStatus: 'active',
      newStatus: 'revoked',
...
it.effect(
  'support checkpoints forbid unsafe or misplaced session references',
  () =>
    Effect.gen(function* identityScenario4() {
      ...
      for (const sessionRef of [
        'raw-session-token',
        'better-auth-session:contains whitespace',
      ]) {
        expect(
          yield* Effect.flip(
            decode({
