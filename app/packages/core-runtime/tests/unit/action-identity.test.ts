...
void test('identity Actions are generated, sensitive, idempotent, and owned by Core identity', () => {
  for (const registration of registrations) {
    assert.equal(
      registration.descriptor.actionKey.startsWith('core.identity.'),
      true
    );
    ...
    assert.equal(
      registration.descriptor.accessEvidencePolicy.captureMode,
      'metadata_only'
    );
...
void test('identity administration and support starts declare independent tenant permissions', () => {
  ...
  assert.equal(
    setSelfApiKeyBindingStatusAction.descriptor.tenantPermission,
    undefined
  );
