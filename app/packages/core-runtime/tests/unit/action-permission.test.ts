...
const makeClient = (
  responses: readonly Effect.Effect<
...
it.effect(
  'loads the root SpiceDB environment independently of the invocation directory',
  () =>
    Effect.gen(function* loadsTheRootSpiceDBEnvironmentIndependentlyOfThe() {
      const originalDirectory = process.cwd();
      const rootExamplePath = SPICEDB_ROOT_ENV_PATH.replace(
        /\.env$/u,
        '.env.example'
      );
...
it.effect(
  'requires complete configuration and explicit secure or localhost-insecure transport',
  () =>
    Effect.gen(
      function* requiresCompleteConfigurationAndExplicitSecureOrLocalhostinsecure() {
        ...
        const failures = yield* Effect.forEach(
          [
            ...
            {
              SPICEDB_ENDPOINT:
                'spicedb.internal.example:443?credential=test-key',
              SPICEDB_INSECURE: 'false',
              SPICEDB_PRESHARED_KEY: 'test-key',
            },
            {
...
it.effect(
  'allows insecure transport only for the exact Zerops stage private endpoint',
  () =>
...
it.effect(
  'losslessly maps Action keys and exact principal identities using fully consistent requests',
  () =>
    Effect.gen(function* losslesslyMapsActionKeysAndExactPrincipalIdentities() {
      ...
      expect(toSpiceDbActionObjectId(input.actionKey)).toBe(
        'ak_aW52ZW50b3J5LnN0b2NrLnJlc2VydmU'
      );
...
it.effect(
  'classifies fully consistent execute permission as allowed or denied with one check',
  () =>
    Effect.gen(
      function* classifiesFullyConsistentExecutePermissionAsAllowedOr() {
...
it.effect(
  'report-only compatibility distinguishes missing policy from an explicit restriction',
  () =>
    Effect.gen(
      function* reportonlyCompatibilityDistinguishesMissingPolicyFromAnExplicit() {
...
it.effect(
  'fails closed for conditional, unspecified, malformed, and client failures',
  () =>
    Effect.gen(
      function* failsClosedForConditionalUnspecifiedMalformedAndClient() {
        const failures = yield* Effect.forEach(
          [
            ...
            makeClient([
              response(v1.CheckPermissionResponse_Permissionship.UNSPECIFIED),
            ]),
            ...
            makeClient([
              Effect.fail(
                spiceDbPermissionClientError(
                  new Error(
                    'ontos-local-development-key unavailable at internal host'
                  )
                )
              ),
            ]),
        ...
        for (const failure of failures) {
          ...
          expect(failure.reason.includes('ontos-local-development-key')).toBe(
            false
          );
...
it.effect(
  'constructs the live client with a bounded deadline and finalizes it with the scope',
  () =>
...
it.effect(
  'turns missing live configuration into a fail-closed permission service',
  () =>
    Effect.gen(
      function* turnsMissingLiveConfigurationIntoAFailclosedPermission() {
