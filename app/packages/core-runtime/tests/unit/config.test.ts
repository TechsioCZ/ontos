...
it.effect(
  'loads the root environment independently of the invocation directory',
  () =>
...
it.effect(
  'keeps missing and malformed configuration in the typed error channel',
  () =>
    Effect.gen(function* migratedTest() {
...
it.effect(
  'requires distinct administrative and least-privilege runtime identities',
  () =>
    Effect.gen(function* migratedTest() {
      ...
      const queryParameterIdentities = yield* parseDatabaseConnectionPair({
        DATABASE_ADMIN_URL:
          'postgresql://connection-proxy@localhost:5433/ontos?user=ontos_admin',
        DATABASE_URL:
          'postgresql://connection-proxy@localhost:5433/ontos?user=ontos_runtime',
      });
      const queryParameterCollision = yield* Effect.flip(
        parseDatabaseConnectionPair({
          DATABASE_ADMIN_URL:
            'postgresql://admin-authority@localhost:5433/ontos?user=effective_role',
          DATABASE_URL:
            'postgresql://runtime-authority@localhost:5433/ontos?user=effective_role',
        })
      );
      ...
      expect(
        Predicate.isTagged(queryParameterCollision, 'DatabaseConfigError')
      ).toBe(true);
      expect(
        Predicate.isTagged(superuserCompatible, 'DatabaseConfigError')
      ).toBe(true);
    })
);
