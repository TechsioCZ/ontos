...
it.effect(
  ...
  () =>
    Effect.gen(function* scopeClassification() {
      ...
      const allowed = makeOperationalScopeResolver(
        repository,
        access('allowed')
      );
      ...
      const { legalEntityId: _legalEntityId, ...principalWithoutLegalEntity } =
        principal;
  ...
  () =>
    Effect.gen(function* systemProvenance() {
      ...
      const untrusted = yield* Effect.exit(
        decodeTrustedPrincipalContext({ ...scope })
      );
...
it.effect(
  ...
  () =>
    Effect.gen(function* supportRecovery() {
      ...
      expect(isTrustedSupportRecoveryPrincipalContext({ ...scope })).toBe(
        false
      );
    })
);
