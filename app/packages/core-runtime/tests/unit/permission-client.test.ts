...
type PermissionRpcError = NonNullable<
  Parameters<
    NonNullable<Parameters<v1.PermissionsServiceClient['checkPermission']>[3]>
  >[0]
>;
...
it.effect('permission RPCs are lazy and execute again on each Effect run', () =>
  Effect.gen(function* checksLazyRpcExecution() {
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => rstest.restoreAllMocks())
    );
    const check = rstest
      ...
      .mockImplementation((request, metadata) => {
        ...
        return rstest.fn<v1.PermissionsServiceClient['checkPermission']>()(
          request,
          metadata
        );
      });
    const bulk = rstest
      ...
      .mockImplementation((request, metadata) => {
        ...
        return rstest.fn<v1.PermissionsServiceClient['checkBulkPermissions']>()(
          request,
          metadata
        );
      });
    const client = yield* Effect.acquireRelease(
      Effect.sync(() =>
        createSpiceDbPermissionClient(configuration, SPICEDB_CHECK_TIMEOUT_MS)
      ),
  ...
  () =>
    Effect.gen(function* checksPermissionDeadline() {
      ...
      rstest
        .spyOn(v1.PermissionsServiceClient.prototype, 'checkPermission')
        .mockImplementation((request, metadata) =>
          rstest.fn<v1.PermissionsServiceClient['checkPermission']>()(
            request,
            metadata
          )
        );
