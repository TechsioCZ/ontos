...
const repository = (overrides: Partial<PrincipalManagementPersistence>): PrincipalManagementRepositoryService =>
  principalManagementRepositoryFromPersistence({
    ...repositoryDefaults,
    ...overrides,
  });
type OptionValue<Outcome> = Outcome extends Option.Option<infer Value> ? Value : never;
type PrincipalRecord = OptionValue<Effect.Success<ReturnType<PrincipalManagementPersistence['loadPrincipal']>>>;
type ApiKeyBindingRecord = OptionValue<Effect.Success<ReturnType<PrincipalManagementPersistence['loadApiKeyBinding']>>>;
const selectingPrincipal = (record: PrincipalRecord | undefined) =>
  repository({
    loadPrincipal: () => Effect.succeed(Option.fromNullishOr(record)),
  });
const selectingBinding = (record: ApiKeyBindingRecord | undefined) =>
  repository({
    loadApiKeyBinding: () => Effect.succeed(Option.fromNullishOr(record)),
  });
const repositoryForSupportParticipants = (results: readonly (readonly { readonly authBindingId: string }[])[]) => {
  let call = 0;
  return repository({
    loadSupportBindings: () =>
      Effect.sync(() => {
...
it.effect('rejects human principal administration and managed keys targeting humans', () =>
  Effect.gen(function* rejectsHumanPrincipalAdministration() {
    const transaction = selectingPrincipal({
      kind: 'human',
      status: 'active',
    });
...
it.effect('binds only eligible active self and managed principal kinds without secret material', () =>
...
it.effect('requires exactly one active tenant-local user binding for both impersonation participants', () =>
  Effect.gen(function* validatesSupportParticipants() {
    ...
    const error = yield* Effect.flip(
      validateSupportImpersonation(input).pipe(provideRepository(repositoryForSupportParticipants([original, []]))),
    );
