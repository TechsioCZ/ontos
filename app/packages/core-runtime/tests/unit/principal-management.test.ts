...
type OptionValue<Outcome> =
  Outcome extends Option.Option<infer Value> ? Value : never;
...
type ApiKeyBindingRecord = OptionValue<
  Effect.Success<
    ReturnType<PrincipalManagementPersistence['loadApiKeyBinding']>
  >
>;
...
it.effect(
  ...
  () =>
    Effect.gen(function* rejectsHumanPrincipalAdministration() {
      ...
      expect(
        Predicate.isTagged(principalError, 'IdentityTargetInvalidError')
      ).toBe(true);
      expect(
        Predicate.isTagged(bindingError, 'IdentityTargetInvalidError')
      ).toBe(true);
    })
);
it.effect(
  ...
  () =>
    Effect.gen(function* enforcesBindingLifecycle() {
      ...
      expect(
        Predicate.isTagged(conflictError, 'IdentityLifecycleConflictError')
      ).toBe(true);
      expect(
        Predicate.isTagged(terminalError, 'IdentityLifecycleConflictError')
      ).toBe(true);
      expect(
      ...
      yield* Effect.forEach(
        ...
        (record) =>
          Effect.gen(function* rejectsIneligibleBindingTarget() {
            ...
            expect(
              Predicate.isTagged(error, 'IdentityTargetInvalidError')
            ).toBe(true);
          }),
        { concurrency: 1 }
      );
  ...
  () =>
    Effect.gen(function* bindsEligiblePrincipal() {
      ...
      const transaction = repository({
        ...
        loadPrincipal: () =>
          Effect.succeedSome({ kind: 'service', status: 'active' }),
      });
...
it.effect('maps an existing API key binding to a lifecycle conflict', () =>
  Effect.gen(function* mapsExistingBindingToConflict() {
    ...
    expect(Predicate.isTagged(error, 'IdentityLifecycleConflictError')).toBe(
      true
    );
  })
);
it.effect(
  ...
  () =>
    Effect.gen(function* validatesSupportParticipants() {
      ...
      expect(Predicate.isTagged(error, 'IdentityTargetInvalidError')).toBe(
        true
      );
