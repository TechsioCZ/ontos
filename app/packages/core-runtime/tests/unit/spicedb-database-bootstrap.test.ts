...
it.layer(NodeFileSystem.layer)('SpiceDB bootstrap sources', (suite) => {
  suite.effect(
    'keeps the stage bootstrap schema aligned without development relationships',
    () =>
  ...
  suite.effect('grants fresh development module access only to Contacts', () =>
    Effect.gen(function* testScenario2() {
      ...
      const contactsObjectId = toModuleAccessObjectId(
        tenantId,
        legalEntityId,
        'contacts.core'
      );
      expect(
        contactsObjectId !== undefined && contactsObjectId.length > 0
      ).toBe(true);
      expect(
        development.match(
    ...
    Effect.gen(function* testScenario3() {
      ...
      for (const permission of [
        ...
        expect(development).toMatch(
          new RegExp(`permission ${permission} =`, 'u')
        );
      }
    })
  );
  ...
  suite.effect(
    'declares the Counterparty Legal Entity permission vocabulary',
    () =>
