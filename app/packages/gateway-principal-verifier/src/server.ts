...
export type ActionPrincipalMissingError =
  typeof ActionPrincipalMissingErrorSchema.Type;
export const ActionPrincipalInvalidErrorSchema = Schema.TaggedStruct(
  'ActionPrincipalInvalidError',
  errorFields
);
export type ActionPrincipalInvalidError =
  typeof ActionPrincipalInvalidErrorSchema.Type;
export const ActionPrincipalExpiredErrorSchema = Schema.TaggedStruct(
  'ActionPrincipalExpiredError',
  errorFields
);
export type ActionPrincipalExpiredError =
  typeof ActionPrincipalExpiredErrorSchema.Type;
export const ActionPrincipalScopeErrorSchema = Schema.TaggedStruct(
  'ActionPrincipalScopeError',
  errorFields
);
export type ActionPrincipalScopeError =
  typeof ActionPrincipalScopeErrorSchema.Type;
export const ActionPrincipalConfigurationErrorSchema = Schema.TaggedStruct(
  'ActionPrincipalConfigurationError',
  errorFields
);
export type ActionPrincipalConfigurationError =
  typeof ActionPrincipalConfigurationErrorSchema.Type;
export const ActionPrincipalUnavailableErrorSchema = Schema.TaggedStruct(
  'ActionPrincipalUnavailableError',
  errorFields
);
export type ActionPrincipalUnavailableError =
  typeof ActionPrincipalUnavailableErrorSchema.Type;
...
type GatewayPrincipalVerificationEnvironmentOptions =
...
const PublicVerificationKeySchema = Schema.Struct({
  ...
  key_ops: Schema.optionalKey(
    Schema.Array(Schema.Literal('verify')).check(Schema.isMinLength(1))
  ),
...
const VerificationEnvironmentSchema = Schema.Struct({
  ...
  ONTOS_GATEWAY_PUBLIC_JWKS: Schema.fromJsonString(
    PublicVerificationKeysSchema
  ),
});
...
> => {
  ...
  return configuration.pipe(
    ...
    Effect.flatMap(
      ({ ONTOS_GATEWAY_ISSUER: issuer, ONTOS_GATEWAY_PUBLIC_JWKS: jwks }) => {
...
const decodeVerificationFailure = Schema.decodeUnknownOption(
...
const readBearer = (
  ...
  const token = /^Bearer (?<token>[^\s]+)$/iu.exec(authorizationValue)
    ?.groups?.['token'];
  return token === undefined
    ? Effect.fail(invalidError())
...
const verifyAuthenticatedToken = Effect.fn(
  'GatewayPrincipalVerifier.verifyAuthenticatedToken'
  ...
  > {
    const verification = readBearer(authorization).pipe(
      Effect.flatMap((token) =>
        verifyAuthenticatedToken(expectedAudience, token, options)
      )
    );
