...
test('an audience-bound verifier accepts only its exact topology app ID', async () => {
  ...
  const verify = async (token: string, environment: typeof partyFixture.environment) =>
  ...
  assert.deepEqual(await verify(partyFixture.token, partyFixture.environment), principal);
...
test('redemption failures remain sanitized and distinguish replay from unavailability', async () => {
  const fixture = await makeFixture('party-registry');
  const verifier = bindGatewayPrincipalVerifier('party-registry');
  const verify = async (redemption: Parameters<typeof verifier.verifyAndRedeem>[1]['redemption']) =>
    await runEffectTestPromise(
...
test('malformed Ed25519 public keys fail during configuration acquisition', async () => {
  ...
  await Promise.all([
    assert.rejects(verifyWithKey({ ...fixture.publicJwk, key_ops: [] }), isConfigurationError),
    assert.rejects(verifyWithKey({ ...fixture.publicJwk, x: '!!!' }), isConfigurationError),
  ]);
});
