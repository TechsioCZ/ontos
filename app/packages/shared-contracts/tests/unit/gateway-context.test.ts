...
void test('decodes the exact versioned public assertion contract', async () => {
  assert.deepEqual(
    await runEffectTestPromise(decodeGatewayContextClaims(claims)),
    claims
  );
  assert.deepEqual(
...
void test('rejects malformed audiences, invalid ordering, and subject mismatch', async () => {
  await assert.rejects(
    runEffectTestPromise(
      Schema.decodeUnknownEffect(GatewayContextRequestSchema)({ audience: '' })
    )
  );
  await assert.rejects(
    runEffectTestPromise(
...
void test('publishes the exact API-key credential boundary and failure statuses', () => {
  assert.deepEqual(Object.keys(ApiKeyGatewayHeadersSchema.fields), [
    'x-api-key',
  ]);
  assert.deepEqual(
    endpointStatuses(
      GatewayContextApiGroup.endpoints.issueApiKeyGatewayContext
    ),
