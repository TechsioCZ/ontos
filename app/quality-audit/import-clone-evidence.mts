...
export const importCloneEvidence = Effect.fn(
  'QualityAudit.importCloneEvidence'
)(function* collectImportCloneEvidence(root: string, source: string) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const report = yield* Schema.decodeUnknownEffect(CloneReport)(source);
  const provesBindings = Effect.fn('QualityAudit.provesImportBindings')(
    function* proveImportBindings(location: typeof CloneLocation.Type) {
      ...
      return (
        ...
        containsOnlyImportBindings(
          lines.slice(location.start - 1, location.end).join('\n')
        )
      );
    }
  );
  ...
  for (const pair of report.duplicates) {
    if (
      (yield* provesBindings(pair.firstFile)) &&
      (yield* provesBindings(pair.secondFile))
    ) {
      evidence.push(pair);
    }
  }
