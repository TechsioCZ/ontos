...
import {
  buildKnipRuntimeEvidence,
  workspaceDirectories,
} from './knip-runtime-model.mts';
...
const ExportLeaf = Schema.Union([
  Schema.String,
  Schema.Record(Schema.String, Schema.String),
]);
const ExportsSchema = Schema.Union([
  Schema.String,
  Schema.Record(Schema.String, ExportLeaf),
]);
...
const PackageSchema = Schema.Struct({
  ...
  'zephyr:dependencies': Schema.optional(
    Schema.Record(Schema.String, Schema.String)
  ),
});

class KnipModelError extends Schema.TaggedError<KnipModelError>()(
  'KnipModelError',
  {
...
const exportedObject = ({
  program,
  variables,
}: SourceFacts): ObjectExpression | undefined => {
...
const parseSource = Effect.fn('QualityAudit.parseKnipModelSource')(
  function* parseModelSource(file: string, source: string) {
...
const sourceFiles = Effect.fn('QualityAudit.knipModelSourceFiles')(
...
const federationFieldEvidence = (
  ...
  for (const item of entries?.properties ?? []) {
    ...
    const target =
      field === 'exposes' ? staticString(item.value, facts.variables) : name;
    if (target !== undefined) {
      result.push(
        evidenceAt(
          facts,
...
const federationEvidence = (
  facts: SourceFacts,
  workspace: string
): KnipModelEvidence[] => {
  if (
    !/(?:module-federation|backend-federation)\.config\.[cm]?[jt]s$/u.test(
      facts.file
    )
  ) {
    return [];
...
const requiredSourceEvidence = (facts: SourceFacts): KnipModelEvidence[] => {
  ...
  const requiredPaths = unwrap(
    facts.variables.get('requiredPaths'),
    facts.variables
  );
...
const configuredBuildDirectories = (
  facts: SourceFacts,
  field: string
): string[] => {
  ...
  const topology = objectExpression(
    objectValue(contract, 'topology'),
    facts.variables
  );
  const compact = objectExpression(
    objectValue(topology, 'compactConfig'),
    facts.variables
  );
  const collection =
    field === 'apps'
      ? objectValue(compact, 'apps')
      : objectValue(contract, field);
  ...
  return array.elements.flatMap((element) => {
    const target = staticString(
      objectValue(
        objectExpression(element ?? undefined, facts.variables),
        'path'
      ),
      facts.variables
    );
...
const buildSourceDirectories = (
  ...
  return collection === undefined
    ? []
    : configuredBuildDirectories(facts, collection);
};
...
const validatedBuildExport = (
  ...
  if (
    ...
    node.callee.name === 'assertBuildFacadeExport'
  ) {
  ...
  const { name } =
    expected?.match(/^export const (?<name>[A-Za-z_$][A-Za-z0-9_$]*)\b/u)
      ?.groups ?? {};
...
const namedBuildEvidence = (facts: SourceFacts): KnipModelEvidence[] => {
  ...
  const recordBuildExport = (
    ...
    for (const directory of buildSourceDirectories(
      contract.source,
      variables,
      facts
    )) {
...
const validatorEvidence = (facts: SourceFacts): KnipModelEvidence[] => {
  ...
  const evidence = [
    ...requiredSourceEvidence(facts),
    ...namedBuildEvidence(facts),
  ];
  const appPaths = configuredBuildDirectories(facts, 'apps');
  const recordValidatorCall = (
    node: Extract<Node, { type: 'CallExpression' }>
  ) => {
...
const sourceEvidence = (
  ...
  const recordUrl = (node: Extract<Node, { type: 'NewExpression' }>) => {
    ...
    const target = staticString(
      node.arguments[0],
      scopedVariables(facts, node.start)
    );
...
const resolveInstalledDependency = (
  dependency: string,
  anchor: string
): string | undefined => {
  try {
...
const staticPath = (
  ...
  if (
    ...
    value.callee.name === 'fileURLToPath'
  ) {
...
const drizzleEvidence = (
  ...
  for (const facts of factsByPath.values()) {
    ...
    const schema = staticString(
      objectValue(exportedObject(facts), 'schema'),
      facts.variables
    );
    const target =
      schema === undefined
...
const nearestPackage = Effect.fn('QualityAudit.nearestPackage')(
  function* readNearestPackage(anchor: string) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
const proveVendorDependency = Effect.fn('QualityAudit.proveVendorDependency')(
  function* proveVendor(
    ...
    for (const dependency of Object.keys(owner.declared.dependencies ?? {})) {
      ...
      const producer =
        resolved === undefined ? null : yield* nearestPackage(resolved);
      if (
        producer !== null &&
...
const workspaceModel = Effect.fn('QualityAudit.knipWorkspaceModel')(
  function* buildWorkspace(
    ...
    const manifest = yield* Schema.decodeUnknownEffect(
      Schema.fromJsonString(PackageSchema)
    ...
    for (const file of ownedFiles) {
      ...
      const directory = path.dirname(
        path.relative(
          path.resolve(appRoot, prefix),
          path.resolve(appRoot, file)
        )
      );
  ...
  for (const fact of runtime) {
    ...
    if (fact.kind === 'dependency' && fact.workspace !== '.') {
      workspaces[fact.workspace] = {
        ...current,
        ignoreDependencies: [
          ...new Set([...(current.ignoreDependencies ?? []), fact.target]),
        ],
      };
...
export const buildKnipModel = Effect.fn('QualityAudit.buildKnipModel')(
  function* buildModel(
    appRoot: string,
    ...
    for (const workspace of directories) {
      ...
      const result = yield* workspaceModel(
        appRoot,
        workspace,
        inherited,
        files,
