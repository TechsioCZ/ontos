...
export default function reportCoverage({ configurationHints, counters, includedWorkspaceDirs }: ReporterOptions): void {
  const source = Result.getOrThrow(
    Schema.encodeResult(Schema.fromJsonString(Schema.Unknown))({
      configurationHints,
      coverage: counters,
      findingCounts: Object.fromEntries(
        Object.entries(counters).filter(([category]) => category !== 'processed' && category !== 'total'),
      ),
