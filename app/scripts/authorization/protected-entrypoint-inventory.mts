...
const EntrypointKeySchema = StableIdentifierSchema.pipe(
...
export type ProtectedEntrypointInventory =
  typeof ProtectedEntrypointInventorySchema.Encoded;

const encodeJsonResult = Schema.encodeResult(
...
const getOrThrowTypeError = <A,>(
  result: Result.Result<A, ProtectedEntrypointInventoryError>
): A => Result.getOrThrowWith(result, toTypeError);
...
const stableValue = (
  ...
  Schema.is(StableIdentifierSchema)(value)
    ...
    : Result.fail(
        invalidInventory(`${field} must be a stable, non-sensitive identifier`)
      );
...
const normalizeAuthorization = (
  ...
  Schema.decodeUnknownResult(InventoryAuthorizationSchema, {
    ...
    Result.mapError(() =>
      invalidInventory(
        'inventory authorization classification is invalid or contains excess data'
      )
    )
  );
...
const normalizeSurface = (
  ...
  ProtectedEntrypointSurface,
  ProtectedEntrypointInventoryError
> =>
  Schema.decodeUnknownResult(ProtectedEntrypointSurfaceSchema)(surface).pipe(
    Result.mapError(() =>
      invalidInventory(`unsupported inventory surface: ${surface}`)
    )
  );
...
> =>
  Result.gen(function* normalizeEntryResult() {
    ...
    const entrypointKey = yield* stableValue(
      entry.entrypointKey,
      'entrypointKey'
    );
    ...
    for (const entry of normalized) {
      if (seen.has(entry.entrypointKey)) {
        return yield* Result.fail(
          invalidInventory(
            `duplicate protected entrypoint: ${entry.entrypointKey}`
          )
        );
      }
  ...
  getOrThrowTypeError(
    Result.gen(function* makeInventoryResult() {
      if (!Schema.is(SourceRevisionSchema)(sourceRevision)) {
        return yield* Result.fail(
          invalidInventory(
            'sourceRevision must be a stable revision identifier'
          )
        );
      }
      const normalized =
        yield* normalizeProtectedEntrypointInventoryResult(entries);
      return {
        entries: normalized,
