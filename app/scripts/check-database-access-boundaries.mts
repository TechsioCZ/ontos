        ...
        return fileSystem.stat(candidate).pipe(
          Effect.flatMap((info) => {
            ...
            return Effect.succeed(
              sourceExtensions.has(path.extname(entry)) ? [candidate] : []
            );
          })
        );
      }),
...
const isGovernedAdapter = (relative: string, source: string): boolean =>
  ...
  ((!relative.startsWith(coreRuntimeSourcePrefix) &&
    ...
    (!relative.startsWith('packages/core-runtime/') &&
      ...
      /@generated by OntOS Codesmith (?:Action v|Governed Contribution)/u.test(
        source
      )));
...
const importedSpecifier = new RegExp(
  `${importPrefix}['"](?<specifier>[^'"]+)['"]`,
  'gu'
);
...
const hiddenCoreSchema =
  /\b(?:coreDatabaseSchema|actionInvocations|CORE_TABLES)\b/u;
...
const resolveLocalSource = (
  ...
  const packageGroups =
    /^@app\/(?<packageName>[^/]+)(?:\/(?<subpath>.+))?$/u.exec(
      specifier
    )?.groups;
...
const crossOwnerPrivateImport = (
  ...
  const sourceOwner = /(?:^|\/)verticals\/(?<owner>[^/]+)\/src\//u.exec(
    relative
  )?.groups?.owner;
  ...
  const targetOwner = /(?:^|\/)verticals\/(?<owner>[^/]+)\//u.exec(
    targetRelative
  )?.groups?.owner;
  return (
  ...
  for (const match of context.source.matchAll(importedSpecifier)) {
    ...
    const dependencyRelative =
      dependency === undefined
        ? undefined
        : context.path
            .relative(context.root, dependency)
            .split(context.path.sep)
            .join('/');
    if (
      specifier.startsWith('@app/core-runtime/testing/') ||
      dependencyRelative?.startsWith('packages/core-runtime/src/testing/') ===
        true
    ) {
      context.record({
...
const recordTransitiveDatabaseCapabilities = (
  ...
  for (const match of context.source.matchAll(importedSpecifier)) {
    ...
    if (
      ...
      context.record({
        ...
        line: sourceLine(context.source, match.index),
        reason:
          'transitive global database capability in governed handler requirements',
      });
    }
  }
};

const isDirectDatabaseImport = (
  context: SourceCheckContext,
  source: string
): boolean =>
  context.governedAdapter ||
  (isVerticalOwnerSource(context.relative) &&
    importsCoreDatabaseSchema.test(source)) ||
  crossOwnerPrivateImport(
    context.path,
    context.root,
    context.file,
    context.relative,
...
const recordLineDatabaseViolations = (context: SourceCheckContext): void => {
  for (const [index, line] of context.source.split('\n').entries()) {
    const directImport =
      ...
      (isVerticalOwnerSource(context.relative) &&
        importsCoreDatabaseSchema.test(line)) ||
      crossOwnerPrivateImport(
        context.path,
        context.root,
        context.file,
    ...
    if (directImport) {
      ...
      context.record({
        ...
        line: index + 1,
        reason:
          'database capability exposed through governed handler requirements',
      });
    }
  }
};
