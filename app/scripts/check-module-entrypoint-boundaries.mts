...
const readObjectIdentifierProperty = (
  ...
  for (let cursor = openBraceIndex; cursor < tokens.length; cursor += 1) {
    ...
    if (current.kind === SyntaxKind.OpenBraceToken) {
      ...
      ((tokenKindAt(tokens, cursor + 1) === SyntaxKind.ColonToken &&
        tokenKindAt(tokens, cursor + 2) === SyntaxKind.Identifier) ||
        tokenKindAt(tokens, cursor + 1) === SyntaxKind.CommaToken ||
        tokenKindAt(tokens, cursor + 1) === SyntaxKind.CloseBraceToken)
    ) {
      return tokens[cursor + 1] === undefined || tokenKindAt(tokens, cursor + 1) !== SyntaxKind.ColonToken
        ? property
...
const validateVerticalApiSource = (state: BoundaryCheckState, file: string, source: string) =>
  Effect.gen(function* validateVerticalApiSourceEffect() {
    ...
    const actionHttpContract =
      /^(?<vertical>verticals\/[a-z][a-z0-9-]*)\/shared\/apis\/(?<slug>[a-z][a-z0-9-]*)-action\.ts$/u.exec(file);
    const actionHttpModuleId =
      actionHttpContract?.groups?.vertical === undefined
        ? undefined
        : providerModuleId(sourceOrEmpty(sourceMap, `${actionHttpContract.groups.vertical}/vertical.manifest.ts`));
    const isGeneratedActionHttpContract =
      actionHttpContract?.groups?.slug !== undefined &&
      actionHttpModuleId !== undefined &&
      source.startsWith(
        `// @generated by OntOS Codesmith Action HTTP v1\n// @ontos-action-http-owner ${actionHttpModuleId}\n// @ontos-action-http-slug ${actionHttpContract.groups.slug}\n`,
      );
    const isGeneratedDomainProblemSchema =
    ...
    if (
      ...
      !isGeneratedActionHttpContract &&
      !isGeneratedDomainProblemSchema
    ) {
