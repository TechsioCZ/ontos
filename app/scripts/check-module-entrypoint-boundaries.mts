...
export const checkModuleEntrypointBoundaries = async (root: string): Promise<void> => {
  ...
  for (const [index, absoluteFile] of files.entries()) {
    ...
    if (file.includes('/verticals/') || file.startsWith('verticals/')) {
      if (file.endsWith('/shared/api.ts') && source.includes('HttpApiEndpoint')) {
        fail(
          file,
    ...
    if (
      file.startsWith('verticals/') &&
      file.endsWith('package.json') &&
      /"exports"[\s\S]*(?:vertical\.registration|\/src\/(?:handlers|workers|routes|search|reports|db))/u.test(
        source,
      )
    ) {
      fail(
