...
const accessesIdentifier = (source: string, identifier: string): boolean => {
  const tokens = tokenize(source);
  return tokens.some(
    (token, index) =>
      token.kind === SyntaxKind.Identifier &&
...
export const checkModuleEntrypointBoundaries = async (root: string): Promise<void> => {
  ...
  for (const [index, absoluteFile] of files.entries()) {
    ...
    if (file.includes('/verticals/') || file.startsWith('verticals/')) {
      if (file.endsWith('/shared/api.ts') && accessesIdentifier(source, 'HttpApiEndpoint')) {
        fail(
          file,
      ...
      if (/\/src\/(?:public-components|search|reports)\//u.test(`/${file}`)) {
        fail(
          file,
