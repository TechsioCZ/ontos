...
const failure = (reason: string): PublicSurfaceGenerationError => new PublicSurfaceGenerationError({ reason });

const exit = await Effect.runPromiseExit(
  runUltramodernScript({
    command: 'public-surface',
    directoryFailure: 'Unable to resolve the public-surface generator directory',
    failure,
