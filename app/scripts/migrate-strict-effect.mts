...
const program = Effect.gen(function* migrateStrictEffect() {
  ...
  const moduleDirectory = yield* path
    ...
    .pipe(
      Effect.mapError(() => failure('Unable to resolve the strict-Effect migration directory')),
    );
  ...
  const ultramodernArgs = ['ultramodern', 'migrate-strict-effect', ...forwardedArgs];
