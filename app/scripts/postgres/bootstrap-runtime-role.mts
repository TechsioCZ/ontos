...
const bootstrapRuntimeRole = (
  ...
  Effect.gen(function* bootstrapRuntimeRoleEffect() {
    ...
    yield* query(
      client,
      `grant connect on database ${quoteIdentifier(database)} to ontos_runtime`
    );
...
const main = Effect.gen(function* mainEffect() {
  ...
  const password = yield* Effect.try({
    ...
    try: () =>
      new Client({ connectionString: connections.runtime.connectionString })
        .password,
  });
