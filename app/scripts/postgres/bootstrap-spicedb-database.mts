...
const bootstrapFailure = (
  ...
  new SpiceDbDatabaseBootstrapError(
...
const query = <Row extends QueryResultRow = QueryResultRow>(
  ...
  Effect.tryPromise({
      ...
      bootstrapFailure('SpiceDB PostgreSQL bootstrap query failed', cause),
  ...
  Effect.gen(function* loadRootConfigurationEffect() {
    const fileProvider = yield* ConfigProvider.fromDotEnv({
      ...
      Effect.mapError((cause) =>
        bootstrapFailure(
          `Unable to load the root environment from ${APP_ENV_PATH}`,
          cause
        )
      )
    );
    ...
    const [adminUrl, spiceDbUrl] = yield* Effect.all(
      ...
      Effect.mapError((cause) =>
        bootstrapFailure(
          'SpiceDB PostgreSQL bootstrap configuration is invalid',
          cause
        )
      )
    );
...
const bootstrapDatabase = (
  ...
  Effect.gen(function* bootstrapDatabaseEffect() {
    ...
    if (database.rows.length === 0) {
      ...
      yield* bootstrapFailure(
        'Existing spicedb database must be owned by the spicedb role'
      );
    }
  });
...
const main = Effect.gen(function* mainEffect() {
  ...
  yield* Console.log(
    'Verified least-privilege PostgreSQL database and role for SpiceDB'
  );
}).pipe(Effect.tapError((failure) => Console.error(failure.reason)));

const exit = await Effect.runPromiseExit(
  Effect.provide(main, NodeFileSystem.layer)
);
