...
const prepareDevModuleContractCommand = Command.make(
  ...
  {
    vertical: Argument.string('vertical').pipe(
      Argument.withSchema(VerticalNameSchema)
    ),
  },
  ({ vertical }) =>
    Effect.gen(function* prepareDevModuleContractProgram() {
      ...
      const workspaceRoot = yield* pathService.fromFileUrl(
      ...
      const publicDirectory = pathService.join(
        workspaceRoot,
        'verticals',
        vertical,
        '.dev-public'
      );
      const contractDirectory = pathService.join(
        publicDirectory,
        '.well-known'
      );
      ...
      const headers = yield* fileSystem.readFileString(
        pathService.join(
          pathService.dirname(pathService.dirname(generated.path)),
          '_headers'
        )
      );
      yield* fileSystem.writeFileString(
        pathService.join(publicDirectory, '_headers'),
        headers
      );
      yield* Console.log(
