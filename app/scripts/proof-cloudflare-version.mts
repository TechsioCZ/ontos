...
const program = Effect.gen(function* proofCloudflareVersionEffect() {
  const path = yield* Path.Path;
  const stdio = yield* Stdio.Stdio;
  const processSpawner = yield* ChildProcessSpawner.ChildProcessSpawner;
  const moduleDirectory = yield* path
    .fromFileUrl(new URL('.', import.meta.url))
    .pipe(Effect.mapError(() => failure('Unable to resolve the Cloudflare proof directory')));
  ...
  const ultramodernArgs = ['ultramodern', 'cloudflare-proof', ...forwardedArgs];
