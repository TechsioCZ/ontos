...
const DEGRADED_BOUNDARY_MARKER =
...
const WranglerSchema = Schema.Struct({
  ...
  services: Schema.optionalKey(
    Schema.Array(
      Schema.Struct({ binding: Schema.String, service: Schema.String })
    )
  ),
...
const containsApiReleaseMarker = Schema.is(
  Schema.Struct({ marker: ApiReleaseMarkerSchema })
);
const isJsonScalar = Schema.is(
  Schema.Union([Schema.Null, Schema.Boolean, Schema.Number, Schema.String])
);
const findReleaseMarkers = (
  value: Schema.Json
): readonly ApiReleaseMarker[] => {
...
const TargetUrlsSchema = Schema.fromJsonString(
...
type ProofEffect<Value> = Effect.Effect<
  Value,
  ProofFailure,
  FileSystem.FileSystem
>;
type ScopedProofEffect<Value> = Effect.Effect<
  Value,
  ProofFailure,
...
const readJsonDocument = <
  DocumentSchema extends Schema.ConstraintDecoder<unknown>,
>(
  absolutePath: string,
  ...
  Effect.gen(function* readJsonDocumentEffect() {
    ...
    return yield* Schema.decodeUnknownEffect(Schema.fromJsonString(schema))(
      ...
      Effect.mapError((cause) =>
        proofError(`Invalid JSON document ${absolutePath}`, cause)
      )
    );
  });
    ...
    const nested = yield* Effect.forEach(
      ...
      (name) =>
        Effect.gen(function* collectEntryEffect() {
          ...
          return info.type === 'File' && /\.(?:c|m)?js$/u.test(name)
            ? [absolutePath]
            : [];
        }),
      { concurrency: 1 }
...
const readExecutionEnvelope = (
  ...
  Effect.gen(function* readExecutionEnvelopeEffect() {
    ...
    const envelope = yield* readJsonDocument(
      envelopePath,
...
const bindExecutedModule = (
  ...
  Effect.gen(function* bindExecutedModuleEffect() {
    ...
    const logicalPath = normalizePath(
      path.relative(app.outputRoot, module.path)
    );
    yield* ensure(
...
const resolveProofRoutes = (
  ...
  const proofRoutes = [
    ...new Set(configuredRoutes.filter((route) => route.startsWith('/'))),
  ];
  if (proofRoutes.length > 0) {
    return proofRoutes;
  }
  return [
    configuredSsrRoute?.startsWith('/') === true ? configuredSsrRoute : '/',
  ];
};
    ...
    return yield* Effect.forEach(
      ...
      (rawApp) =>
        Effect.gen(function* loadAppEffect() {
          const kind: App['kind'] =
            rawApp.kind === 'vertical' ? 'vertical' : 'shell';
          ...
          const wrangler = yield* readJsonDocument(
            wranglerPath,
            WranglerSchema
          );
          const executedEnvelope =
            kind === 'vertical'
              ? yield* readExecutionEnvelope(
                  rawApp.id,
                  outputRoot,
...
const workerName = (app: App): Effect.Effect<string, WorkerdProofError> =>
  app.wrangler.name.length > 0
    ...
    : Effect.fail(
        proofError(`${app.id} wrangler output must define a worker name`)
      );
...
const createWorkerConfiguration = (
  ...
  Effect.gen(function* createWorkerConfigurationEffect() {
    ...
    const validateSelectedSurfaces = Effect.gen(
      function* validateSelectedSurfacesEffect() {
        const apiBackend = app.envelope?.surfaces.apiBackend ?? [];
        const ssr = app.envelope?.surfaces.ssr ?? [];
        const selectedPaths = new Set(
          boundModules.map((module) => module.logicalPath)
        );
        yield* ensure(
          app.kind !== 'vertical' ||
            (apiBackend.length > 0 &&
    ...
    const options: ProofWorkerOptions = {
      ...
      bindings: {
        ...app.wrangler.vars,
        DATABASE_URL:
          'postgresql://workerd-proof:workerd-proof@127.0.0.1:5432/workerd-proof',
    ...
    return {
      executionEvidence: {
        ...
        modulesRoot: normalizePath(
          path.relative(workspaceRoot, app.outputRoot)
        ),
        worker: name,
      },
      options,
...
const responseEvidence = (
  ...
  Effect.gen(function* responseEvidenceEffect() {
    const arrayBuffer = yield* Effect.tryPromise({
        ...
        proofError(`${app.id} API response body could not be read`, cause),
    ...
    const body = yield* Schema.decodeUnknownEffect(
      ...
      Effect.mapError((cause) =>
        proofError(`${app.id} API response is not valid JSON`, cause)
      )
    );
...
const runApiCheck = (
  ...
  Effect.gen(function* runApiCheckEffect() {
    ...
    const body =
      check.body === undefined
        ...
        : yield* Schema.encodeEffect(JsonTextSchema)(check.body).pipe(
            Effect.mapError((cause) =>
              proofError(`${app.id} smoke body is invalid`, cause)
            )
          );
    if (body !== undefined) {
    ...
    const target = yield* Effect.tryPromise({
        ...
        proofError(`${app.id} worker could not be resolved`, cause),
      try: async () => await miniflare.getWorker(appWorkerName),
    });
    const directResponse = yield* Effect.tryPromise({
      catch: (cause) =>
        proofError(`${app.id} direct API request failed`, cause),
      try: async () =>
        await target.fetch(
          `https://${appWorkerName}.invalid${check.route}`,
          init
        ),
    });
    ...
    const shellResponse = yield* Effect.tryPromise({
      ...
      try: async () =>
        await miniflare.dispatchFetch(
          `https://${shellWorkerName}.invalid${check.route}`,
          init
        ),
    });
...
const runAppApiProofs = (
  ...
  Effect.gen(function* runAppApiProofsEffect() {
    ...
    if (binding === undefined || targetEvidence === undefined) {
      return yield* Effect.fail(
        proofError(`${app.id} service-binding evidence is missing`)
      );
    }
    return yield* Effect.forEach(
      checks,
...
const readRequestBody = (
  ...
  Effect.callback((resume) => {
    ...
    const onError = (cause: Error) => {
      resume(
        Effect.fail(proofError('Could not read incoming proof request', cause))
      );
    };
...
const listen = (
  ...
  Effect.callback((resume) => {
    const onError = (cause: Error) => {
      resume(
        Effect.fail(proofError(`Could not listen on proof port ${port}`, cause))
      );
    };
  ...
  Effect.gen(function* handleTargetRequestEffect() {
    ...
    if (
      ...
      const command = yield* Schema.decodeUnknownEffect(
        ...
        Effect.mapError((cause) =>
          proofError('Invalid service-binding fault command', cause)
        )
      );
      const targetApp = yield* Effect.fromOption(
        Option.fromNullishOr(
          apps.find((candidate) => candidate.id === command.appId)
        ),
        () =>
      ...
      const encoded = yield* Schema.encodeEffect(
        ServiceBindingFaultResponseSchema
      )({
        failed: failedServices.has(service),
        service,
      }).pipe(
        Effect.mapError((cause) =>
          proofError('Could not encode fault response', cause)
        )
      );
      yield* Effect.sync(() => {
      ...
      const response = yield* Effect.tryPromise({
        ...
        try: async () =>
          await runtime.dispatchFetch(
            `https://${name}.invalid${incoming.url ?? '/'}`,
            init
          ),
      });
...
const startTargetServer = (
  ...
  Effect.gen(function* startTargetServerEffect() {
    ...
    if (configuration === undefined) {
      return yield* Effect.fail(
        proofError(`${app.id} Worker configuration is missing`)
      );
    }
    const runtime =
      app.kind === 'vertical'
...
const disposeTargetRuntime = (
  runtime: Option.Option<Miniflare>
): Effect.Effect<void> => {
...
const startWorkerdTargetServers = (
  ...
  Effect.gen(function* startWorkerdTargetServersEffect() {
    ...
    const runtimeDisposals = started.map(({ runtime }) =>
      disposeTargetRuntime(runtime)
    );
    const stop = Effect.all(
      [
        ...started.map(({ server }) => closeServer(server)),
        ...runtimeDisposals,
      ],
    ...
    (match) =>
      Effect.gen(function* collectBoundaryEffect() {
        ...
        if (key === undefined || separator <= 0) {
          return yield* Effect.fail(
            proofError(`Invalid distributed SSR boundary key ${key}`)
          );
        }
        return {
...
const readRequiredFragmentHeader = (
  ...
  return value === null || value.length === 0
    ? Effect.fail(
        proofError(`Distributed SSR fragment request is missing ${header}`)
      )
...
> =>
  Effect.gen(function* decodeDistributedSsrFragmentRequestEffect() {
    ...
    const headers = new Map(
      DISTRIBUTED_SSR_REQUIRED_HEADERS.map((header, index) => [
        header,
        values[index],
      ])
    );
    ...
    if (
      ...
      return yield* Effect.fail(
        proofError('Distributed SSR fragment headers are incomplete')
      );
    }
    const props = yield* Schema.decodeUnknownEffect(
      Schema.fromJsonString(FragmentPropsSchema)
    )(decodeURIComponent(propsSource)).pipe(
      Effect.mapError((cause) =>
        proofError('Fragment props must be an object', cause)
      )
    );
    yield* ensure(
      URL.canParse(sourceUrl),
      'Distributed SSR fragment source URL must be absolute'
    );
...
const handleServiceBinding = (
  ...
  Effect.gen(function* serviceBindingEffect() {
    ...
    const target = yield* Effect.tryPromise({
        ...
        proofError(`Could not resolve service ${service.service}`, cause),
      try: async () => await miniflare.getWorker(service.service),
    });
    const response = yield* Effect.tryPromise({
      catch: (cause) =>
        proofError(`Service ${service.service} request failed`, cause),
...
const waitForTerminationSignal: Effect.Effect<void> = Effect.callback(
  (resume) => {
    const done = () => {
      resume(Effect.void);
    };
  ...
  Effect.gen(function* writeReportEffect() {
    ...
    const routes = proofs.flatMap((proof) => {
      const decoded = Schema.decodeUnknownOption(
        Schema.Struct({ route: Schema.String })
      )(proof);
...
const routeHtml = (
  ...
  Effect.gen(function* routeHtmlEffect() {
    const html = yield* Effect.tryPromise({
        ...
        proofError(`${appId} route ${route} body failed`, cause),
...
const proveShellRoute = (
  ...
  Effect.gen(function* proveShellRouteEffect() {
    ...
    const response = yield* Effect.tryPromise({
      ...
      try: async () =>
        await miniflare.dispatchFetch(
          `https://${shellWorkerName}.invalid${route}`,
          {
    ...
    const routeFragmentBindingRequests =
      state.fragmentBindingRequests.slice(fragmentStart);
  ...
  Effect.gen(function* proveRemoteEffect() {
    if (!state.renderedRemoteIds.has(remote.id)) {
      ...
      const target = yield* Effect.tryPromise({
          ...
          proofError(`${remote.id} Worker could not be resolved`, cause),
        try: async () => await miniflare.getWorker(remoteWorkerName),
      });
      const response = yield* Effect.tryPromise({
        catch: (cause) =>
          proofError(`${remote.id} route ${route} failed`, cause),
        try: async () =>
      ...
      yield* ensure(
        response.headers.get(CONTENT_TYPE_HEADER)?.includes('text/html') ===
          true,
...
const runShellProof = (
  ...
  Effect.gen(function* runShellProofEffect() {
    ...
    const executions = workerConfigurations.map(
    ...
    yield* Effect.forEach(
      shell.proofRoutes,
      (route) =>
        proveShellRoute(apps, miniflare, route, shell, shellWorkerName, state),
      { concurrency: 1 }
    );
    yield* Effect.forEach(
      remotes,
      (remote) => proveRemote(miniflare, remote, state),
      {
        concurrency: 1,
      }
    );
    const apiProofs = yield* runApiProofs(
      apps,
      miniflare,
      shell,
      executionByAppId
    ...
    if (keepWorkerd) {
      ...
      const encodedTargetUrls = yield* Schema.encodeEffect(TargetUrlsSchema)(
        ...
        Effect.mapError((cause) =>
          proofError('Could not encode Workerd target URLs', cause)
        )
      );
      yield* Effect.log(`WORKERD_TARGET_URLS=${encodedTargetUrls}`);
      yield* Effect.log(
        `WORKERD_URL=${targetServers.targetUrls[shell.id] ?? ''}`
      );
...
const loggedMain = main.pipe(
  Effect.tapCause((cause) => Effect.logError(cause))
);
const exit = await Effect.runPromiseExit(
  Effect.provide(loggedMain, NodeFileSystem.layer)
);
