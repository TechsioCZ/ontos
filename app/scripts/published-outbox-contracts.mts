...
const outboxExportPattern =
  /^\.\/outbox\/(?<slug>[a-z][a-z0-9]*(?:-[a-z0-9]+)*)$/u;
const resourceExportPattern =
  /^\.\/resources\/(?<slug>[a-z][a-z0-9]*(?:-[a-z0-9]+)*)$/u;
...
export const resolvePublishedContractModuleId = (input: {
  ...
  const moduleIds = [
    ...input.manifestSource.matchAll(
      /^\/\/ @ontos-module-id (?<moduleId>[^\s]+)$/gmu
    ),
  ].map((match) => match.groups?.moduleId);
  const appIds = [
    ...input.manifestSource.matchAll(
      /^\/\/ @ontos-deployment-app-id (?<appId>[^\s]+)$/gmu
    ),
  ].map((match) => match.groups?.appId);
  const [moduleId] = moduleIds;
  assertCondition(
    input.manifestSource.startsWith(
  ...
  sortLexically(
    Object.entries(packageJson.exports ?? {})
      .filter(([exportKey, target]) => {
        ...
        return (
          match !== null &&
          target === `./shared/outbox/${match.groups?.slug ?? ''}.ts`
        );
      })
  ...
  sortLexically(
    Object.entries(packageJson.exports ?? {})
      .filter(([exportKey, target]) => {
        ...
        return (
          match !== null &&
          target === `./shared/resources/${match.groups?.slug ?? ''}.ts`
        );
      })
      .map(([exportKey]) => exportKey)
  );
...
const hasResourceRefDeclarations = (input: {
  ...
  return (
    /export const [A-Z][A-Za-z0-9]*RefSchema = Schema\.Struct\(/u.test(
      input.source
    ) &&
    ...
    /export const [a-z][A-Za-z0-9]*ResourceDescriptor = \{/u.test(
      input.source
    ) &&
...
const isGeneratedSchemaOnlyResourceRef = (input: {
  ...
  return (
    ...
    /^import type \{ OntosResourceType \} from '@app\/core-runtime';$/mu.test(
      input.source
    ) &&
    /^import \{ Schema \} from 'effect';$/mu.test(input.source) &&
    imports.length === 2 &&
    imports.every(
      (specifier) => specifier === '@app/core-runtime' || specifier === 'effect'
    ) &&
...
const isAllowedEffectClientAggregateImport = (specifier: string): boolean =>
  ...
  /^\.\/(?:action-gateway|[a-z][a-z0-9]*(?:-[a-z0-9]+)*-client)\.ts$/u.test(
    specifier
  );
...
const isGeneratedEffectClientLeaf = (source: string, appId: string): boolean =>
  ...
  source.startsWith(
    `${COMMAND_CLIENT_HEADER}// @ontos-command-client-owner ${appId}\n`
  );
...
const isGeneratedPublicEffectClient = (input: {
  ...
  const localClients = [
    ...new Set(
      imports.filter((specifier) => /^\.\/.+-client\.ts$/u.test(specifier))
    ),
  ];
  ...
  for (const localClient of localClients) {
    const localSource = input.readOwnerSource(
      `./src/api/${localClient.slice(2)}`
    );
...
export const assertPublishedCrossMicroVerticalContractUsage = (
  ...
  assertCondition(
    input.projectReferenceDeclared,
    `consumer must project-reference ${input.dependencyPackageName}`
  );

  const outboxExports = publishedOutboxContractExports(
    input.dependencyPackageJson
  );
  const resourceExports = publishedResourceRefContractExports(
    input.dependencyPackageJson
  );
  const effectClientExports = publishedEffectClientContractExports(
    input.dependencyPackageJson
  );
