...
const ACTION_HTTP_CLIENT_HEADER = '// @generated by OntOS Codesmith Action HTTP v1\n';
const ACTION_GATEWAY_HEADER = '// @generated by OntOS Codesmith MicroVertical Action Boundary v1\n';

const sortLexically = (values: readonly string[]): readonly string[] => {
  const sorted: string[] = [];
...
export const resolvePublishedContractModuleId = (input: {
  ...
  const moduleIds = [...input.manifestSource.matchAll(/^\/\/ @ontos-module-id (?<moduleId>[^\s]+)$/gmu)].map(
    (match) => match.groups?.moduleId,
  );
  const appIds = [...input.manifestSource.matchAll(/^\/\/ @ontos-deployment-app-id (?<appId>[^\s]+)$/gmu)].map(
    (match) => match.groups?.appId,
  );
  ...
  assertCondition(
    input.manifestSource.startsWith('// @generated by OntOS Codesmith Module Contract v1\n') &&
      ...
      packageMatchesManifest(input.dependencyPackageJson, input.dependencyPackageName, input.expectedAppId, moduleId),
...
export const assertPublishedOutboxContractSource = (input: {
  ...
  assertCondition(
    input.source.startsWith('// @generated by OntOS Codesmith Outbox Message Contract v1\n') &&
      ...
      input.source.includes(`export const outboxProducerModuleKey = '${input.moduleId}' as const;`) &&
      !/(?:src\/actions|create[A-Za-z0-9]+Message|handler|repository|transport)/u.test(input.source),
    `${input.specifier} must remain a generated schema-only Outbox contract`,
  );
};

export const publishedOutboxContractExports = (packageJson: PublishedOutboxPackage): readonly string[] =>
  sortLexically(
...
export const publishedResourceRefContractExports = (packageJson: PublishedOutboxPackage): readonly string[] =>
  sortLexically(
    Object.entries(packageJson.exports ?? {})
      .filter(([exportKey, target]) => {
...
const publishedEffectClientContractExports = (packageJson: PublishedOutboxPackage): readonly string[] => {
...
const hasResourceRefDeclarations = (input: {
  ...
  return (
    ...
    /export type [A-Z][A-Za-z0-9]*Ref = typeof [A-Z][A-Za-z0-9]*RefSchema\.Type;/u.test(input.source) &&
...
const isGeneratedSchemaOnlyResourceRef = (input: {
  ...
  return (
    ...
    !/\b(?:import\s*\(|require\s*\(|async\b|function\b|class\b|fetch\s*\(|new\s+|process\.)/u.test(input.source) &&
...
const isGeneratedEffectClientLeaf = (input: {
  readonly appId: string;
  ...
  return (
    slug !== undefined &&
    input.source.startsWith(
      `${ACTION_HTTP_CLIENT_HEADER}// @ontos-action-http-owner ${input.moduleId}\n// @ontos-action-http-slug ${slug}\n`,
    )
  );
};
...
const hasValidActionGateway = (
  ...
  input: {
    readonly appId: string;
    readonly readOwnerSource: (path: string) => string;
  },
): boolean => {
...
const isGeneratedPublicEffectClient = (input: {
  ...
  const localClients = [...new Set(imports.filter((specifier) => /^\.\/.+-client\.ts$/u.test(specifier)))];
...
export const assertPublishedCrossMicroVerticalContractUsage = (input: PublishedContractUsageInput): void => {
  const dependencySpecifiers = input.moduleSpecifiers.filter(
    (specifier) => specifier === input.dependencyPackageName || specifier.startsWith(`${input.dependencyPackageName}/`),
  );
  assertCondition(
    dependencySpecifiers.length > 0,
    `${input.dependencyPackageName} is an unused cross-MicroVertical dependency`,
  );
  assertCondition(
    input.dependencyDeclared,
    `consumer must declare ${input.dependencyPackageName} as a workspace dependency`,
  );
  assertCondition(input.projectReferenceDeclared, `consumer must project-reference ${input.dependencyPackageName}`);
  ...
  const forbiddenSpecifier = dependencySpecifiers.find((specifier) => !allowedSpecifiers.has(specifier));
  assertCondition(
    forbiddenSpecifier === undefined,
...
export const assertPublishedOutboxDependencyUsage = (input: {
  ...
  assertCondition(
    forbiddenSpecifier === undefined,
    `${forbiddenSpecifier} is not a published schema-only Outbox contract subpath`,
  );
};
