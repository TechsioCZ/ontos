...
const CountSchema = Schema.Finite.check(
  Schema.isInt(),
...
class QualityAuditError extends Schema.TaggedError<QualityAuditError>()(
  'QualityAuditError',
  {
...
const failure = (reason: string): QualityAuditError =>
  new QualityAuditError({ reason });
const jsonCodec = Schema.fromJsonString(Schema.Unknown, { space: 2 });

const writeJson = Effect.fn('qualityAudit.writeJson')(function* writeJsonEffect<
  A,
>(file: string, value: A) {
...
const decodeReport = <S extends Schema.Top>(
  ...
  Schema.decodeUnknownEffect(Schema.fromJsonString(schema))(source).pipe(
    Effect.mapError((issue) =>
      failure(`Malformed ${subject}: ${String(issue)}`)
    )
  );
...
const nonempty = (count: number, label: string) =>
  count > 0
    ? Effect.void
    : Effect.fail(failure(`${label}: analysis contains no files`));
...
const validateKnip = Effect.fn('qualityAudit.validateKnip')(
  function* validateKnipEffect(name: string, source: string) {
    ...
    return {
      ...
      findings: Object.values(coverage.findingCounts).reduce(
        (sum, count) => sum + count,
        0
      ),
    };
...
const modeledUsage = (
  ...
  evidence.find((entry) => {
    ...
    return (
      entry.kind === 'compiler-option' &&
      issue.line === undefined &&
      issue.col === undefined
    );
  });

const calibrateKnip = Effect.fn('qualityAudit.calibrateKnip')(
  function* calibrateKnipEffect(source: string, directory: string) {
    const fs = yield* FileSystem.FileSystem;
    ...
    const evidence = yield* decodeReport(
      ...
      yield* fs.readFileString(
        path.join(path.dirname(directory), 'knip-model.json')
      )
    );
...
const evaluateKnip = Effect.fn('qualityAudit.evaluateKnip')(
  function* evaluateKnipEffect(report: string, directory: string) {
...
const validateJscpd = Effect.fn('qualityAudit.validateJscpd')(
  function* validateJscpdEffect(name: string, source: string) {
...
const validateClones = Effect.fn('qualityAudit.validateClones')(
  function* validateClonesEffect(name: string, source: string) {
    ...
    if ((report.workspace_diagnostics?.length ?? 0) > 0) {
      return yield* failure(
        'Fallow reports incomplete workspace discovery; inspect raw report'
      );
    }
...
const validateHealth = Effect.fn('qualityAudit.validateHealth')(
  function* validateHealthEffect(name: string, source: string) {
    ...
    const complexity = report.findings.map((finding) => {
      ...
      const hookDensityWeight = finding.contributions
        .filter(
          (entry) =>
            entry.metric === 'cognitive' && entry.kind === 'hook-density'
        )
        .reduce((sum, entry) => sum + entry.weight, 0);
      const propCountWeight = finding.contributions
        .filter(
          (entry) => entry.metric === 'cognitive' && entry.kind === 'prop-count'
        )
        .reduce((sum, entry) => sum + entry.weight, 0);
      const controlFlowCognitive =
        cognitive - hookDensityWeight - propCountWeight;
      return {
    ...
    if (complexity.some((finding) => !finding.reconstructed)) {
      return yield* failure(
        'Fallow complexity contributions disagree with function metrics'
      );
    }
    if (
      ...
      return yield* failure(
        'Fallow reported a function below both configured thresholds'
      );
    }
...
const collectSourceFiles = Effect.fn('qualityAudit.collectSourceFiles')(
  function* collectSourceFilesEffect(root: string, output: string) {
    ...
    const scope = yield* decodeReport(
      ScopeSchema,
      source,
      ...
      outputPaths.some((directory) =>
        scope.patterns.some((pattern) =>
          nodePath.matchesGlob(
            path.join(directory, 'knip-consumers.mts'),
            pattern
          )
        )
      )
  ...
  function* readProvenance(root: string) {
    ...
    return {
      ...
      sourceState: Result.match(status, {
        ...
        onSuccess: (output) =>
          output.trim().length > 0 ? 'modified' : 'clean',
      }),
...
const snapshotConfiguration = Effect.fn('qualityAudit.snapshotConfiguration')(
  function* snapshotConfigurationEffect(
    ...
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const selected = ['knip', 'jscpd', 'fallow'].filter(
      (name) => tool === 'all' || tool === name
    );
    const configs = [
      'scope.json',
    ...
    yield* Effect.forEach(
      ...
      (name) =>
        fs.copyFile(
          path.join(root, 'quality-audit', name),
          path.join(directory, 'configs', name)
        ),
      { concurrency: 'unbounded' }
    );
    if (selected.includes('knip')) {
      ...
      yield* fs.copyFile(
        target,
        path.join(directory, 'configs/knip-base.json')
      );
    ...
    if (selected.includes('fallow')) {
      ...
      const source = yield* fs.readFileString(target);
      const config = yield* decodeReport(
...
const verifyFallowCounts = (results: readonly AuditResult[]) => {
  ...
  return discovery?.status === 'reported' &&
    ...
    ? Effect.fail(
        failure('Fallow discovery and complexity file counts disagree')
      )
    : Effect.void;
};

const reconcileFallowCoverage = Effect.fn(
  'qualityAudit.reconcileFallowCoverage'
...
const reconcileCoverage = Effect.fn('qualityAudit.reconcileCoverage')(
  function* reconcileCoverageEffect(
    ...
    const expectedManifests = yield* fs.glob(
      '{apps,verticals,packages}/*/package.json',
      {
    ...
    const expectedWorkspaces = [
      '.',
      ...expectedManifests.map((file) => path.dirname(file)),
    ];
    const knip = results.find(
      (result) => result.name === 'knip' && result.status === 'reported'
    );
    if (knip !== undefined) {
    ...
    return yield* reconcileFallowCoverage(
      directory,
      files,
      results,
      expectedWorkspaces
...
const executeStep = Effect.fn('qualityAudit.executeStep')(
  function* executeStepEffect(
    ...
    yield* Effect.all(
      [fs.writeFileString(stdoutPath, ''), fs.writeFileString(stderrPath, '')],
      {
        concurrency: 'unbounded',
      }
    );
    ...
    const execution = yield* Effect.gen(function* launchAnalyzer() {
      ...
      const installed = yield* decodeReport(
        ...
        yield* fs.readFileString(
          path.join(root, 'node_modules', step.tool, 'package.json')
        )
      );
    ...
    const executionError = Result.isFailure(execution)
      ? String(execution.failure)
      : '';
    const exitCode = Result.isSuccess(execution)
      ? execution.success.exitCode
      : -1;
    yield* writeJson(path.join(directory, 'metadata.json'), {
      ...
      startedAt,
      verifiedVersion: Result.isSuccess(execution)
        ? execution.success.verifiedVersion
        : '',
    });
    const evaluated = yield* Effect.gen(function* evaluateAnalyzer() {
      ...
      if (exitCode !== 0) {
        return yield* failure(
          `Analyzer exited ${exitCode}; inspect stdout.txt and stderr.txt`
        );
      }
      ...
      if (
        ...
        !(
          step.name === 'jscpd' &&
          stderr.trim() === `Using config from ${step.args[1]}`
        )
      ) {
        return yield* failure(
      ...
      yield* fs.writeFileString(
        path.join(
          directory,
          step.name === 'knip' ? 'report.ndjson' : 'report.json'
        ),
        report
      );
    ...
    if (Result.isFailure(evaluated)) {
      ...
      yield* fs.writeFileString(
        path.join(directory, 'validation-error.txt'),
        `${diagnostic}\n`
      );
...
export const auditSteps = (
  ...
  const steps: readonly AuditStep[] = [
    ...
    },
    {
      args: [
        '--config',
        `${runDirectory}/jscpd.config.json`,
        '--output',
        `${runDirectory}/jscpd`,
      ],
      name: 'jscpd',
...
const writeSummary = Effect.fn('qualityAudit.writeSummary')(
...
export const runQualityAudit = Effect.fn('qualityAudit.runQualityAudit')(
  function* runQualityAuditEffect(
    root: string,
    output: string,
    tool: AuditTool
  ) {
    ...
    const results = yield* Effect.gen(function* collectAudit() {
      ...
      const consumerPath =
        tool === 'all' || tool === 'knip'
          ? yield* createConsumerPath(root)
          : undefined;
      const configs = yield* snapshotConfiguration(
        root,
        runDirectory,
        tool,
        consumerPath
      ...
      if (tool === 'all' || tool === 'jscpd') {
        const jscpdConfig = yield* decodeReport(
          Schema.Record(Schema.String, Schema.Json),
          yield* fs.readFileString(
            path.join(runDirectory, 'configs/jscpd.json')
          ),
          'configs/jscpd.json'
        );
      ...
      return yield* reconcileCoverage(
        root,
        runDirectory,
        files,
        stepResults
      ...
      Effect.match({
        onFailure: (issue) => [
          errorResult('setup', runDirectory, String(issue)),
        ],
...
const cli = Command.make(
  'quality-audit',
  {
    output: Flag.string('output').pipe(
      Flag.withDefault('.codex/reports/quality-audit')
    ),
    tool: Flag.choice('tool', ['all', 'knip', 'jscpd', 'fallow']).pipe(
