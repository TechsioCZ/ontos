...
const planCoreActionScaffold = Effect.fn('ActionScaffold.planCore')(
  function* planCoreActionScaffold(
    ...
    const indexContent = yield* fileSystem
      ...
      .pipe(
        Effect.mapError((cause) =>
          scaffoldFailure(`failed to read ${indexPath}`, cause)
        )
      );
    const nextIndex = yield* tryScaffold(
      'failed to patch the Core Action export slot',
      () =>
        insertSortedSlot(
          indexContent,
          CORE_ACTION_SLOT_START,
    ...
    const catalogContent = yield* fileSystem
      ...
      .pipe(
        Effect.mapError((cause) =>
          scaffoldFailure(`failed to read ${catalogPath}`, cause)
        )
      );
    const nextCatalog = yield* tryScaffold(
      'failed to patch the Core Action catalog',
      () =>
        insertSortedSlot(
          insertSortedSlot(
            catalogContent,
...
const planActionScaffold = Effect.fn('ActionScaffold.plan')(
  function* planActionScaffold(
    workspaceRoot: string,
    ...
    const dependencyMutation = yield* tryScaffold(
      'failed to patch the Core dependency',
