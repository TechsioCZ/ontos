...
const isMatchingGeneratedAction = (
  ...
  const hasGeneratedActionPrefix =
    ...
    actionContent.startsWith(
      `${FORMATTED_ACTION_GENERATOR_PREFIX}${ACTION_GENERATOR_HEADER}\n`
    );
  return (
...
> =>
  Effect.gen(function* planOutboxScaffoldEffect() {
    ...
    const action = yield* fromLegacySync(() =>
    ...
    const vertical = yield* discoverOntosModuleEffect(
      workspaceRoot,
      config.vertical
    ).pipe(Effect.catchCause(recoverDiscoveryCause));
    ...
    const actionContent = yield* fileSystem
      ...
      .pipe(
        Effect.catchIf(isNotFoundPlatformError, (cause) =>
          Effect.fail(
            planningFailure(
              `Outbox Message requires the generated Action at ${actionPath}`,
              cause
            )
          )
        )
    ...
    if (
      ...
      return yield* Effect.fail(
        planningFailure(`Outbox identifier ${base} already exists`)
      );
    }
    ...
    const actionMutation = updateMutation(
      actionPath,
      actionContent,
      patchedAction
    );
    if (actionMutation === undefined) {
      return yield* Effect.fail(
        planningFailure(
          'Outbox Message Action export patch unexpectedly made no change'
        )
      );
    }
    const packageDocument = yield* Schema.decodeUnknownEffect(
      ...
      Effect.mapError((cause) =>
        planningFailure(
          `vertical ${vertical.slug} package exports must be a JSON object`,
          cause
        )
      )
    );
    ...
    if (exportsValue[contractExport] !== undefined) {
      return yield* Effect.fail(
        planningFailure(
          `Outbox contract export ${contractExport} already exists`
        )
      );
    }
    ...
    if (packageMutation === undefined) {
      return yield* Effect.fail(
        planningFailure(
          'Outbox Message package export patch unexpectedly made no change'
        )
      );
    }
    const mutations = [
