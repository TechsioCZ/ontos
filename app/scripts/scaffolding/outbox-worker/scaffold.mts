    ...
    return yield* fileSystem.readFileString(filePath).pipe(
      Effect.mapError((error) =>
        Match.value(error.reason).pipe(
          Match.tag('NotFound', () =>
            scaffoldError(error, `${label} is missing at ${filePath}`)
          ),
...
const OUTBOX_WORKER_HOST_HEADER =
  '// @generated by scaffold:outbox-worker worker-host';
const TsconfigReferenceSchema = Schema.StructWithRest(
...
const patchConsumerPackage = (
  ...
  Effect.gen(function* patchConsumerPackageEffect() {
      ...
      dependenciesValue === undefined
        ...
        : {
            ...(yield* trySync(() =>
              asJsonObject(
                dependenciesValue,
                `vertical ${consumer.slug} dependencies`
              )
            )),
          };
    ...
    const sortedScripts = Object.fromEntries(
      Object.entries(scripts).toSorted(([left], [right]) =>
        left.localeCompare(right)
      )
    );
  ...
  Effect.gen(function* patchConsumerTsconfigEffect() {
    const parsed = yield* Schema.decodeUnknownEffect(
      ...
      Effect.mapError((cause) =>
        scaffoldError(
          cause,
          `vertical ${consumer.slug} tsconfig is not valid JSON`
        )
      )
    );
    const root = yield* trySync(() =>
      asJsonObject(parsed, `vertical ${consumer.slug} tsconfig`)
    );
    const referencesValue = root['references'];
    const references = yield* Schema.decodeUnknownEffect(
      TsconfigReferencesSchema
    )(referencesValue).pipe(
      Effect.mapError((cause) =>
        scaffoldError(
          cause,
          `vertical ${consumer.slug} tsconfig references must be an array`
        )
      )
    );
    const producerReference = `../${producer.slug}`;
    const matching = references.filter(
      (reference) => reference.path === producerReference
    );
    ...
    const patched = [...references, { path: producerReference }].toSorted(
      (left, right) => left.path.localeCompare(right.path)
    );
    return yield* trySync(() =>
      patchJsonObjectProperty(content, [], 'references', patched)
    );
  });
...
const planRegistryMutation = (
  ...
  Effect.gen(function* planRegistryMutationEffect() {
    ...
    if (Option.isSome(registryContent)) {
      ...
      if (
        new RegExp(`\\b${workerVariable}\\b`, 'u').test(registryContent.value)
      ) {
        return yield* new OutboxWorkerScaffoldError({
      ...
      const withRegistration = yield* trySync(() => {
        const withImport = insertSortedSlot(
          ...
          (candidate) =>
            /^import \{ [A-Za-z0-9]+Worker \} from '\.\/[a-z0-9-]+\.worker\.ts';$/u.test(
              candidate
            )
        );
      ...
      registryMutation = yield* createMutationEffect(
        registryPath,
        withRegistration
      );
    }
...
> =>
  Effect.gen(function* planOutboxWorkerScaffoldProgram() {
    const worker = yield* trySync(() =>
      requireCanonicalSlug(config.worker, 'worker')
    );
    ...
    const contract = yield* readRequiredFile(
      contractPath,
      'published producer Outbox contract'
    );
    ...
    const workerVariable = `${toCamelCase(worker)}Worker`;
    const registryMutation = yield* planRegistryMutation(
      registryPath,
      registryContent,
      worker
    );
    ...
    const [
      workerHostLayerMutation,
      workerHostMainMutation,
      workerHostScriptMutation,
    ] = yield* Effect.all([
      planWorkerHostFile(
        workerHostLayerPath,
        consumer,
        renderWorkerHostLayer(consumer)
      ),
      planWorkerHostFile(
        workerHostMainPath,
        consumer,
        renderWorkerHostMain(consumer)
      ),
      planWorkerHostFile(
        workerHostScriptPath,
        consumer,
        renderWorkerHostScript(consumer)
      ),
    ]);
    ...
    const patchedTsconfig = yield* patchConsumerTsconfig(
      tsconfigContent,
      consumer,
      producer
    );
    ...
    const mutations = [
      workerMutation,
      registryMutation,
      ...(ownerRegistrationMutation === undefined
        ? []
        : [ownerRegistrationMutation]),
