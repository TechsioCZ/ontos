...
import type { Mutation, PolicyScaffoldConfig, PolicyScaffoldResult, ScaffoldPlan } from '../shared.mts';
...
const fromLegacySync = <Value,>(operation: () => Value): Effect.Effect<Value, PolicyScaffoldError> =>
...
const createPolicyMutation = (
  ...
  createMutationEffect(filePath, content).pipe(Effect.mapError(planningFailure));

const renderPolicy = (policy: string, scope: 'global' | 'microvertical', owner?: string): string => {
...
const planPolicyScaffold = Effect.fn('PolicyScaffold.planPolicyScaffold')(function* planPolicyScaffoldEffect(
  workspaceRoot: string,
  config: PolicyScaffoldConfig,
): Effect.fn.Return<ScaffoldPlan<PolicyScaffoldResult>, PolicyScaffoldError, FileSystem.FileSystem> {
  ...
  if (config.scope === 'global') {
    ...
    const policyPath = yield* fromLegacySync(() =>
      resolveContainedPath(workspaceRoot, 'packages', 'core-runtime', 'src', 'policies', `${policy}.policy.ts`),
    );
    const policyMutation = yield* createPolicyMutation(policyPath, renderPolicy(policy, 'global'));
    ...
    const patchedIndex = yield* fromLegacySync(() =>
      insertSortedSlot(indexContent, CORE_POLICY_SLOT_START, CORE_POLICY_SLOT_END, [exportEntry], (candidate) =>
        /^export \{ [A-Za-z][A-Za-z0-9]*Policy \} from '\.\/policies\/[a-z0-9-]+\.policy\.ts';$/u.test(candidate),
      ),
    );
  ...
  const policyPath = yield* fromLegacySync(() =>
    resolveContainedPath(workspaceRoot, 'verticals', vertical.slug, 'src', 'policies', `${policy}.policy.ts`),
  );
  ...
  const mutations = dependencyMutation === undefined ? [policyMutation] : [policyMutation, dependencyMutation];
  yield* fromLegacySync(() => ensureUniqueMutationPaths(mutations));
  return { mutations, result: { policyPath } };
});
