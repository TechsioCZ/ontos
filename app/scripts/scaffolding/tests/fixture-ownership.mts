...
export const linkFixtureDependencies = Effect.fn(
  function* linkFixtureDependencies(
    ...
    for (const [name, source] of Object.entries(dependencies)) {
      ...
      yield* Effect.promise(
        async () => await mkdir(path.dirname(target), { recursive: true })
      );
      yield* Effect.promise(
        async () => await symlink(path.join(appRoot, source), target, 'dir')
      );
    }
  }
);
