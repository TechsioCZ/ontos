...
const projectsVertical: FixtureVertical = {
  appId: 'projects',
...
`,
  ...
  await createVertical(root, projectsVertical);
  await writeFixtureFile(
    root,
    'topology/reference-topology.json',
    json({
      schemaVersion: 1,
      verticals: [inventoryVertical, billingVertical, hrVertical, projectsVertical].map(
        (vertical) => ({
  ...
  await Promise.all(
    [inventoryVertical, billingVertical, hrVertical, projectsVertical].map((vertical) =>
...
test('documents every command and treats --help as a write-free operation', async () => {
  ...
  assert.match(getHelpText('microvertical-page'), /\/projects\/customers\/:id\/edit/u);
  assert.match(
  ...
  assert.match(
    getHelpText('external-http-adapter'),
    /--vertical projects --provider ares --operation subject/u,
  );
});
...
test('generates exactly one private owner-local external HTTP adapter', async () => {
  await withFixture(async (fixture) => {
    ...
    const adapterPath = path.join(
      fixture.root,
      'verticals/projects/src/integrations/ares/ares-subject.service.ts',
    );
    ...
    assert.equal(
      after['verticals/projects/src/integrations/ares/ares-subject.service.ts'],
...
>()('@app/projects/integrations/ares/ares-subject/AresSubjectService') {}
...
`,
    ...
    const source = after['verticals/projects/src/integrations/ares/ares-subject.service.ts'] ?? '';
...
test('rejects unsafe external HTTP adapter command input without writing', async () => {
  await withFixture(async (fixture) => {
    ...
    const invalidCalls: readonly [readonly string[], RegExp][] = [
      [['--vertical', 'projects', '--operation', 'subject'], /missing required flag --provider/u],
      [['--vertical', 'projects', '--provider', 'ares'], /missing required flag --operation/u],
      ...
      [
        ['--vertical', 'projects', '--provider', 'Ares', '--operation', 'subject'],
      ...
      [
        ['--vertical', 'projects', '--provider', 'src', '--operation', 'subject'],
        /provider must be canonical lower-kebab-case/u,
      ],
      [
        ['--vertical', 'projects', '--provider', 'ares', '--operation', 'node_modules'],
        /operation must be canonical lower-kebab-case/u,
      ],
      [
        ['--vertical', 'projects', '--provider', '../ares', '--operation', 'subject'],
...
test('external HTTP adapter planner rejects malformed OntOS ownership atomically', async () => {
  await withFixture(async (fixture) => {
    const manifestPath = path.join(fixture.root, 'verticals/projects/vertical.manifest.ts');
    const manifest = await readFile(manifestPath, 'utf-8');
    await writeFile(
      manifestPath,
      manifest.replace(
  ...
  await withFixture(async (fixture) => {
    await writeFixtureFile(
      fixture.root,
      'verticals/projects/src/integrations',
...
`,
    ...
    const ownerRoute = 'verticals/inventory-stock/src/routes/[lang]/projects/customers/[id]/edit';
    const shellRoute = 'apps/shell-super-app/src/routes/[lang]/projects/customers/[id]/edit';
    ...
    assert.match(ownerMetadata, /canonicalPath: '\/projects\/customers\/:id\/edit'/u);
    assert.match(ownerMetadata, /en: '\/projects\/customers\/:id\/edit'/u);
    assert.match(shellMetadata, /canonicalPath: '\/projects\/customers\/:id\/edit'/u);
    assert.match(manifest, /routePath: '\/projects\/customers\/:id\/edit'/u);
...
test('generates the Projects Contact-detail two-parameter page atomically and safely reruns it', async () => {
  const arguments_ = [
    '--vertical',
    'inventory-stock',
    '--page',
  ...
  await withFixture(async (fixture) => {
    const ownerRoute =
      'verticals/inventory-stock/src/routes/[lang]/projects/customers/[id]/contacts/[contactId]';
    const shellRoute =
      'apps/shell-super-app/src/routes/[lang]/projects/customers/[id]/contacts/[contactId]';
    ...
    assert.match(
      ownerMetadata,
      /canonicalPath: '\/projects\/customers\/:id\/contacts\/:contactId'/u,
    );
    assert.match(
      shellMetadata,
      /canonicalPath: '\/projects\/customers\/:id\/contacts\/:contactId'/u,
    );
    assert.match(manifest, /routePath: '\/projects\/customers\/:id\/contacts\/:contactId'/u);
  ...
  await withFixture(async (fixture) => {
    await writeFixtureFile(
      fixture.root,
      'apps/shell-super-app/src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/page.tsx',
