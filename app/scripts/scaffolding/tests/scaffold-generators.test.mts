...
const crmVertical: FixtureVertical = {
  appId: 'crm',
...
`,
  ...
  await createVertical(root, crmVertical);
  await writeFixtureFile(
    root,
    'topology/reference-topology.json',
    json({
      schemaVersion: 1,
      verticals: [inventoryVertical, billingVertical, hrVertical, crmVertical].map((vertical) => ({
  ...
  await Promise.all(
    [inventoryVertical, billingVertical, hrVertical, crmVertical].map((vertical) =>
...
test('documents every command and treats --help as a write-free operation', async () => {
  ...
  assert.match(
    getHelpText('external-http-adapter'),
    /--vertical crm --provider ares --operation subject/u,
  );
});
...
test('generates exactly one private owner-local external HTTP adapter', async () => {
  await withFixture(async (fixture) => {
    ...
    assert.equal(
      after['verticals/crm/src/integrations/ares/ares-subject.service.ts'],
...
`,
    ...
    const source = after['verticals/crm/src/integrations/ares/ares-subject.service.ts'] ?? '';
...
test('rejects unsafe external HTTP adapter command input without writing', async () => {
  await withFixture(async (fixture) => {
    ...
    const invalidCalls: readonly [readonly string[], RegExp][] = [
      [['--vertical', 'crm', '--operation', 'subject'], /missing required flag --provider/u],
      [['--vertical', 'crm', '--provider', 'ares'], /missing required flag --operation/u],
      [['--provider', 'ares', '--operation', 'subject'], /missing required flag --vertical/u],
      [
        ['--vertical', 'crm', '--provider', 'ares', '--operation', 'subject', '--unknown', 'x'],
        /unknown flag --unknown/u,
      ],
      [
        [
      ...
      [
        ['--vertical', 'crm', '--provider', 'Ares', '--operation', 'subject'],
      ...
      [
        ['--vertical', 'crm', '--provider', 'src', '--operation', 'subject'],
        /provider must be canonical lower-kebab-case/u,
      ],
      [
        ['--vertical', 'crm', '--provider', 'ares', '--operation', 'node_modules'],
        /operation must be canonical lower-kebab-case/u,
      ],
      [
        ['--vertical', 'crm', '--provider', '../ares', '--operation', 'subject'],
...
test('external HTTP adapter planner rejects malformed OntOS ownership atomically', async () => {
  await withFixture(async (fixture) => {
    const manifestPath = path.join(fixture.root, 'verticals/crm/vertical.manifest.ts');
    const manifest = await readFile(manifestPath, 'utf-8');
    await writeFile(
      manifestPath,
      manifest.replace(
  ...
  await withFixture(async (fixture) => {
    await writeFixtureFile(
      fixture.root,
      'verticals/crm/src/integrations',
