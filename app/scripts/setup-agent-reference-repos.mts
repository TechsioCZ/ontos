...
import { Config, Context, DateTime, Effect, FileSystem, Layer, Option, Path, Schema, Stream } from 'effect';
...
const loadRuntimeSettings = Effect.fn('loadRuntimeSettings')(function* loadRuntimeSettingsEffect() {
  ...
  return {
    gitIdentity: {
      ...
      GIT_COMMITTER_EMAIL: identityValue(environment.committerEmail, 'ultramodern-agent-refs@local'),
      GIT_COMMITTER_NAME: identityValue(environment.committerName, 'UltraModern Agent Reference Setup'),
    },
...
const assertSafeRepoPath = Effect.fn('assertSafeRepoPath')(function* assertSafeRepoPathEffect(relativePath: string) {
...
const hasGitSubtree = Effect.fn('hasGitSubtree')(function* hasGitSubtreeEffect() {
  ...
  return (result.status === 0 || result.status === 129) && result.stdout.includes('usage: git subtree');
});
...
const commitInstallerChanges = Effect.fn('commitInstallerChanges')(function* commitInstallerChangesEffect(
  message: string,
) {
  return yield* runCommand('git', ['commit', '-m', message], 120_000);
});
const ensureGitRepository = Effect.fn('ensureGitRepository')(function* ensureGitRepositoryEffect(checkOnly: boolean) {
  ...
  if (!(yield* hasCommits())) {
    ...
    yield* Effect.logInfo(`${LOG_PREFIX} creating initial workspace commit before adding reference subtrees`);
...
const remoteCommit = Effect.fn('remoteCommit')(function* remoteCommitEffect(repository: ReferenceRepository) {
  const branchOutput = yield* runCommand('git', ['ls-remote', repository.url, `refs/heads/${repository.ref}`], 120_000);
  const output =
...
const installedManifestEntry = Effect.fn('installedManifestEntry')(function* installedManifestEntryEffect(
  manifestPath: string,
  repository: ReferenceRepository,
) {
  ...
  return repositories.pipe(
    Option.flatMap((entries) => Option.fromUndefinedOr(entries.find((entry) => entry.id === repository.id))),
  );
});
const assertSubtreePresent = Effect.fn('assertSubtreePresent')(function* assertSubtreePresentEffect(
  ...
  const fileSystem = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  if (!(yield* fileSystem.exists(path.join(WORKSPACE_ROOT, repository.path)))) {
    return yield* setupError(`${repository.path} is missing`);
  }
  if (!(yield* subtreeCommitExists(repository))) {
    return yield* setupError(`${repository.path} is present but has no git-subtree commit evidence`);
  }
...
const addSubtree = Effect.fn('addSubtree')(function* addSubtreeEffect(
  ...
  const path = yield* Path.Path;
  const settings = yield* RuntimeConfiguration;
  const existing = yield* fileSystem.exists(path.join(WORKSPACE_ROOT, repository.path));
  ...
  yield* Effect.logInfo(`${LOG_PREFIX} adding ${repository.name} as git subtree at ${repository.path} (${commit})`);
...
const commitManifestIfChanged = Effect.fn('commitManifestIfChanged')(function* commitManifestIfChangedEffect(
  manifestPath: string,
) {
...
const runSetup = Effect.fn('runSetup')(function* runSetupEffect(checkOnly: boolean) {
  ...
  const config = yield* fileSystem.readFileString(configPath).pipe(
    Effect.flatMap(Schema.decodeUnknownEffect(ReferenceRepositoryConfigJsonSchema)),
    Effect.mapError(() => setupError(`Invalid reference repository configuration at ${configPath}`)),
  );
  if (!config.defaultEnabled || settings.skipRequested) {
    yield* Effect.logInfo(`${LOG_PREFIX} setup skipped; set ULTRAMODERN_SKIP_AGENT_REPOS=0 to enable it again`);
    return yield* Effect.void;
  }
  // Validate all destinations before any Git mutation, including initial commits.
  yield* Effect.forEach(config.repositories, (repository) => assertSafeRepoPath(repository.path), {
    discard: true,
  });
  if (!(yield* hasGit())) {
  ...
  const entries = yield* Effect.forEach(
    config.repositories,
    (repository) => (checkOnly ? assertSubtreePresent(manifestPath, repository) : addSubtree(manifestPath, repository)),
    { concurrency: 1 },
  );
  if (!checkOnly) {
...
const applicationLayer = Layer.merge(NodeServices.layer, Layer.effect(RuntimeConfiguration, loadRuntimeSettings()));
