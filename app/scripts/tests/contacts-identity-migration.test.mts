...
const legacyModule = `${['c', 'r', 'm'].join('')}.core`;
...
test('Contacts Core identity migration is preserving, scoped, rerunnable, and collision-safe', async () => {
  ...
  try {
    ...
    const payload = { freeText: `${legacyModule} must remain untouched inside arbitrary JSON` };
    await pool.query(
    ...
    const migrationSource = await readFile(
      new URL(
        `../../packages/core-runtime/drizzle/0008_rename-${['c', 'r', 'm'].join('')}-module-identity.sql`,
        import.meta.url,
      ),
      'utf-8',
    );
