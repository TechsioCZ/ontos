...
it.effect(
  'accepts only a development configuration with local service endpoints',
  () =>
    Effect.gen(function* testEffect1() {
      const configuration =
        yield* parseLocalDevelopmentConfiguration(localEnvironment);
      expect(configuration.email).toBe(LOCAL_DEVELOPMENT_CONTEXT.email);
      expect(configuration.databaseAdminUrl).toBe(
          ...
          Effect.gen(function* testEffect2() {
            expect(
              Schema.is(LocalDevelopmentInitializationError)(
                yield* Effect.flip(
                  parseLocalDevelopmentConfiguration(environment)
                )
              )
            ).toBe(true);
          })
...
it.effect(
  ...
  () =>
    Effect.gen(function* testEffect3() {
      ...
      expect(
        yield* classifyExactLocalRecord('tenant', undefined, expected)
      ).toBe('create');
      expect(
        yield* classifyExactLocalRecord('tenant', expected, expected)
      ).toBe('existing');
...
it.effect(
  'derives only configured Party Registry through its generated owner contract',
  () =>
    Effect.gen(function* testEffect5() {
      ...
      yield* Effect.tryPromise(() =>
        mkdir(path.join(root, TOPOLOGY_DIRECTORY), { recursive: true })
      );
      yield* Effect.tryPromise(() =>
        writeFile(path.join(root, TOPOLOGY_PATH), topology, 'utf-8')
      );
...
it.effect('rejects duplicate module IDs derived from different verticals', () =>
  Effect.gen(function* testEffect6() {
    ...
    const deriveContract = () =>
      Effect.succeed(moduleContract('duplicate.core'));
    expect(
      Schema.is(LocalDevelopmentInitializationError)(
        yield* Effect.flip(
...
it.effect(
  'generates stable module state IDs and complete access relationships',
  () =>
    Effect.gen(function* testEffect7() {
      ...
      expect(
        relationships.filter(({ relation }) => relation === 'legal_entity')
          .length
      ).toBe(2);
    })
);
...
it.effect(
  'a late module conflict rolls back Core bootstrap and retains its typed reason',
  () =>
    Effect.gen(function* testEffect8() {
      ...
      const conflict = yield* reconcileCoreContext(
        database,
        LOCAL_AUTH_USER_ID,
        [PARTY_REGISTRY_MODULE_ID]
      ).pipe(Effect.flip);
...
it.effect('native commit failure becomes a typed bootstrap error', () =>
  Effect.gen(function* testEffect9() {
    ...
    const error = yield* reconcileCoreContext(
      database,
      LOCAL_AUTH_USER_ID,
      []
    ).pipe(Effect.flip);
...
it.effect('bootstrap preserves unrelated defects after native rollback', () =>
  Effect.gen(function* testEffect10() {
    ...
    const exit = yield* reconcileCoreContext(
      database,
      LOCAL_AUTH_USER_ID,
      []
    ).pipe(Effect.exit);
