...
const makeFixture = async () => {
  ...
  await mkdir(path.join(root, LEDGER_PATH, 'src/worker-host'), {
    recursive: true,
  });
...
test('generates a separate supervised worker setup without changing owner configuration', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      generated.split("setup: 'ledger-worker'")[1],
      / run build/u
    );
    assert.doesNotMatch(
      generated.split("setup: 'ledger-worker'")[1],
    ...
    assert.equal(
      await generateOutboxWorkerDeployment(root, generated),
      generated
    );
  } finally {
...
test('bundles the real Party host including the production Effect HTTP health adapter', async () => {
  const runtimeDir = await mkdtemp(
    path.join(os.tmpdir(), 'ontos-party-worker-bundle-')
  );
  try {
    ...
    assert.equal(
      runtimePackage.dependencies['@effect/platform-node'],
      '4.0.0-beta.107'
    );
  } finally {
