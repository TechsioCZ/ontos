...
const decodeExitEvent = Schema.decodeUnknownSync(
...
const makeFixture = async () => {
  ...
  await mkdir(path.join(root, LEDGER_PATH, 'src/worker-host'), { recursive: true });
...
void test('generates a separate supervised worker setup without changing owner configuration', async () => {
  ...
  try {
    ...
    const generated = await generateOutboxWorkerDeployment(root, owner);
    ...
    assert.equal(await generateOutboxWorkerDeployment(root, generated), generated);
  } finally {
...
void test('materializes and starts a relocatable production worker artifact', async () => {
  ...
  try {
    ...
    const [statusOption] = decodeExitEvent(exitEvent);
    const status = Option.getOrNull(statusOption);
    ...
    assert.equal(
      artifact.sourceInputs.some((input: string) => input.endsWith(WORKER_HOST_ENTRY)),
      true,
    );
    ...
    const dataEvent: unknown = await once(runtime.stdout, 'data', {
      signal: AbortSignal.timeout(3000),
    });
    const [workerOutput] = decodeDataEvent(dataEvent);
