...
it.effect(
  ...
  Effect.fn(function* testEffect2() {
    const development = yield* selectActionAuthorizationProvisioningTarget(
    ...
    const stage =
      yield* selectActionAuthorizationProvisioningTarget(stageConfiguration);
...
it.effect(
  ...
  Effect.fn(function* testEffect4() {
    ...
    ).toBe(`${expected.code}: ${expected.reason}`);
    ...
    for (const error of [
      ...
      expect(formatActionAuthorizationProvisioningFailure(error)).toBe(
        unexpectedMessage
      );
    }
...
it.effect(
  ...
  Effect.fn(function* testEffect5() {
    ...
    const validationRoot = yield* Effect.acquireRelease(
      Effect.tryPromise(() =>
        mkdtemp(path.join(os.tmpdir(), 'ontos-workspace-validation-'))
      ),
      (directory) =>
...
it.effect(
  ...
  Effect.fn(function* testEffect7() {
      ...
      (): Promise<{
        ...
        import(
          pathToFileURL(
            path.resolve(
              import.meta.dirname,
              '../provision-current-action-authorization.mts'
            )
          ).href
        )
    );
    expect(
      yield* discoverCurrentActionKeys(workspaceRoot).pipe(
    ...
    expect(
      currentActionKeys.filter((key) => key.startsWith('party.registry.'))
        .length
    ).toBe(30);
  })
);
...
it.effect(
  ...
  Effect.fn(function* testEffect15() {
    ...
    expect(missingMembership.code).toBe(
...
const writeInventory = (
  ...
  Effect.gen(function* testEffect18() {
    yield* Effect.tryPromise(() =>
      mkdir(path.join(root, 'topology'), { recursive: true })
    );
    yield* Effect.all(
      [
        Effect.promise(() =>
...
it.effect(
  ...
  Effect.fn(function* testEffect19() {
    ...
    const { ActionAuthorizationProvisioningError: NativeProvisioningError } =
    ...
    const [currentPublicAction] =
      currentContract.manifest.publicSurface.actions;
    expect(currentPublicAction !== undefined).toBe(true);
    const root = yield* Effect.acquireRelease(
      Effect.tryPromise(() =>
        mkdtemp(path.join(os.tmpdir(), 'ontos-action-discovery-'))
      ),
      (directory) =>
        Effect.promise(() => rm(directory, { force: true, recursive: true }))
    );
    ...
    const incompleteError = yield* rejectionOf(
      discoverCurrentActionKeys(root, incomplete).pipe(
        Effect.provide(NodeServices.layer)
      )
    );
    expect(Schema.is(NativeProvisioningError)(incompleteError)).toBe(true);
    expect(
      Schema.decodeUnknownSync(NativeProvisioningError)(incompleteError).code
    ).toBe('action_authorization_discovery_failed');
    ...
    const duplicateError = yield* rejectionOf(
      discoverCurrentActionKeys(root, duplicate).pipe(
        Effect.provide(NodeServices.layer)
      )
    );
    expect(Schema.is(NativeProvisioningError)(duplicateError)).toBe(true);
    expect(
      Schema.decodeUnknownSync(NativeProvisioningError)(duplicateError).code
    ).toBe('action_authorization_discovery_failed');

    yield* writeInventory(root, [vertical, vertical]);
    const duplicateInventoryError = yield* discoverCurrentActionKeys(
      root,
      duplicate
    ).pipe(Effect.provide(NodeServices.layer), Effect.flip);
    expect(Schema.is(NativeProvisioningError)(duplicateInventoryError)).toBe(
      true
    );
    expect(duplicateInventoryError.code).toBe(
