...
const fixture = async () => {
  const root = realpathSync(
    mkdtempSync(path.join(tmpdir(), 'ontos-knip-model-'))
  );
  symlinkSync(
...
await test('real pinned Knip models exact consumers and preserves neighboring findings', async () => {
  ...
  try {
    ...
    const model = await runEffectTestPromise(
      buildKnipModel(root, base, consumerPath).pipe(
    ...
    const report = await runEffectTestPromise(
      Schema.decodeUnknownEffect(Schema.fromJsonString(ReportSchema))(
        run.stdout
      )
    );
    const findings = (
  ...
  try {
    ...
    await assert.rejects(
      runEffectTestPromise(
        buildKnipModel(root, { entry: [indexFile] }).pipe(
          Effect.provide(NodeServices.layer)
        )
      ),
...
await test('runner calibrates only the proven resolver record and retains the direct import', async () => {
  ...
  try {
    ...
    await runEffectTestPromise(
      runQualityAudit(root, output, 'knip').pipe(
        Effect.provide(NodeServices.layer)
      )
    );
    assert.deepEqual(
      readFileSync(path.join(appRoot, knipManifestFile)),
      installedManifest
    );
    assert.deepEqual(
      readFileSync(path.join(appRoot, 'node_modules/.bin/knip')),
      installedBinary
    );
...
await test('vendor ownership rejects a different installed copy and accepts the same canonical target', async () => {
  ...
  try {
    write(
      root,
      packageFile,
      await stringify({ name: 'copy-controls', private: true })
    );
