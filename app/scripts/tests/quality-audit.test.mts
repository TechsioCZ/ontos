...
await test('Fallow rejects missing discovery, unsupported schema and incomplete workspaces', async () => {
  ...
  await assert.rejects(
    validate(
      'fallow-files',
      await stringify({ file_count: 2, files: ['a.ts'] })
    ),
    /count disagrees/u
  );
});
...
`
    ...
    await assert.rejects(
      validate(FALLOW_HEALTH, corrupted),
      /contributions disagree/u
    );
    ...
    await assert.rejects(
      validate(FALLOW_HEALTH, wrongCount),
      /count disagrees/u
    );
  } finally {
  ...
  try {
        ...
        const matchesDistinctPolicies = report.clone_groups.some((group) =>
          policyFiles.every((file) =>
            group.instances.some(
              (instance) => path.basename(instance.file) === file
            )
          )
        );
    ...
    const jscpd = await runEffectTestPromise(
      Schema.decodeUnknownEffect(
        ...
        readFileSync(
          path.join(result.runDirectory, 'jscpd/report.json'),
          'utf-8'
        )
      )
    );
        ...
        Layer.effectDiscard(
          Effect.gen(function* externalReportDirectory() {
            ...
            const coverage = yield* Schema.decodeUnknownEffect(
              ...
              readFileSync(
                path.join(result.runDirectory, 'coverage.json'),
                'utf-8'
              )
            );
        ...
        yield* Effect.forEach(
          ...
          (args) =>
            spawner
              ...
              .pipe(
                Effect.tap((code) =>
                  Effect.sync(() => assert.equal(Number(code), 0))
                )
              ),
          { concurrency: 1 }
        );
...
await test('narrowed workspace and Fallow source discovery produce coverage errors', async () => {
  ...
  try {
    ...
    assert.match(
      narrowed.results.at(-1)?.diagnostic ?? '',
...
await test('a selected tool with the wrong installed version fails before launch', async () => {
  ...
  try {
    ...
    assert.equal(
      readFileSync(
        path.join(mismatch.runDirectory, 'knip/stdout.txt'),
        'utf-8'
      ),
      ''
    );
