...
const NodeFileSystemModuleSchema = Schema.Struct({
  ...
  rmSync: Schema.declare((input): input is typeof nodeRmSync =>
    Predicate.isFunction(input)
  ),
...
const fs = Result.getOrThrow(
  Schema.decodeUnknownResult(NodeFileSystemModuleSchema)(
    nodeRequire(nodeFileSystemModule)
  )
);
const { execFileSync } = Result.getOrThrow(
  Schema.decodeUnknownResult(NodeChildProcessModuleSchema)(
    nodeRequire(nodeChildProcessModule)
  )
);
...
const SHARED_VALIDATOR_STRING_008 =
...
const SHARED_VALIDATOR_STRING_011 =
  '.output/server/modern-worker-manifest.json';
...
const SHARED_VALIDATOR_STRING_034 =
...
const SHARED_VALIDATOR_STRING_049 =
...
const SHARED_VALIDATOR_STRING_052 =
...
const SHARED_VALIDATOR_STRING_055 =
...
const SHARED_VALIDATOR_STRING_066 =
...
const SHARED_VALIDATOR_STRING_071 =
  'http://localhost:4102/backend-mf-manifest.json';
const SHARED_VALIDATOR_STRING_072 =
  'http://localhost:4102/backendRemoteEntry.cjs';
...
const SHARED_VALIDATOR_STRING_085 =
...
const SHARED_VALIDATOR_STRING_087 =
...
const SHARED_VALIDATOR_STRING_097 =
...
const SHARED_VALIDATOR_STRING_112 =
...
const SHARED_VALIDATOR_STRING_116 =
  'scripts/generate-node-backend-federation.mts';
const SHARED_VALIDATOR_STRING_117 =
...
const SHARED_VALIDATOR_STRING_153 =
  'VERTICAL_PARTY_REGISTRY_BACKEND_MF_MANIFEST';
const SHARED_VALIDATOR_STRING_154 =
  'VERTICAL_PARTY_REGISTRY_DISPATCH_NAMESPACE';
...
const SHARED_VALIDATOR_STRING_164 =
...
const SHARED_VALIDATOR_STRING_167 =
  'verticals/party-registry/src/api/party-registry-client.ts';
...
const ComparableJsonObjectSchema = Schema.Record(
  Schema.String,
...
const workspaceValidationContractDefinition = {
  ciEvidenceScripts: {
    'action:test:integration':
      'pnpm --filter @app/core-runtime action:test:integration',
    ...
    'test:unit':
      'pnpm -r --if-present run test:unit && pnpm -r --if-present run test:component',
  },
  ...
  fullStackVerticals: [
    {
      ...
      port: 4102,
      routeMetaPaths: [
        'verticals/party-registry/src/routes/[lang]/contacts/route.meta.ts',
      ],
      routePagePaths: [
        'verticals/party-registry/src/routes/[lang]/contacts/page.tsx',
      ],
  ...
  packageScripts: {
    ...
    'database-access:check':
      'node ./scripts/check-database-access-boundaries.mts',
    'db:bootstrap-runtime-role':
      'node ./scripts/postgres/bootstrap-runtime-role.mts',
    'db:check':
    ...
    'scaffold:microvertical-page':
      'node ./scripts/scaffolding/cli.mts microvertical-page',
    'scaffold:module-api': 'node ./scripts/scaffolding/cli.mts module-api',
    'scaffold:module-contract':
      'node ./scripts/scaffolding/cli.mts module-contract',
    'scaffold:outbox-message':
      'node ./scripts/scaffolding/cli.mts outbox-message',
    'scaffold:outbox-worker':
      'node ./scripts/scaffolding/cli.mts outbox-worker',
    'scaffold:policy': 'node ./scripts/scaffolding/cli.mts policy',
    'scaffold:public-component':
      'node ./scripts/scaffolding/cli.mts public-component',
    'scaffold:report': 'node ./scripts/scaffolding/cli.mts report',
    'scaffold:search-provider':
      'node ./scripts/scaffolding/cli.mts search-provider',
    'scaffold:search-provider-access':
      'node ./scripts/scaffolding/cli.mts search-provider-access',
  ...
  topology: {
      ...
      owners: [
        ...
        },
        {
          ...
          ownership: {
            ...
            blastRadius: {
              references: [
                'docs/super-app-rfc-adr/blast-radius.md#party-registry',
              ],
    ...
    referenceTopology: {
      ...
      sharedPackages: [
        {
          description:
            'Server-only Core infrastructure and typed PostgreSQL ownership.',
          id: SHARED_VALIDATOR_STRING_064,
          package: SHARED_VALIDATOR_STRING_017,
          path: SHARED_VALIDATOR_STRING_092,
        },
        {
          description:
            'Server-only audience-bound Shell gateway assertion verification.',
          id: SHARED_VALIDATOR_STRING_176,
          package: SHARED_VALIDATOR_STRING_175,
          path: SHARED_VALIDATOR_STRING_177,
        },
...
type ReferenceTopologyVerticalDocument =
  ReferenceTopologyDocument['verticals'][number];
...
interface ReferenceTopology extends Omit<
  ReferenceTopologyDocument,
  'verticals'
> {
...
const PackageJsonSchema = Schema.Struct({
  ...
  engines: Schema.optionalKey(
    Schema.Struct({ node: Schema.optionalKey(Schema.String) })
  ),
...
const NodeFileTraceSchema = Schema.declare<NodeFileTrace>(
  (input): input is NodeFileTrace => Predicate.isFunction(input)
);
...
const expectedModernPackageSpecifier = (
  packageName: string
): string | undefined => {
...
const canonicalizeJsonValue = (value: ComparableJson): ComparableJson => {
  ...
  if (isComparableJsonObject(value)) {
    return Object.fromEntries(
      sortedCopy(Object.entries(value), ([left], [right]) =>
...
const canonicalizeJson = <Value,>(value: Value): ComparableJson | undefined =>
  value === undefined
    ...
    : canonicalizeJsonValue(
        Result.getOrThrow(
          Schema.decodeUnknownResult(ComparableJsonSchema)(value)
        )
      );
...
const formatJson = <Value,>(value: Value): string =>
  value === undefined
    ? 'undefined'
    : Inspectable.toStringUnknown(canonicalizeJson(value), 0);
...
const yamlMappingBlock = (
  ...
  const nextSibling = source
    ...
    .search(new RegExp(`\\n${indentation}\\S`, 'u'));
  const end =
    nextSibling === -1 ? undefined : start + marker.length + nextSibling;
  return source.slice(start, end);
};
const selfCheckFailure = (
  contract: string,
  message: string,
  fixArea: string
): string =>
  `MicroVertical contract self-check failed: ${contract}. ${message}. Fix area: ${fixArea}.`;
const assertSelfCheck: AssertSelfCheck = (
  condition,
  contract,
  message,
  fixArea
...
const assertSameIdCohort = (
  ...
  assertSameJson(
    sortedCopy(entries?.map((entry) => entry.id) ?? [], (left, right) =>
      left.localeCompare(right)
    ),
...
const assertGeneratedSurfaceRules = (
...
const assertWorkspaceValidationContract = (
  ...
  assert(
    metadataEntries.length ===
      (contract.cohort.releaseCohort === undefined ? 4 : 5),
...
const generatedSurfacePolicyFiles = (
  target: GeneratedSurfaceTarget
): string[] => {
  ...
  if (target.kind === 'file') {
    assert(
      ...
      `Missing generated surface policy file ${target.path}`
    );
    return [target.path];
  }
  ...
  assert(
    fs.existsSync(absolutePath),
    `Missing generated surface policy directory ${target.path}`
  );
  const files: string[] = [];
    ...
    if (current !== undefined) {
      for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          (target.extensions ?? []).includes(path.extname(entry.name))
        ) {
          files.push(
            path.relative(root, absoluteEntryPath).split(path.sep).join('/')
          );
        }
      }
    }
...
const skipJsxNonAttributeSource = (source: string, cursor: number): number => {
  ...
  return source[cursor] === '/'
    ? Math.max(cursor + 1, skipSourceComment(source, cursor))
    : cursor;
};
...
const findJsxAttribute = (
  ...
  while (elementIndex !== -1) {
    ...
    if (!isJsxNameCharacter(source[start])) {
      const match = findOpeningElementAttribute(
        source,
        start,
...
const assertGeneratedSurfacePolicy = () => {
  for (const rule of workspaceValidationContract.generatedSurfacePolicy.rules) {
    const files = sortedCopy(
      rule.paths.flatMap(generatedSurfacePolicyFiles),
      (left, right) => left.localeCompare(right)
    );
...
const assertStructuredWorkspaceMetadata = (): void => {
  ...
  for (const entry of observedMetadata) {
    assert(
      isMetadataDocument(entry.value),
...
const packageNameFor = (scope: string, suffix: string): string =>
  `@${scope}/${suffix}`;
...
const tailwindPrefixFor = (app: NormalizedApp): string =>
  app.kind === 'shell'
    ? 'shell'
    : tailwindPrefixForNamespace(app.domain ?? app.id);
...
const assertBuildFacadeExport = (
  ...
  const exportPattern = new RegExp(
    `export const ${exportName} = ${escapedSourcePath};`,
    'u'
  );
  assertSelfCheck(
...
const normalizedAppPath = (
  ...
  return kind === 'shell'
    ...
    : `verticals/${toKebabCase(rawApp.id)}`;
};
...
const normalizedAppApi = (
  ...
  return {
    consumedBy:
      rawApi.consumedBy === undefined
        ? [SHARED_VALIDATOR_STRING_131, id]
        : [...rawApi.consumedBy],
...
const normalizedApiExports = (
  ...
  const packageExports =
    readJson(PackageJsonSchema, `${appPath}/package.json`).exports ?? {};
  const apiContractExport =
    packageExports['./api'] === undefined ? undefined : './api';
  const clientExport =
    api?.protocol === 'rpc'
      ? SHARED_VALIDATOR_STRING_003
      : SHARED_VALIDATOR_STRING_002;
  const apiClientExport =
    packageExports[clientExport] === undefined ? undefined : clientExport;
...
const normalizeCompactApp = (rawApp: CompactApp): NormalizedApp => {
  ...
  const { apiClientExport, apiContractExport } = normalizedApiExports(
    appPath,
    api
  );
  ...
  return {
    ...
    portEnv,
    verticalRefs:
      moduleFederation.verticalRefs === undefined
        ? []
...
const remoteContractsFor = (
  ...
  (app.verticalRefs ?? [])
    .flatMap((ref) => {
      ...
      return remote === undefined || remote.exposes.length === 0
        ? []
        : [remote];
    })
    .map((remote) => ({
    ...
    files:
      publicRoutes.length > 0
        ? [
            ...
            SHARED_VALIDATOR_STRING_135,
            SHARED_VALIDATOR_STRING_134,
          ]
        : [SHARED_VALIDATOR_STRING_113],
...
const createCloudflareRoutes = (app: NormalizedApp) => {
  ...
  return {
    apiReadiness:
      app.api?.protocol === 'rest'
        ? `${app.api.prefix}/${app.api.stem}/readiness`
        : undefined,
    locale: hasRenderedSurface
      ? `/locales/en/${appNamespace(app)}.json`
      : undefined,
    mfManifest: SHARED_VALIDATOR_STRING_031,
  ...
  return {
    ...
    domainOperations:
      app.apiContractExport === undefined
        ? undefined
        : createEffectDomainOperations(app),
    import: '@modern-js/plugin-bff/effect-edge',
    openapi: '/openapi.json',
    prefix: api.prefix,
...
const createAppFederationContract = (
  app: NormalizedApp,
  apps: readonly NormalizedApp[]
) => ({
  ...
  name: app.mfName,
  remotes:
    app.verticalRefs.length > 0 ? remoteContractsFor(app, apps) : undefined,
...
const createPackageSourceView = (config: CompactConfig) => {
  ...
  if (source.strategy === 'install') {
    assert(
      isString(source.modernPackageVersion) &&
...
const backendFederationSubset = (
  backendFederation: CompactBackendFederation | undefined
) => {
  ...
  const effectApiExpose =
    backendFederation.exposes[SHARED_VALIDATOR_STRING_004];
...
const serverExecutionSubset = (
  serverExecution: OverlayServerExecution | undefined
) => ({
...
const verticalExposes = (vertical: FullStackVertical): string[] =>
  Array.isArray(vertical.exposes)
    ? vertical.exposes
    : Object.keys(vertical.exposes ?? {});
const regenerateMicroVerticalContractFix =
...
const topologyVerticalFederationView = (
  topologyEntry: ReferenceTopologyVertical
) => ({
...
const assertVerticalOwnershipAndOverlay = (
  vertical: FullStackVertical
): void => {
...
const assertShellDependenciesForVertical = (
  ...
  const composed =
    expectedShellVerticalIds.includes(vertical.id) &&
    vertical.exposes.length > 0;
  if (vertical.emitsApi || composed) {
    assertSameJson(
      valueForKey(
        Object.entries(shellPackage.dependencies ?? {}),
        vertical.packageName
      ),
      SHARED_VALIDATOR_STRING_169,
...
const generatedVerticalFederationView = (
  contractEntry: ReturnType<typeof createAppContract>
) => ({
...
const assertVerticalTsConfigReferenceGraph = (
  vertical: FullStackVertical
): void => {
  const verticalTsConfig = readJson(
    TsConfigSchema,
    `${vertical.path}/tsconfig.json`
  );
  ...
  const publishedContractRefs = fullStackVerticals.flatMap((candidate) => {
    ...
    const importsCandidate = sourceSpecifiers.some(
      (specifier) =>
        specifier === candidate.packageName ||
        specifier.startsWith(`${candidate.packageName}/`)
    );
    const dependencyDeclared =
      valueForKey(
        Object.entries(verticalPackage.dependencies ?? {}),
        candidate.packageName
      ) === SHARED_VALIDATOR_STRING_169;
    if (!importsCandidate && !dependencyDeclared) {
      return [];
    }
    const candidatePackage = readJson(
      PackageJsonSchema,
    ...
    assertPublishedCrossMicroVerticalContractUsage({
      ...
      readExportSource: (exportTarget) =>
        readText(`${candidate.path}/${exportTarget.slice(2)}`),
    });
    for (const exportKey of publishedOutboxContractExports(candidatePackage)) {
      ...
      const contractSource = readText(
        `${candidate.path}/${exportTarget.slice(2)}`
      );
      assertPublishedOutboxContractSource({
...
const primaryShellTsConfigReferences = () => {
  const expectedShellReferences = [
    ...
    ...sharedPackagePaths.filter(
      (packagePath) => packagePath !== SHARED_VALIDATOR_STRING_177
    ),
    ...(topology.shell?.verticalRefs ?? [])
      .flatMap((verticalRef) => {
        const vertical = fullStackVerticals.find(
    ...
    referenceFrom(SHARED_VALIDATOR_STRING_047, referencePath)
  );
...
const assertTsConfigReferenceGraph = () => {
  ...
  assertSameJson(
    EffectArray.sort(
      ...
      Order.mapInput(
        Order.String,
        (reference: { readonly path: string }) => reference.path
      )
    ),
    EffectArray.sort(
...
const observeModernPackageDependencies = (
  ...
  const standaloneModernTools =
    workspaceValidationContract.cohort.standaloneModernTools ?? {};
  for (const packageName of modernDependencyNames(packageJson)) {
    ...
    if (!modernPackageNameSet.has(packageName)) {
      ...
      assert(
        declared.every(
          (specifier) => specifier === undefined || specifier === expected
        ),
...
const assertModernPackageSpecifiers = (
  packageJson: PackageJson,
  relativePath: string
): void => {
...
const assertModernPackageCohort = () => {
  ...
  for (const packageJsonPath of packageJsonFiles(root)) {
    ...
    observeModernPackageDependencies(
      packageJson,
      relativePath,
...
const assertPublicHeadContract = (
  ...
  assert(
    publicHead.structuredData.optional,
  ...
  if (!hasOwnerPage) {
    for (const snippet of [
      ...
      assert(
        ...
        `${appId} private API head is missing ${snippet}`
      );
    }
    for (const snippet of [
      'rel="canonical"',
...
const extractAssetPrefixExpression = (modernConfig: string): string => {
  const match = /const\s+assetPrefix\s*=\s*(?<expression>[\s\S]*?);/u.exec(
    modernConfig
  );
  assert(
...
const assertTargetIsolatedBuildArtifacts = (
  appId: string,
  modernConfig: string
): void => {
  assert(
...
const assertCloudflareBuildSkipsDeployRebuild = (
  appId: string,
  packageJson: PackageJson
): void => {
  const cloudflareBuild =
    packageJson.scripts?.[SHARED_VALIDATOR_STRING_059] ?? '';
...
const assertActivePnpmVersion = (packageManagerUserAgent: string): void => {
  const activePnpmVersion = /^pnpm\/(?<version>\d+\.\d+\.\d+)/u.exec(
...
assert(
  pnpmWorkspace.includes(
    "'@vercel/nft@0.29.2': patches/@vercel__nft@0.29.2.patch"
  ),
...
const traceDeploymentSystemGlobs = Effect.gen(
  function* traceDeploymentSystemGlobs() {
    if (process.platform === 'win32') {
      return;
    }
    const partyRegistryRequire = createRequire(
      path.join(root, SHARED_VALIDATOR_STRING_165)
    );
    const appToolsRequire = createRequire(
      partyRegistryRequire.resolve(SHARED_VALIDATOR_STRING_022)
    );
    ...
    const tracerFixtureDirectory = fs.mkdtempSync(
      path.join(os.tmpdir(), 'ultramodern-system-glob-')
    );
...
assert(
  isString(rootPackage.packageManager),
...
const packageManagerPnpmVersion =
  packageManagerPnpmVersionMatch?.groups?.version ?? '';
assert(
...
assert(
  workflowNodeVersions.length > 0,
  'CI workflow must configure setup-node node-version'
);
assert(
  workflowNodeVersions.every(
    (nodeVersion) => nodeVersion === expectedNodeVersion
  ),
...
for (const [name, command] of requiredWorkflowEvidence) {
  assert(
    workflowText.includes(`name: ${name}`) &&
      workflowText.includes(`command: ${command}`),
...
for (const [jobId, jobName] of [
  [
    'service-integration',
    'Database, Migration, RLS, Authorization, and Outbox Integration',
  ],
  ['node-runtime', 'Node Backend Federation Artifact Proof'],
  ['cloudflare-runtime', 'Cloudflare Workerd Artifact Proof'],
]) {
  assert(
    workflowText.includes(`  ${jobId}:`) &&
      workflowText.includes(`name: ${jobName}`),
...
assert(
  workflowText.includes(
    'name: Apply and verify Core, Auth, and Contacts migrations'
  ) &&
    workflowText.includes(
...
assert(
  workflowText.includes('mise exec -- pnpm build') &&
    ...
    workflowText.includes(
      'MODERN_PUBLIC_SITE_URL: https://shell-super-app.invalid'
    ) &&
    workflowText.includes(
...
for (const vertical of topology.verticals ?? []) {
  ...
  assert(
    isString(verticalId),
    'Topology vertical deployment identity must be a string'
  );
...
assert(
  zeropsDeploymentSource.includes(
    `setup: ${quoteYamlString(shellDeploymentSetup)}`
...
const emitSourceCodeCharacter = (
  ...
  scanner.currentWord = isIdentifierChar(character)
    ? scanner.currentWord + character
    : '';
...
type StructuralShellPolicy =
  typeof workspaceValidationContractDefinition.structuralShellPolicy;
    ...
    if (fs.existsSync(srcAbsolute)) {
      for (const file of collectSourceFiles(srcAbsolute)) {
        ...
        for (const pattern of policy.forbiddenImportPatterns) {
          const match = new RegExp(pattern.expression, pattern.flags).exec(
            source
          );
          assert(
            match === null,
...
const assertFederatedCompositionSourcePolicy = (
  policy: FederatedCompositionSourcePolicy
): void => {
...
assert(
  packageSource.generatedWorkspacePackages?.specifier ===
    SHARED_VALIDATOR_STRING_169,
...
assert(
  rootPackage.scripts?.[SHARED_VALIDATOR_STRING_059] ===
...
if (bridgeConfig === undefined) {
  ...
  for (const workspacePackage of bridgeConfig.workspacePackages) {
    ...
    assert(
      readText(SHARED_VALIDATOR_STRING_103).includes(
        `  - ${workspacePackage.pattern}`
      ),
  ...
  assert(
    (
      valueForKey(Object.entries(rootPackage.scripts ?? {}), 'bridge:check') ??
      ''
    ).length > 0,
...
for (const [scriptName, expectedCommand] of Object.entries(
  ...
  assert(
    valueForKey(Object.entries(rootPackage.scripts ?? {}), scriptName) ===
      expectedCommand,
    `Root CI evidence command ${scriptName} is missing or incorrect`
  );
}
const coreRuntimePackage = readJson(
  PackageJsonSchema,
  SHARED_VALIDATOR_STRING_093
);
assert(
  coreRuntimePackage.scripts?.['test:unit'] ===
    'node --test tests/unit/*.test.ts',
  'Core runtime must expose its complete unit test surface'
);
assert(
  coreRuntimePackage.scripts?.['test:integration'] ===
...
const sharedContractsPackage = readJson(
  PackageJsonSchema,
  SHARED_VALIDATOR_STRING_095
);
assert(
...
assert(
  sameJson(gatewayPrincipalVerifierPackage.exports, {
    ...
    gatewayPrincipalVerifierPackage.dependencies?.[
      SHARED_VALIDATOR_STRING_017
    ] === SHARED_VALIDATOR_STRING_169 &&
    gatewayPrincipalVerifierPackage.dependencies?.[
      SHARED_VALIDATOR_STRING_019
    ] === SHARED_VALIDATOR_STRING_169 &&
...
const gatewayContractSource = readText(
  'packages/shared-contracts/src/gateway-context.ts'
);
const problemDetailsContractSource = readText(
...
assert(
  gatewayContractSource.includes('GATEWAY_ASSERTION_VERSION = 1') &&
    ...
    gatewayContractSource.includes(
      'GATEWAY_ASSERTION_CLOCK_SKEW_SECONDS = 30'
    ) &&
    gatewayContractSource.includes(
      "HttpApiEndpoint.post('issueGatewayContext'"
    ) &&
    gatewayContractSource.includes("alg: Schema.Literal('EdDSA')") &&
    gatewayContractSource.includes(
      "from '@app/core-runtime/actions/principal-context'"
    ) &&
    gatewayContractSource.includes('makeProblemDetailsSchema') &&
    problemDetailsContractSource.includes(
...
assert(
  !fs.existsSync(
    path.join(
      root,
      'packages/shared-contracts/src/gateway-topology.generated.ts'
    )
...
assert(
  rootPackage.scripts?.[SHARED_VALIDATOR_STRING_053] ===
    actionAuthorizationProvisioningCommand,
  'Root must expose the explicit current-Action authorization provisioning command'
);
assert(
  rootPackage.scripts?.['local:initialize'] ===
    'node ./scripts/initialize-local-development.mts' &&
    !readText('scripts/initialize-local-development.mts').includes(
      SHARED_VALIDATOR_STRING_106
    ) &&
    !readText('scripts/initialize-local-development.mts').includes(
      SHARED_VALIDATOR_STRING_053
    ),
...
for (const automaticScript of [
  ...
  const automaticCommand = valueForKey(
    Object.entries(rootPackage.scripts ?? {}),
    automaticScript
  );
  assert(
...
if (hasDeliveryUnits) {
  ...
  assert(
    rootPackage.scripts?.[SHARED_VALIDATOR_STRING_059]?.endsWith(
...
if (hasDeliveryUnits) {
  ...
  assert(
    zeropsYaml.includes(
      `setup: ${quoteYamlString(SHARED_VALIDATOR_STRING_132)}`
    ),
    'Zerops manifest must include shell service'
  );
  const shellSetup = yamlListItemBlock(
    zeropsYaml,
    'setup',
    SHARED_VALIDATOR_STRING_132
  );
  ...
  const cleanWorkspaceDependencies =
    'node scripts/reset-workspace-dependencies.mjs';
  assert(
  ...
  assert(
    zeropsYaml.split('--config.enable-global-virtual-store=false').length -
      1 ===
  ...
  assert(
    zeropsYaml.split('PNPM_CONFIG_ENABLE_GLOBAL_VIRTUAL_STORE=false').length -
      1 ===
      expectedZeropsPnpmCommands,
  ...
  const allDeclaredPortsPresent = [
    ...
    (app) => app !== undefined && zeropsYaml.includes(`PORT: '${app.port}'`)
  );
  assert(
  ...
  for (const deliveryUnitPath of [
    ...
    const deliveryUnitPackage = readJson(
      PackageJsonSchema,
      `${deliveryUnitPath}/package.json`
    );
    assert(
...
const assertToolWrapper = (scriptPath: string, command: string): void => {
  ...
  assert(
    hasUltramodernDispatch(
      source,
      command,
      readText('scripts/shared/ultramodern-command.mts')
    ) ||
      (command === 'skills' &&
        hasUltramodernSkillsDispatch(
          source,
          readText('scripts/shared/ultramodern-launch.mts')
        )),
    `${scriptPath} must delegate ${command} through the override-aware UltraModern runner`
  );
};
assert(
  performanceReadinessConfig.includes(
...
const i18nBoundaryScript = readText(
  'scripts/check-ultramodern-i18n-boundaries.mts'
);
...
assert(
  rootPackage.scripts?.[SHARED_VALIDATOR_STRING_060] ===
    expectedCloudflareDeployScript,
  'Root must expose cloudflare:deploy'
);
assert(
  rootPackage.scripts?.[SHARED_VALIDATOR_STRING_061] ===
    SHARED_VALIDATOR_STRING_084,
  'Root must expose cloudflare:proof'
);
assert(
  rootPackage.scripts?.['migrate:strict-effect'] ===
    SHARED_VALIDATOR_STRING_083,
...
assert(
  !agentSkillsBootstrap.includes("run('brew'") &&
    !agentSkillsBootstrap.includes('runShell('),
  'Agent skills bootstrap must never invoke system package managers'
);
const agentReferenceRepoSetup = readText(
  'scripts/setup-agent-reference-repos.mts'
);
assert(
...
const shellModernAppEnv = readText(
  'apps/shell-super-app/src/modern-app-env.d.ts'
);
...
const expectedZephyrDependencies = Object.fromEntries(
  expectedPrimaryShellVerticalIds.flatMap((verticalId) => {
    ...
    assert(
      vertical !== undefined,
      `Missing primary-shell vertical ${verticalId}`
    );
...
assert(
  sameJson(
    shellPackage[SHARED_VALIDATOR_STRING_173],
...
assert(
  shellPackage.scripts?.[SHARED_VALIDATOR_STRING_060] ===
    SHARED_VALIDATOR_STRING_144,
  'Shell must expose cloudflare:deploy'
);
assertTargetIsolatedBuildArtifacts(
  SHARED_VALIDATOR_STRING_131,
  shellModernConfig
);
assertCloudflareBuildSkipsDeployRebuild(
  SHARED_VALIDATOR_STRING_131,
  shellPackage
);
...
assert(
  shellContract?.deploy?.worker?.name ===
    expectedWorkerName(SHARED_VALIDATOR_STRING_131),
...
assert(
  shellContract?.config?.rspack?.output?.uniqueName ===
    SHARED_VALIDATOR_STRING_133,
...
assert(
  shellContract?.moduleFederation?.dts?.compilerInstance ===
    SHARED_VALIDATOR_STRING_068,
  'Shell must keep mandatory DTS compiler'
);
assert(
  shellContract?.moduleFederation?.dts?.tsConfigPath ===
    SHARED_VALIDATOR_STRING_007,
  'Shell must keep dedicated Module Federation DTS tsconfig'
);
assert(
  shellModuleFederationConfig.includes(
...
assert(
  shellContract?.routes?.privateByDefault ?? false,
...
for (const vertical of fullStackVerticals) {
  ...
  const hasActionPrincipal = fs.existsSync(
    path.join(root, actionPrincipalPath)
  );
  ...
  if (hasActionPrincipal) {
    ...
    assert(
      actionPrincipal.includes(
        "from '@app/gateway-principal-verifier/server'"
      ) &&
        actionPrincipal.includes(
          'bindGatewayPrincipalVerifier(ACTION_GATEWAY_AUDIENCE)'
        ) &&
  ...
  const routeMetadata = vertical.emitsUi
    ...
    : '';
  const ultramodernBuildSource = readText(
    `${vertical.path}/shared/ultramodern-build.ts`
  );
  ...
  assert(
    packageJson.scripts?.[SHARED_VALIDATOR_STRING_060] ===
      SHARED_VALIDATOR_STRING_144,
  ...
  assert(
    packageJson.scripts?.[SHARED_VALIDATOR_STRING_061]?.includes(
      `--app ${vertical.id}`
    ) ?? false,
  ...
  const expectedVerticalZephyrDependencies = Object.fromEntries(
    fullStackVerticals
      .filter((candidate) =>
        new Set<string>(vertical.verticalRefs).has(candidate.id)
      )
      .map((candidate) => [
  ...
  assert(
    sameJson(
      packageJson[SHARED_VALIDATOR_STRING_173],
  ...
  assert(
    modernConfig.includes(
      `const cloudflareWorkerName = '${expectedWorkerName(vertical.id)}'`
    ),
  ...
  if (vertical.emitsUi && vertical.exposes.length > 0) {
    ...
    assert(
      moduleFederationConfig.includes(
        "tsConfigPath: './tsconfig.mf-types.json'"
      ),
  ...
  if (vertical.emitsApi) {
    ...
    if (vertical.apiProtocol === 'rpc') {
      ...
      const restApi =
        contractEntry?.api && !('rpc' in contractEntry.api)
          ? contractEntry.api
          : undefined;
      assert(
      ...
      assert(
        restApi?.requestContext?.propagatedHeaders?.includes(
          SHARED_VALIDATOR_STRING_142
        ) ?? false,
  ...
  assert(
    sameJson(
      topologyEntry?.moduleFederation?.verticalRefs ?? [],
      vertical.verticalRefs
    ),
  ...
  if (vertical.emitsUi) {
    assert(
      valueForKey(
        Object.entries(overlay.manifests ?? {}),
        vertical.id
      )?.includes(SHARED_VALIDATOR_STRING_031) ?? false,
...
for (const expectedApp of workspaceValidationContract.topology.compactConfig
  ...
  const topologyUnitLabel =
    expectedApp.kind === 'shell' ? 'shell' : `verticals.${expectedApp.id}`;
  assertSameJson(
  ...
  const buildIdentity =
    readJson(BuildArtifactSchema, buildArtifactPath).deliveryUnit ?? {};
  assertSelfCheck(
    buildIdentity.unitId === expectedDeliveryUnit.unitId &&
      buildIdentity.buildMarker === expectedDeliveryUnit.buildMarker,
...
for (const legacyIdentityProbe of [
  'crm',
  'CRM',
  'Crm',
  'crmClient',
