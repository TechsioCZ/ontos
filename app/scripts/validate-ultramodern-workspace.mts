...
const assertOutboxWorkerSubscriptionCatalog = () => {
  const expectedSubscriptions = fullStackVerticals
    .flatMap((vertical) => {
      const workersPath = `${vertical.path}/src/workers`;
      if (!fs.existsSync(path.join(root, workersPath))) {
        return [];
      }
      return generatedSurfacePolicyFiles({
        extensions: ['.ts'],
        kind: 'directory',
        path: workersPath,
      }).flatMap((workerPath) => {
        const source = readText(workerPath);
        if (!source.startsWith('// @generated by OntOS Codesmith Outbox Worker v1\n')) {
          return [];
        }
        ...
        if (
          ...
          throw new Error(`Generated Outbox Worker markers are incomplete in ${workerPath}`);
        }
  ...
  assert(
    readText('packages/core-runtime/src/outbox/subscriptions.generated.ts') ===
      renderOutboxWorkerSubscriptionCatalog(expectedSubscriptions),
    'Generated Outbox Worker subscription catalog is stale; rerun scaffold:outbox-worker so every process matches against the complete installed catalog',
  );
};
...
assertOutboxWorkerSubscriptionCatalog();
