...
const run = runOxlint(
  join(pluginDirectory, 'report.config.ts'),
  scope,
  appRoot
);
...
for (const diagnostic of run.diagnostics) {
  ...
  if (!row)
    throw new Error(
      `Unexpected diagnostic in Effect report: ${diagnostic.code}`
    );
...
if (args.includes('--json')) {
  ...
  console.log(
    `Scope: ${scope.join(' ')}. Reporting only; no application files changed.`
  );
  console.log('\nRULE | TOTAL | SOURCE | TESTS | SCRIPTS');
  for (const row of report.rules)
    console.log(
      `${row.rule} | ${row.total} | ${row.source} | ${row.tests} | ${row.scripts}`
    );
  console.log('\nFILES (top 20; use --json for the complete report)');
  for (const row of report.files.slice(0, 20))
