...
function inScope(path: string, options: RuleOptions): boolean {
  ...
  if (
    ...
    (!options.includeTests && isTestFile(path))
  )
    return false;
  if (
    isScriptFile(path)
...
function constantInitializer(
  ...
  if (
    def?.type !== 'Variable' ||
    (def.parent as ESTree.VariableDeclaration)?.kind !== 'const'
  )
    return null;
...
function importedPolicy(
  ...
  if (declaration.importKind === 'type' || specifier.importKind === 'type')
    return null;
  const name =
    specifier.type === 'ImportSpecifier' ? importedName(specifier) : localName;
...
function hasHttpAnnotation(
  context: Context,
  binding: ESTree.Node | undefined
): boolean {
...
export const rule = defineRule({
  ...
  create(context) {
    ...
    const policyHelper = compile(
      options.policyHelperPattern,
      DEFAULT_POLICY_HELPER_PATTERN
    );
    const effectMemberOf = (node: ESTree.Node): string | null => {
      const identity = bindingPath(context, node);
      return identity?.length === 2 && identity[0] === 'Effect'
        ? (identity[1] ?? null)
        : null;
    };
    ...
    const effectCallee = (call: ESTree.CallExpression): string | null => {
      ...
      if (
        ...
        effectMemberOf(callee.callee) === 'fn'
      )
        return 'fn';
    ...
    const policyOf = (
      ...
      if (def?.type === 'ImportBinding')
        return importedPolicy(def, node.name, policyHelper);
    ...
    const pipeKind = (
      ...
      if (identity === 'pipe' || identity === 'Function.pipe')
        return 'function';
      return callee.type === 'MemberExpression' && memberKey(callee) === 'pipe'
        ? 'member'
        : null;
    };
    ...
    const inspectCall = (
      ...
      if (kind !== null) {
        ...
        mergeFollowing(
          call,
          Math.max(index + 1, kind === 'function' ? 1 : 0),
          merge
        );
        return 'continue';
      }
    ...
    const inspectAncestor = (
      ...
      return current.type === 'CallExpression'
        ? inspectCall(current, child, merge)
        : 'continue';
    };
    const inspectAncestors = (
      site: ESTree.Node
    ): { policy: Policy; finalizer: boolean } => {
    ...
    const initializedHttpClient = (
      init: ESTree.Node,
      seen: Set<unknown>
    ): boolean => {
      const actual = identityUnwrap(init);
      if (
        actual.type === 'YieldExpression' &&
        actual.delegate &&
        actual.argument !== null
      )
        return (
          bindingPath(context, actual.argument)?.join('.') ===
          'HttpClient.HttpClient'
        );
      if (actual.type === 'CallExpression') {
        const identity = bindingPath(context, actual.callee)?.join('.');
        return (
          identity === 'HttpClient.make' || identity === 'FetchHttpClient.make'
        );
      }
    ...
    const localImport = (call: ESTree.CallExpression): boolean => {
      ...
      if (!/(?:^packages\/core-runtime\/|\/api\/|\/server\/)/u.test(path))
        return false;
      ...
      return (
        ...
        /^\.{1,2}\//u.test(staticString(body.source) ?? '')
      );
    };
    const report = (
      node: ESTree.Node,
      ...
      context.report({
        ...
        messageId,
        data: {
          callee: context.sourceCode.getText(
    ...
    return {
      ...
      MemberExpression(node) {
        if (!options.promiseBridges.includes(effectMemberOf(node) ?? ''))
          return;
