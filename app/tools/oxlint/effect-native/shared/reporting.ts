...
export function snippet(text: string, limit: number, sliceLength = limit - 1, ellipsis = '…'): string {
  const flat = text.replace(/\s+/gu, ' ').trim();
  return flat.length > limit ? `${flat.slice(0, sliceLength)}${ellipsis}` : flat;
}
export function sameNode(left: ESTree.Node | null | undefined, right: ESTree.Node | null | undefined): boolean {
...
export function nodeKey(node: ESTree.Node, kindSeparator?: ':' | '@'): string {
  ...
  return kindSeparator === undefined ? span : `${node.type}${kindSeparator}${span}`;
}
/** Use node.start/end directly when null checking is not required. */
export function spanOf(node: ESTree.Node | null | undefined): { readonly start: number; readonly end: number } | null {
