...
function isTopLevelImmediatelyInvoked(fn: ESTree.Node): boolean {
  ...
  return parent?.type === 'CallExpression' && parent.callee === node && isTopLevel(parent);
}
function isOnlyCalledFromTopLevel(context: Context, fn: ESTree.Node, name: string): boolean {
  const variable = resolveVariable(context, name, fn);
  if (!variable) return false;
  const offsets = new Set(variable.identifiers.map((identifier) => identifier.start));
