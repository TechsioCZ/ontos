...
it('JSON rule scope preserves defaults, overrides, and exclusions', () => {
  ...
  expect(
    inJsonRuleScope(testFile, { ignoreTestFiles: false }, includePaths)
  ).toBe(true);
  expect(
    inJsonRuleScope(sourceFile, { allowPaths: ['apps/**'] }, includePaths)
  ).toBe(false);
  expect(inJsonRuleScope(sourceFile, { includePaths: [] }, includePaths)).toBe(
    true
  );
  expect(inJsonRuleScope(sourceFile, { includePaths: [1] }, includePaths)).toBe(
    true
  );
  expect(
    inJsonRuleScope(
