...
it('rejects crashes, stderr failures, and inconsistent exit statuses', () => {
  expect(() => parseOxlintOutput(report(), '', null)).toThrow(/did not complete/u);
  expect(() => parseOxlintOutput(report(), '', 2)).toThrow(/did not complete/u);
  expect(() => parseOxlintOutput(report(), 'plugin crashed', 0)).toThrow(/stderr/u);
  expect(() => parseOxlintOutput(report(), '', 1)).toThrow(/contradicts/u);
  expect(() => parseOxlintOutput(report([diagnostic]), '', 0)).toThrow(/contradicts/u);
});
