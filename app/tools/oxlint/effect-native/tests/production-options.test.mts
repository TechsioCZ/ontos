...
const FixtureConfig = Schema.fromJsonString(
  Schema.Struct({
    overrides: Schema.optional(
      Schema.Array(
        Schema.Struct({ files: Schema.Array(Schema.String), rules: RuleMap })
      )
    ),
    rules: RuleMap,
  })
  ...
  it(`effect-native/${rule} production settings report a positive fixture`, () => {
    withTemporaryWorkspace((directory) => {
      for (const kind of ['invalid', 'valid']) {
        cpSync(
          nodePath.join(fixturesDirectory, rule, kind),
          nodePath.join(directory, kind),
          {
            recursive: true,
          }
        );
      ...
      const fixture = decodeFixtureConfig(
        readFileSync(fixtureConfigPath(rule), 'utf-8')
      );
