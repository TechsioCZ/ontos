...
if (flags.some((flag) => flag !== '--all' && !/^--limit=\d+$/u.test(flag))) throw new Error('Unknown report flag');
const limit = Number(flags.find((flag) => flag.startsWith('--limit='))?.slice('--limit='.length) ?? 40);
if (!Number.isSafeInteger(limit) || limit < 1) throw new Error('--limit must be a positive integer');
const run = runOxlint(fixtureConfigPath(rule), ['apps', 'verticals', 'packages', 'scripts'], appRoot);
if (run.stderr.trim()) console.error(run.stderr.trim());
for (const diagnostic of run.diagnostics) {
  if (diagnostic.code !== `effect-native(${rule})`) throw new Error(`Unexpected diagnostic: ${diagnostic.code}`);
}
...
for (const [file, count] of perFile) {
  if (/(?:^|\/)(?:tests?|__tests__)\/|\.(?:test|spec|test-d|spec-d)\.[cm]?[jt]sx?$/u.test(file)) groups.tests += count;
