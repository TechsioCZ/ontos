-- Forward-upgrade cleanup: description changed the create/update routine contracts. Remove only
-- stale overloads so no previously granted runtime capability survives alongside the owner API.
DO $$
DECLARE
  v_routine record;
BEGIN
  FOR v_routine IN
    ...
    WHERE namespace.nspname = 'commerce_customer_context'
      AND routine.proname IN ('create_customer_group', 'update_customer_group')
      AND routine.oid <> coalesce(
        CASE routine.proname
          WHEN 'create_customer_group' THEN pg_catalog.to_regprocedure(
            'commerce_customer_context.create_customer_group(uuid,uuid,text,text,text,text,text,text,text,timestamptz,uuid,uuid)'
          )
          ELSE pg_catalog.to_regprocedure(
            'commerce_customer_context.update_customer_group(uuid,uuid,uuid,integer,text,text,text,text,text,timestamptz,uuid,uuid)'
          )
        END,
        0
      )
  LOOP
    EXECUTE pg_catalog.format(
      'REVOKE ALL ON FUNCTION %s FROM PUBLIC, ontos_runtime',
      v_routine.signature
    );
    EXECUTE pg_catalog.format('DROP FUNCTION %s', v_routine.signature);
  END LOOP;
END;
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."customer_group_document"(
  p_tenant_id uuid,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."customer_group_membership_document"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."read_customer_group"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid
)
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."create_customer_group"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."update_customer_group"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."archive_customer_group"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid,
  p_expected_revision integer,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."reactivate_customer_group"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid,
  p_expected_revision integer,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."assign_customer_group_membership"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."remove_customer_group_membership"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_membership_id uuid,
  p_customer_group_id uuid,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."read_customer_group_members"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid,
  p_effective_at timestamptz,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."read_customer_group_history"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_group_id uuid,
  p_as_of timestamptz,
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION "commerce_customer_context"."read_effective_customer_group_memberships"(
  p_tenant_id uuid,
  p_legal_entity_id uuid,
  p_customer_profile_id uuid,
  p_profile_kind text,
