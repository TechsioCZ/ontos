...
ALTER TABLE "commerce_customer_context"."purchase_approval_requests" ADD COLUMN "committed_order_ref" jsonb;--> statement-breakpoint
ALTER TABLE "commerce_customer_context"."purchase_approval_requests" ADD COLUMN "consumption_commitment_id" text;--> statement-breakpoint
ALTER TABLE "commerce_customer_context"."counterparty_purchasing_profiles" ADD CONSTRAINT "ccc_counterparty_profiles_tenant_id_uk" UNIQUE("tenant_id","counterparty_purchasing_profile_id");--> statement-breakpoint
ALTER TABLE "commerce_customer_context"."customer_profiles" ADD CONSTRAINT "ccc_profiles_tenant_id_uk" UNIQUE("tenant_id","customer_profile_id");--> statement-breakpoint
CREATE UNIQUE INDEX "ccc_approval_requests_active_proposal_uk" ON "commerce_customer_context"."purchase_approval_requests" ("tenant_id","legal_entity_id","proposal_revision_resource_id") WHERE "status" in ('PENDING', 'APPROVED');--> statement-breakpoint
CREATE UNIQUE INDEX "ccc_purchase_proposals_current_resource_uk" ON "commerce_customer_context"."purchase_proposal_revisions" ("tenant_id","legal_entity_id","proposal_revision_resource_id") WHERE "state" = 'CURRENT';--> statement-breakpoint
CREATE UNIQUE INDEX "ccc_purchase_proposals_current_cart_revision_uk" ON "commerce_customer_context"."purchase_proposal_revisions" ("tenant_id","legal_entity_id",("proposal_snapshot"->'sourceCart'->'cartRef'->>'moduleId'),("proposal_snapshot"->'sourceCart'->'cartRef'->>'resourceType'),("proposal_snapshot"->'sourceCart'->'cartRef'->>'resourceId'),("proposal_snapshot"->'sourceCart'->'cartRef'->>'tenantId'),("proposal_snapshot"->'sourceCart'->>'revision')) WHERE "state" = 'CURRENT';--> statement-breakpoint
