...
const port = Option.getOrElse(
  Result.getOrThrow(
    ...
    )(getBuildConfigEnvironment('VERTICAL_COMMERCE_CUSTOMER_CONTEXT_PORT')),
  ),
  () => 4101,
);
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_COMMERCE_CUSTOMER_CONTEXT');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_COMMERCE_CUSTOMER_CONTEXT, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
