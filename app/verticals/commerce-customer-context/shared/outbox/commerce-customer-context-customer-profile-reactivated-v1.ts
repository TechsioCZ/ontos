// @generated by OntOS Codesmith Outbox Message Contract v1
// @ontos-outbox-producer commerce.customer-context
// @ontos-outbox-topic commerce.customer-context.customer-profile-reactivated.v1
import { Schema } from 'effect';
import { ProfileInstantSchema } from '../domain/profile-contracts.ts';
import { CommerceCustomerProfileRefSchema } from '../domain/profile-decisions.ts';

export const OutboxPayloadSchema = Schema.Struct({
  effectiveAt: ProfileInstantSchema,
  previousState: Schema.Literals(['SUSPENDED', 'ARCHIVED']),
  profileRef: CommerceCustomerProfileRefSchema,
  revision: Schema.Finite.check(Schema.isInt(), Schema.isGreaterThanOrEqualTo(1)),
  state: Schema.Literal('ACTIVE'),
});
export type OutboxPayload = Schema.Schema.Type<typeof OutboxPayloadSchema>;

export const outboxTopic = 'commerce.customer-context.customer-profile-reactivated.v1' as const;
export const outboxProducerModuleKey = 'commerce.customer-context' as const;
