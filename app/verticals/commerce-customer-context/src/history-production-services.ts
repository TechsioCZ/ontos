        ...
        Effect.map((decisions) => {
          const malformed =
            ...
            decisions.some(({ key }, index) => {
              ...
              return (
                ref === undefined || key !== `${ref.moduleId}:${ref.resourceType}:${ref.resourceId}`
              );
            });
...
export const customerHistoryPortsForOperation = Effect.fn(
  'CustomerHistoryProduction.customerHistoryPortsForOperation',
    ...
    counterpartyProfiles: {
      current: (input) => {
        if (
          ...
          input.profileRef.tenantId !== scope.tenantId
        ) {
          return Effect.succeed('ABSENT');
        }
