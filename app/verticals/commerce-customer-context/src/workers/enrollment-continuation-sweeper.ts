import { Clock, Duration, Effect, Layer, Ref, Schedule } from 'effect';
...
const sweepEntry = (state: Ref.Ref<Registry>, inner: Continuation, entry: TrackedAttempt): Effect.Effect<boolean> =>
  entry.sweeps >= SWEEP_BUDGET
    ? Ref.update(state, (registry) => withoutKey(registry, registryKey(entry.attempt))).pipe(Effect.as(false))
...
/**
 * One sweep. Every tracked Attempt whose last activity is older than the stale window is advanced
 * again; the advance itself decides what happens next, exactly as the start route's fork does.
 */
const sweepOnce = Effect.fn('CommerceEnrollmentContinuationSweeper.sweep')(function* sweepOnceEffect(
  ...
  const [now, registry] = yield* Effect.all([Clock.currentTimeMillis, Ref.get(state)], { concurrency: 2 });
  const due = [...registry.values()].filter((entry) => now - entry.lastActivityMillis >= staleAfterMillis);
...
export const commerceEnrollmentContinuationSweeperFor = Effect.fnUntraced(
  function* commerceEnrollmentContinuationSweeperFor(options: CommerceEnrollmentContinuationSweeperOptions) {
    const state = yield* Ref.make<Registry>(new Map());
    ...
    const sweeper: CommerceEnrollmentContinuationSweeper = {
      continuation: { advance: (attempt) => trackedAdvance(state, inner, attempt, 0) },
 ...
 * fail-closed continuation that refuses every Attempt, which is a no-op rather than a second code
 * path.
 */
