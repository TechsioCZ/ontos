...
const grantRow = (
  state: 'ACTIVE' | 'PENDING_GRANT' | 'PENDING_REVOKE' | 'RECONCILIATION_REQUIRED' | 'REVOKED',
  operationOutcome?:
    | 'ALREADY_ACTIVE'
    | 'ALREADY_REVOKED'
    | 'CONFLICT'
...
const pendingInvitationRow = (
  operationOutcome: 'ALREADY_PENDING' | 'ALREADY_SENT' | 'CREATED' | 'RESENT',
) => ({
  claimed_at: null,
...
it.effect('commits a durable grant intent without performing the external relationship write', () =>
  Effect.gen(function* grantAccess() {
    ...
    expect(calls).toEqual([
      'counterparty-access.begin-grant',
      'counterparty-access.transition-grant',
    ]);
  }),
);
...
it.effect(
  'returns last-admin protection and idempotent active outcomes without staging another intent',
  () =>
...
it.effect('reconciles one exact committed mutation and returns terminal grant evidence', () =>
  Effect.gen(function* reconcileOneCommittedMutation() {
    ...
    const result = yield* accessAuthorizationMutationReconciliationForTransaction(
      dependenciesWith(transaction),
      {
        mutate: () =>
          Effect.sync(() => {
...
it.effect(
  'reconciles durable invitation grants and returns replay-safe terminal claim evidence',
  () =>
  ...
  Effect.gen(function* compensateLostInviterAuthority() {
    ...
    const contextAccess = {
      businessPermissions: ({
        ...
        NonNullable<CounterpartyAccessPersistenceContext['contextAccess']['businessPermissions']>
      >[0]) => {
        authorityChecks += 1;
  ...
  Effect.gen(function* workerOwnerOverlay() {
    ...
    const transaction = invokerWith((routineKey) => {
      ...
      if (routineKey === 'counterparty-access.mutate-invitation') {
        return [
          { ...pendingInvitationRow('ALREADY_PENDING'), operation_outcome: 'CLAIM_REJECTED' },
        ];
      }
      return [];
    });
    const contextAccess = {
      businessPermissions: () => {
        ...
        return Effect.succeed([
          { decision: 'allowed' as const, key: 'counterparty.access.manage' as const },
        ]);
      },
    };
    const service = accessAuthorizationMutationReconciliationForWorker(
...
it.effect('resets a claim when inviter authority is lost before any grant row exists', () =>
  Effect.gen(function* compensateClaimWithoutGrantRows() {
    ...
    const contextAccess = {
      ...
      >[0]) =>
        Effect.succeed(
          targets.map(({ permission }) => ({ decision: 'denied' as const, key: permission })),
        ),
    };
    const service = accessAuthorizationMutationReconciliationForWorker(
      contextAccess,
...
it.effect(
  'compensates a partial claim after revoke and converges once every tuple is removed',
  () =>
...
it.effect('rejects a mismatched invitation claim before staging or relationship mutation', () =>
  Effect.gen(function* rejectMismatchedClaim() {
    ...
    const failure = yield* Effect.flip(
      service.reconcile(workerScope(transaction), claimReconciliationRequest),
    );
...
it.effect(
  'binds one consumed attestation to the exact claim and persists only its safe reference',
  () =>
                      ...
                      grant_progress: [
                        {
                          grantRef: {
                            ...
                            resourceId: grantId,
                            resourceType:
                              'commerce.customer-context.counterparty-commerce-access-grant',
                            tenantId,
                          },
                          permission: 'counterparty.access.manage',
...
it.effect('enforces Counterparty-wide last-admin protection without foreign grant disclosure', () =>
  Effect.gen(function* verifyAccessRoutineSecurity() {
    const migration = yield* Effect.promise(() =>
      readFile(
        new URL('../../drizzle/20260909112255_access-routines/migration.sql', import.meta.url),
        'utf-8',
      ),
    );
    ...
    const lastAdministratorCheck = revokeRoutine.slice(
      ...
      revokeRoutine.indexOf(
        '  ELSE\n    UPDATE commerce_customer_context.counterparty_commerce_access_grants',
      ),
    );

    expect(directGrantLookup).toContain(
      'grant_row.counterparty_purchasing_profile_id = v_profile_id',
    );
    ...
    expect(bootstrapCheck).toContain(
      "administrator.permission_code = 'counterparty.access.manage'",
    );
  }),
);
...
it.effect('hardens durable saga transitions and invitation claim recovery in owner routines', () =>
  Effect.gen(function* verifySagaRoutineSecurity() {
    ...
    expect(transition).toContain(
      "p_target_state = 'ACTIVE' AND lifecycle <> 'ACTIVE' THEN statement_timestamp()",
    );
