...
const migration = readFileSync(
  new URL('../../drizzle/20260909151000_bind-ccc-reconciliation-owner-composite/migration.sql', import.meta.url),
  'utf-8',
);
...
it('keeps PURCHASE_LIMITS reconciliation fail-closed for the owner receipt', () => {
  ...
  expect(purchaseLimits).toContain("v_status:=CASE WHEN v_total_count=0 THEN 'NOT_APPLICABLE' ELSE 'RESOLVED' END");
  ...
  expect(purchaseLimits.slice(0, purchaseLimitLogicEnd).replaceAll(/FOR UPDATE(?: OF [a-z_]+)?/gu, '')).not.toMatch(
