...
const appId = 'commerce-fx';
const cloudflareWorkerName = 'app-commerce-fx';
const port = Option.getOrElse(
  Result.getOrThrow(
    ...
    )(getBuildConfigEnvironment('VERTICAL_COMMERCE_FX_PORT')),
  ),
  () => 4104,
);
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_COMMERCE_FX');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_COMMERCE_FX, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
  ...
  presetUltramodern(
    {
      ...
      tools: {
        ...
        bundlerChain: (chain) => {
          chain.output
            .uniqueName('verticalCommerceFx')
            .chunkLoadingGlobal('__ULTRAMODERN_VERTICAL_COMMERCE_FX_LOADED_CHUNKS__');
        },
