...
const cloudflareWorkerName = 'app-contacts';
const port = Number(getBuildConfigEnvironment('VERTICAL_CONTACTS_PORT') ?? 4101);
const envValue = (name: string) => {
  const value = getBuildConfigEnvironment(name)?.trim();
  return value !== undefined && value.length > 0 ? value : undefined;
};
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_CONTACTS');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_CONTACTS, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
    ...
    worker: {
      ...
      security: {
        ...
        cors: {
          ...
          allowedOrigins: contactsCorsAllowedOrigins(moduleFederationDevServerOrigin),
          assets: true,
          reason: 'Allow the configured Shell to load Contacts assets and invoke its BFF.',
        },
        enabled: true,
        headers: {
  ...
  presetUltramodern(
    {
      ...
      source: {
        ...
        globalVars: {
          ULTRAMODERN_CONTACTS_API_BASE_URL: `${remoteAssetOrigin.replace(/\/+$/u, '')}/contacts-api`,
