...
const LoadingContactDetail = ({ copy }: { readonly copy: ContactDetailCopy }) => (
  <div aria-busy="true" className="contacts:grid contacts:min-w-0 contacts:gap-6">
    <h1 className="contacts:sr-only" id="contact-detail-heading">
      {copy.loading}
    </h1>
...
const ContactValueLink = ({
  ...
  href === undefined ? (
    ...
    <Link
      aria-label={label}
      className="contacts:block contacts:min-w-0 contacts:w-full contacts:max-w-full contacts:break-all contacts:whitespace-normal"
      href={href}
    >
      {withSoftWrapOpportunities(value)}
    </Link>
...
const ReadyContactDetail = ({
  ...
  <div className="contacts:grid contacts:min-w-0 contacts:gap-6">
    ...
    <dl className="contacts:grid contacts:min-w-0 contacts:gap-x-6 contacts:gap-y-4 contacts:sm:grid-cols-[minmax(8rem,12rem)_minmax(0,1fr)]">
      <dt className="contacts:font-medium">{copy.contactId}</dt>
      ...
      <dt className="contacts:font-medium">{copy.email}</dt>
      <dd className="contacts:min-w-0 contacts:break-all">
        <ContactValueLink href={contact.emailHref} label={copy.emailLink} value={contact.email} />
      </dd>
      <dt className="contacts:font-medium">{copy.phone}</dt>
      <dd className="contacts:min-w-0 contacts:break-all">
...
export const toContactDetailReadyModel = (
  ...
  if (Schema.is(ContactsEmailSchema)(contact.email)) {
    links.emailHref = `mailto:${contact.email}`;
  }
  if (Schema.is(ContactsPhoneSchema)(contact.phone)) {
...
const ContactDetailQuery = ({
  ...
  return (
    <ContactDetailView
      backHref={backHref}
      copy={copy}
      editHref={`/${language}/contacts/customers/${customerId}/contacts/${contactId}/edit`}
      onRetry={refetch}
...
const ContactDetailFeature = ({ routeParams }: ContactDetailPageProps) => {
  ...
  const copy: ContactDetailCopy = {
    authenticationExpired: t('contacts.pages.contactDetail.states.authenticationExpired'),
    back: t('contacts.pages.contactDetail.back'),
    contactId: t('contacts.pages.contactDetail.fields.contactId'),
    createdAt: t('contacts.pages.contactDetail.fields.createdAt'),
    customerId: t('contacts.pages.contactDetail.fields.customerId'),
    decode: t('contacts.pages.contactDetail.states.decode'),
    edit: t('contacts.pages.contactDetail.edit'),
    email: t('contacts.pages.contactDetail.fields.email'),
    emailLink: t('contacts.pages.contactDetail.links.email'),
    forbidden: t('contacts.pages.contactDetail.states.forbidden'),
    internal: t('contacts.pages.contactDetail.states.internal'),
    loading: t('contacts.pages.contactDetail.states.loading'),
    notFound: t('contacts.pages.contactDetail.states.notFound'),
    phone: t('contacts.pages.contactDetail.fields.phone'),
    phoneLink: t('contacts.pages.contactDetail.links.phone'),
    retry: t('contacts.pages.contactDetail.states.retry'),
    retrying: t('contacts.pages.contactDetail.states.retrying'),
    saved: t('contacts.pages.contactEdit.mutation.success'),
    status: t('contacts.pages.contactDetail.fields.status'),
    statusActive: t('contacts.pages.contactDetail.lifecycle.active'),
    statusArchived: t('contacts.pages.contactDetail.lifecycle.archived'),
    title: t('contacts.pages.contactDetail.title'),
    transport: t('contacts.pages.contactDetail.states.transport'),
    unavailable: t('contacts.pages.contactDetail.states.unavailable'),
    updatedAt: t('contacts.pages.contactDetail.fields.updatedAt'),
  };
  const backHref =
    customerId === undefined ? undefined : `/${language}/contacts/customers/${customerId}`;
  ...
  return customerId === undefined || contactId === undefined ? (
    ...
    <ContactDetailQuery
      backHref={`/${language}/contacts/customers/${customerId}`}
      contactId={contactId}
      copy={copy}
