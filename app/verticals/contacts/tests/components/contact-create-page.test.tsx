...
} from '../../src/routes/[lang]/contacts/customers/[id]/contacts/new/page.tsx';
...
test('renders the empty ready form without loading the parent Customer', () => {
  ...
  expect(screen.getByRole('link', { name: 'Back to Customer' }).getAttribute('href')).toBe(
    `/en/contacts/customers/${customerId}`,
  );
...
test('decodes only bounded Customer UUIDs and builds the localized parent destination', () => {
  ...
  expect(customerDetailHref('cs', customerId)).toBe(`/cs/contacts/customers/${customerId}`);
});
...
test('keeps a non-writable target visible without an enabled mutation path', () => {
  ...
  expect(
    screen.getByText('Contacts is currently read-only. A Contact cannot be created right now.'),
  ).toBeTruthy();
...
test('submits the exact normalized payload through the generated client and navigates to the parent', async () => {
  ...
  await waitFor(() =>
    expect(navigateMock).toHaveBeenCalledWith({ to: `/en/contacts/customers/${customerId}` }),
  );
...
test('creates a new key after an uncertain intent changes', async () => {
  createContactMock.mockReturnValue(Effect.fail({ _tag: 'ContactsUnavailableProblem' }));
  renderFeature();
  const user = await fillForm();
...
test.each([
  ...
  ['ContactsAuthenticationProblem', 'Your session expired before the Contact could be created.'],
  ['ContactsForbiddenProblem', 'You do not have permission to create a Contact for this Customer.'],
  ['ContactsNotFoundProblem', 'This Customer no longer exists or is not available to you.'],
  ['ContactsConflictProblem', 'The Contact could not be created because the request conflicts'],
  ['ContactsInternalProblem', 'The Contact could not be created safely.'],
...
test('maps the complete typed client error union to a closed presentation vocabulary', () => {
  expect(classifyCreateContactError({ _tag: 'ContactsInvalidRequestProblem' })).toEqual({
    state: 'invalid_form',
  });
  ...
  expect(classifyCreateContactError({ _tag: 'ContactsForbiddenProblem' })).toEqual({
    state: 'forbidden',
  });
  expect(classifyCreateContactError({ _tag: 'ContactsNotFoundProblem' })).toEqual({
    state: 'not_found',
  });
  expect(classifyCreateContactError({ _tag: 'ContactsPreconditionRequiredProblem' })).toEqual({
    state: 'conflict',
  });
  expect(classifyCreateContactError({ _tag: 'ContactsConflictProblem' })).toEqual({
    state: 'conflict',
  });
  ...
  for (const tag of ['ContactsUnavailableProblem', 'GatewayUnavailableProblem'] as const) {
  ...
  for (const tag of ['ContactsInternalProblem', 'GatewayInternalProblem'] as const) {
...
test('Back and Cancel use the localized parent route without invoking the mutation', async () => {
  ...
  expect(screen.getByRole('link', { name: 'Zpět k zákazníkovi' }).getAttribute('href')).toBe(
    `/cs/contacts/customers/${customerId}`,
  );
  await user.click(screen.getByRole('button', { name: 'Zrušit' }));
  expect(navigateMock).toHaveBeenCalledWith({ to: `/cs/contacts/customers/${customerId}` });
  expect(createContactMock).not.toHaveBeenCalled();
});
...
describe('generated boundaries and localization', () => {
  test('keeps the form private and the page behind exact owner/Shell wiring', () => {
    const pageSource = readFileSync(
      new URL(
        '../../src/routes/[lang]/contacts/customers/[id]/contacts/new/page.tsx',
        import.meta.url,
      ),
      'utf-8',
    );
    ...
    expect(manifest).toContain("contributionKey: 'contacts.core.page.contact-create'");
    expect(manifest).toContain("routePath: '/contacts/customers/:id/contacts/new'");
    expect(manifest).not.toContain('contacts.core.navigation.contact-create');
    ...
    expect(formSource).not.toMatch(/contacts-client|routeParams|customerId|target\.writable/u);
  });

  test('keeps English and Czech ContactCreate locale structures in parity', () => {
    expect(flattenKeys(csCatalog.contacts.pages.contactCreate)).toEqual(
      flattenKeys(enCatalog.contacts.pages.contactCreate),
    );
  });
});
