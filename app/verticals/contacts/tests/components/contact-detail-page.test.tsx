...
import { getContactsQueryClient } from '../../src/contacts-query-client.ts';
import {
  ContactDetailPage,
...
} from '../../src/routes/[lang]/contacts/customers/[id]/contacts/[contactId]/page.tsx';
...
const getDescriptionList = (): HTMLElement => {
  ...
  if (list === null) {
    throw new Error('Expected Contact details description list');
  }
  return list;
};
...
describe('Contact detail route input', () => {
  test('accepts only bounded Contacts UUIDs and builds a hierarchical query key', () => {
...
test('loads one Contact once through the typed Contacts client with only the Contact ID', async () => {
...
test('renders the parent link, heading, semantic overview, communication links, lifecycle, and times', async () => {
  ...
  expect(heading.className).toContain('contacts:break-words');
  expect(screen.getByRole('link', { name: 'Back' }).getAttribute('href')).toBe(
    `/en/contacts/customers/${customerId}`,
  );
  expect(screen.getByRole('link', { name: 'Edit' }).getAttribute('href')).toBe(
    `/en/contacts/customers/${customerId}/contacts/${contactId}/edit`,
  );
  const list = getDescriptionList();
  expect(within(list).getByText('Contact ID')).toBeTruthy();
...
test('renders Czech archived data and preserves the locale and Customer ID in the parent link', async () => {
  ...
  expect(screen.getByRole('link', { name: 'Upravit' }).getAttribute('href')).toBe(
    `/cs/contacts/customers/${customerId}/contacts/${contactId}/edit`,
  );
});
...
test('retries an unavailable request from the keyboard and restores focus after completion', async () => {
  getContactMock
    .mockReturnValueOnce(Effect.fail({ _tag: 'ContactDetailUnavailableProblem' }))
...
test('maps the complete remaining client failure families without exposing raw errors', () => {
  expect(classifyContactDetailError({ _tag: 'ContactDetailInvalidProblem' })).toEqual({
    state: 'not_found',
  });
  expect(classifyContactDetailError({ _tag: 'GatewayForbiddenProblem' })).toEqual({
    state: 'forbidden',
  });
  expect(classifyContactDetailError({ _tag: 'GatewayAuthenticationRequiredProblem' })).toEqual({
    state: 'authentication_expired',
  });
  expect(classifyContactDetailError({ _tag: 'SchemaError' })).toEqual({
    reason: 'decode',
    state: 'unavailable',
  });
  expect(
  ...
  for (const tag of [
    ...
    expect(classifyContactDetailError({ _tag: tag })).toEqual({
      reason: 'backend',
      state: 'unavailable',
    });
  }
...
test('keeps locale parity, long-value wrapping, and the generated frontend seam', () => {
  expect(flattenKeys(csCatalog.contacts.pages.contactDetail)).toEqual(
    flattenKeys(enCatalog.contacts.pages.contactDetail),
  );
  const source = readFileSync(
    new URL(
      '../../src/routes/[lang]/contacts/customers/[id]/contacts/[contactId]/page.tsx',
      import.meta.url,
    ),
    'utf-8',
  );
  expect(source).toContain("from '../../../../../../../api/contacts-client.ts'");
  expect(source).toContain('contacts:break-words');
  expect(source).toContain('contacts:break-all');
  expect(source).toContain('contacts:block');
