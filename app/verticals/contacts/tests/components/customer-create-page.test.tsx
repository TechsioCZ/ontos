...
} from '../../src/routes/[lang]/contacts/customers/[id]/new/page.tsx';
...
test('emits no lookup before valid loader intent and calls the composed Contacts client exactly', async () => {
  const user = userEvent.setup();
  renderFeature();
...
test('routes ARES lookup through the composed Contacts client instead of decoding the page response', async () => {
...
test('keeps ARES-prefilled fields editable and creates only the final canonical flat payload', async () => {
  ...
  expect(navigateMock).toHaveBeenCalledWith({ to: '/en/contacts/customers' });
});
...
test('submits one normalized keyboard intent through the generated client without route context', async () => {
  ...
  await waitFor(() => expect(navigateMock).toHaveBeenCalledWith({ to: '/en/contacts/customers' }));
...
test('keeps lookup readable while a non-writable target disables only creation', async () => {
  ...
  expect(
    screen.getByText('Contacts is currently read-only. You cannot create a Customer.'),
  ).toBeTruthy();
  expect(screen.getByRole('textbox', { name: /^Customer name/u }).hasAttribute('disabled')).toBe(
    true,
  );
...
test('creates a new idempotency key after any Customer field changes an uncertain intent', async () => {
  createCustomerMock.mockReturnValue(Effect.fail({ _tag: 'ContactsUnavailableProblem' }));
  const user = userEvent.setup();
  renderFeature();
  const name = screen.getByRole('textbox', { name: /^Customer name/u });
...
test('creates a new idempotency key after a definite terminal failure', async () => {
  createCustomerMock.mockReturnValue(Effect.fail({ _tag: 'ContactsConflictProblem' }));
  const user = userEvent.setup();
  renderFeature();
  await user.type(screen.getByRole('textbox', { name: /^Customer name/u }), 'Acme');
...
test.each([
  ['ContactsInvalidRequestProblem', 'Enter a valid Customer name.'],
  ['ContactsAuthenticationProblem', 'Your session expired before the Customer could be created.'],
  ['ContactsForbiddenProblem', 'You do not have permission to create Customers.'],
  [
    'ContactsConflictProblem',
    'This request conflicts with a previous Customer creation attempt. Review the name and try again.',
  ],
  ['ContactsInternalProblem', 'The Customer could not be created safely. Try again.'],
] as const)(
...
test('maps the complete client failure families to a closed presentation vocabulary', () => {
  const cases = [
    [{ _tag: 'ContactsInvalidRequestProblem' }, { state: 'name_invalid' }],
    [{ _tag: 'ContactsAuthenticationProblem' }, { state: 'authentication_expired' }],
    [{ _tag: 'GatewayAuthenticationRequiredProblem' }, { state: 'authentication_expired' }],
    [{ _tag: 'ContactsForbiddenProblem' }, { state: 'forbidden' }],
    [{ _tag: 'GatewayForbiddenProblem' }, { state: 'forbidden' }],
    [{ _tag: 'ContactsConflictProblem' }, { state: 'conflict' }],
    [{ _tag: 'ContactsPreconditionRequiredProblem' }, { state: 'conflict' }],
    [
    ...
    [{ _tag: 'ContactsInternalProblem' }, { state: 'unexpected' }],
...
test('Back and Cancel use the localized Customers route without invoking a mutation', async () => {
  ...
  expect(back.getAttribute('href')).toBe('/cs/contacts/customers');
  expect(screen.getByRole('heading', { name: 'Vytvořit zákazníka' })).toBeTruthy();
  await user.click(screen.getByRole('button', { name: 'Zrušit' }));
  expect(navigateMock).toHaveBeenCalledWith({ to: '/cs/contacts/customers' });
  expect(createCustomerMock).not.toHaveBeenCalled();
});
...
describe('generated boundaries and localization', () => {
  test('keeps creation behind the generated frontend, owner, federation, and Shell seams', () => {
    const pageSource = readFileSync(
      new URL('../../src/routes/[lang]/contacts/customers/[id]/new/page.tsx', import.meta.url),
      'utf-8',
    );
    ...
    const shellConnector = readFileSync(
      new URL(
        '../../../../apps/shell-super-app/src/routes/[lang]/contacts/customers/[id]/new/page.data.ts',
        import.meta.url,
      ),
      'utf-8',
    );
    ...
    expect(manifest).toContain("contributionKey: 'contacts.core.page.customer-create'");
    expect(manifest).toContain("routePath: '/contacts/customers/:id/new'");
    expect(manifest).not.toContain('contacts.core.navigation.customer-create');
    expect(registration).toMatch(/'page-customer-create':\s*\(\) =>\s*import\(/u);
    ...
    expect(shellClient).toContain("componentKey: 'contacts.core.page-customer-create'");
    expect(shellConnector).toContain("const routeParameterNames = ['id'] as const");
    expect(shellConnector).toContain("entrypointKey: 'contacts.core.page.customer-create'");
  ...
  test('keeps English and Czech CustomerCreate locale structures in parity', () => {
    expect(flattenKeys(csCatalog.contacts.pages.customerCreate)).toEqual(
      flattenKeys(enCatalog.contacts.pages.customerCreate),
    );
  });
});
