...
} from '../../src/routes/[lang]/contacts/customers/[id]/edit/page.tsx';
...
test('keeps a read-only Customer readable without an enabled mutation path', async () => {
  ...
  expect(
    await screen.findByText(
      'Contacts is currently read-only. You can review this Customer, but cannot save changes.',
    ),
  ).toBeTruthy();
...
test('creates a new idempotency key after the user changes the failed intent', async () => {
  editCustomerMock.mockReturnValue(Effect.fail({ _tag: 'ContactsUnavailableProblem' }));
  const user = userEvent.setup();
  renderFeature();
  const name = await screen.findByRole('textbox', { name: /^Customer name/u });
...
test.each([
  ...
  ['ContactsAuthenticationProblem', 'Your session expired before the Customer could be saved.'],
  ['ContactsForbiddenProblem', 'You do not have permission to edit this Customer.'],
  ...
  ['ContactsInternalProblem', 'The Customer could not be saved safely. Try again.'],
...
test('maps duplicate IČO to a distinct safe and actionable warning', async () => {
  editCustomerMock.mockReturnValue(
    Effect.fail({ _tag: 'ContactsConflictProblem', code: 'contacts_customer_ico_conflict' }),
  );
  const user = userEvent.setup();
  renderFeature();
  await screen.findByRole('textbox', { name: /^Customer name/u });
...
test('maps all mutation failure families to a closed presentation vocabulary', () => {
  expect(classifyEditCustomerError({ _tag: 'ContactsInvalidRequestProblem' })).toEqual({
    state: 'invalid',
  });
  ...
  expect(
    classifyEditCustomerError({ _tag: 'ContactsConflictProblem', code: 'contacts_conflict' }),
  ).toEqual({
    state: 'conflict',
  });
  expect(
...
describe('generated boundaries and localization', () => {
  test('keeps the form private and the page behind generated owner/Shell wiring', () => {
    const pageSource = readFileSync(
      new URL('../../src/routes/[lang]/contacts/customers/[id]/edit/page.tsx', import.meta.url),
      'utf-8',
    );
    ...
    expect(manifest).toContain("contributionKey: 'contacts.core.page.customer-edit'");
    expect(manifest).toContain("routePath: '/contacts/customers/:id/edit'");
    expect(manifest).not.toContain('contacts.core.navigation.customer-edit');
    expect(registration).toMatch(/'page-customer-edit':\s*\(\) =>\s*import\(/u);
  ...
  test('keeps English and Czech CustomerEdit locale structures in parity', () => {
    expect(flattenKeys(csCatalog.contacts.pages.customerEdit)).toEqual(
      flattenKeys(enCatalog.contacts.pages.customerEdit),
    );
  });
});
