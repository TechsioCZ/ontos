...
import { ContactsPersistenceUnavailable } from '../../shared/apis/customer-detail.ts';
...
const principal = {
  ...
  authContextRef: 'better-auth-session:contacts-integration',
...
const otherPrincipal = {
  authBindingId: randomUUID(),
  authContextRef: 'better-auth-session:contacts-integration-other',
  authMethod: 'session' as const,
...
test('runs Contacts writes and reads through the governed runtimes with durable evidence', async () => {
  ...
  try {
    await adminPool.query(
      `insert into core.tenants (tenant_id, slug, name, status, default_locale)
       values ($1, $2, 'Contacts operations integration', 'active', 'en')`,
      [tenantId, `contacts-operations-${tenantId}`],
    );
    await adminPool.query(
      `insert into core.legal_entities
         (legal_entity_id, tenant_id, legal_name, registration_country, registration_number, status)
       values ($1, $2, 'Contacts operations entity', 'CZ', $3, 'active')`,
      [legalEntityId, tenantId, `contacts-operations-${legalEntityId}`],
    );
    await adminPool.query(
      `insert into core.principals (principal_id, tenant_id, kind, display_name, status)
       values ($1, $2, 'human', 'Contacts operations principal', 'active')`,
    ...
    await adminPool.query(
      ...
      [principal.authBindingId, tenantId, principalId, `contacts-operations-${principalId}`],
    );
    await adminPool.query(
      `insert into core.tenant_module_states (tenant_id, module_key, state)
       values ($1, 'contacts.core', 'active')`,
      [tenantId],
    );
    await adminPool.query(
      `insert into core.tenants (tenant_id, slug, name, status, default_locale)
       values ($1, $2, 'Contacts operations other tenant', 'active', 'en')`,
      [otherTenantId, `contacts-operations-${otherTenantId}`],
    );
    await adminPool.query(
      `insert into core.principals (principal_id, tenant_id, kind, display_name, status)
       values ($1, $2, 'human', 'Contacts operations other principal', 'active')`,
    ...
    await adminPool.query(
      ...
      [
        ...
        otherTenantId,
        otherPrincipalId,
        `contacts-operations-${otherPrincipalId}`,
      ],
    );
    await adminPool.query(
      `insert into core.tenant_module_states (tenant_id, module_key, state)
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* governedOperations() {
          ...
          const verifyReadEvidence = <Success, Failure, Requirements>(
            ...
            Effect.gen(function* verifyOneReadEvidence() {
              ...
              assert.deepEqual(row, {
                ...
                serving_module_key: 'contacts.core',
          ...
          assert.equal(duplicateCreate._tag, 'ContactsCustomerIcoConflict');
          ...
          const firstPage = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('contacts.core.api.customer-list.evidence.v1', 'list'),
          ...
          const detail = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('contacts.core.api.customer-detail.evidence.v1', 'read'),
              resultCount: () => 1,
            },
          );
          ...
          const contactPage = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('contacts.core.api.contact-list.evidence.v1', 'list'),
              resultCount: (result) => result.items.length,
            },
          );
          assert.deepEqual(
          ...
          assert.equal(repeatedContactArchive._tag, 'ContactsLifecycleConflict');
          ...
          assert.equal(duplicateArchivedCreate._tag, 'ContactsCustomerIcoConflict');
          ...
          assert.equal(repeatedCustomerArchive._tag, 'ContactsLifecycleConflict');
          const nonCascadedDetail = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('contacts.core.api.contact-detail.evidence.v1', 'read'),
              resultCount: () => 1,
            },
          );
          ...
          assert.ok(
            standalone.every(
              (row) =>
                ...
                row.evidencePolicyKey.startsWith('contacts.core.'),
            ),
          );
    ...
    for (const cleanupTenantId of [tenantId, otherTenantId]) {
      await adminPool.query('delete from contacts.contacts where tenant_id = $1', [
        cleanupTenantId,
      ]);
      await adminPool.query('delete from contacts.customers where tenant_id = $1', [
        cleanupTenantId,
      ]);
