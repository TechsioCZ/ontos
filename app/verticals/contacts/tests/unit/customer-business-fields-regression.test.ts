...
import { contactsE2eCustomers } from '../support/e2e-customers.ts';
...
test('keeps Czech/English Contacts copy and complete/null Customer fixtures structurally aligned', async () => {
  const [czech, english, fixtureSource] = await Promise.all([
    readJson(new URL('../../locales/cs/contacts.json', import.meta.url)),
    readJson(new URL('../../locales/en/contacts.json', import.meta.url)),
  ...
  assert.deepEqual(
    customerBusinessFields.map((field) => contactsE2eCustomers.active[field]),
    ['CZ00123456', '2026-08-17', '2020-01-02', '00123456', '112', 'E2E Alpha Customer'],
  );
  assert.deepEqual(
    customerBusinessFields.slice(0, -1).map((field) => contactsE2eCustomers.archived[field]),
    [null, null, null, null, null],
  );
  assert.match(contactsE2eCustomers.active.ico ?? '', /^0\d{7}$/u);
  assert.match(
    fixtureSource,
...
test('keeps browser Customer flows on generated Effect clients and ARES server-side', async () => {
  ...
  assert.equal(customerAresLookupRead.descriptor.readKey, 'contacts.core.api.customer-ares-lookup');
