...
test('publishes the exact Contacts contract-derived operation surface', async () => {
  assert.deepEqual(Object.keys(contactsOperationContexts).toSorted(), expectedOperations);
  assert.deepEqual(contactsApiContract, {
  ...
  const client = await readFile(
    new URL('../../src/api/contacts-client.ts', import.meta.url),
    'utf-8',
  );
...
test('uses status-matched, concrete Problem Details schemas', () => {
  const fixtures = [
    [ContactsInvalidRequestProblemSchema, 'ContactsInvalidRequestProblem', 400],
    [ContactsAuthenticationProblemSchema, 'ContactsAuthenticationProblem', 401],
    [ContactsNotFoundProblemSchema, 'ContactsNotFoundProblem', 404],
    [ContactsPreconditionRequiredProblemSchema, 'ContactsPreconditionRequiredProblem', 428],
    [ContactsUnavailableProblemSchema, 'ContactsUnavailableProblem', 503],
    [ContactsInternalProblemSchema, 'ContactsInternalProblem', 500],
  ] as const;
  for (const [schema, tag, status] of fixtures) {
    assertProblemSchemaStatus(Schema.decodeUnknownSync(schema), tag, status, status === 503);
  }
  for (const code of ['contacts_conflict', 'contacts_customer_ico_conflict'] as const) {
    const conflict = Schema.decodeUnknownSync(ContactsConflictProblemSchema)({
...
test('keeps Customer detail and list descriptors and generated clients stable', async () => {
  assert.deepEqual(
    ...
    },
    {
      ...
      entrypoint: {
        access: 'read',
        entrypointKey: 'contacts.core.api.customer-detail',
      ...
      evidencePolicy: {
        ...
        policyKey: 'contacts.core.api.customer-detail.evidence.v1',
      },
      legalEntityScope: 'optional',
      owningModuleKey: 'contacts.core',
      permissionTarget: 'tenant',
      policies: [],
      readKey: 'contacts.core.api.customer-detail',
      schemaVersion: '1',
    },
  );
  assert.deepEqual(
