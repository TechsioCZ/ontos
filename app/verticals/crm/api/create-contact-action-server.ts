...
import { CreateContactSchemaErrorMiddleware } from '../shared/apis/create-contact-action.ts';
import { crmApi } from '../shared/api.ts';
import {
  CreateContactPolicyDenialStatuses,
  createContactAction,
} from '../src/actions/create-contact.action.ts';
import type { CreateContactDomainError } from '../src/actions/create-contact.action.ts';
...
const validationProblem = () =>
  problem(
    'CreateContactValidationProblem' as const,
    400 as const,
...
const authenticationProblem = () =>
  problem(
    'CreateContactAuthenticationProblem' as const,
    401 as const,
...
const forbiddenProblem = () =>
  problem(
    'CreateContactForbiddenProblem' as const,
    403 as const,
    'Action forbidden',
...
const notFoundProblem = () =>
  problem(
    'CreateContactNotFoundProblem' as const,
    404 as const,
...
const conflictProblem = () =>
  problem(
    'CreateContactConflictProblem' as const,
    409 as const,
    'Action conflict',
...
const rejectedProblem = () =>
  problem(
    'CreateContactRejectedProblem' as const,
    422 as const,
    'Action rejected',
...
const preconditionProblem = () =>
  problem(
    'CreateContactPreconditionProblem' as const,
    428 as const,
...
const internalProblem = () =>
  problem(
    'CreateContactInternalProblem' as const,
    500 as const,
    'Action failed',
...
const unavailableProblem = () => ({
  ...problem(
    'CreateContactUnavailableProblem' as const,
    503 as const,
    'Action unavailable',
...
type ActionFailure = ActionCoreError | CreateContactDomainError;
...
const actionProblemByTag = {
  ...
  CreateContactConflict: () => conflictProblem(),
  CreateContactNotFound: () => notFoundProblem(),
  CreateContactRejected: () => rejectedProblem(),
  CreateContactUnavailable: () => unavailableProblem(),
...
const createContactActionSchemaErrorLive = HttpApiMiddleware.layerSchemaErrorTransform(
  CreateContactSchemaErrorMiddleware,
  () => Effect.fail(validationProblem()),
);

export const createContactActionApiLive = HttpApiBuilder.group(
  crmApi,
  'createContactActions',
  (handlers) =>
    handlers.handle('execute', ({ headers, payload }) =>
      Effect.gen(function* executeCreateContactAction() {
          ...
          .pipe(
            Effect.catchCause((cause) => {
              ...
              return Effect.logError(
                'Unexpected CreateContact Action transport defect',
                cause,
...
).pipe(Layer.provide(createContactActionSchemaErrorLive));
