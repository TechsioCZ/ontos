...
import { customerTimelineRead } from '../src/api/customer-timeline.read.ts';
import {
...
const authenticationProblem = () => ({
  _tag: 'CustomerTimelineAuthenticationProblem' as const,
...
const unavailableProblem = () => ({
  _tag: 'CustomerTimelineUnavailableProblem' as const,
...
const forbiddenProblem = () => ({
  _tag: 'CustomerTimelineForbiddenProblem' as const,
...
const notFoundProblem = () => ({
  _tag: 'CustomerTimelineNotFoundProblem' as const,
...
const policyProblem = (status: 409 | 422) =>
  status === 409
    ? {
        _tag: 'CustomerTimelinePolicyConflictProblem' as const,
    ...
    : {
        _tag: 'CustomerTimelinePolicyProblem' as const,
...
const internalProblem = () => ({
  _tag: 'CustomerTimelineInternalProblem' as const,
...
export const customerTimelineReadApiLive = HttpApiBuilder.group(
  crmApi,
  'customerTimeline',
  (handlers) =>
      ...
      Effect.gen(function* governedProviderRead() {
        ...
        return yield* runtime
          .runRead({
            input: payload,
            principal,
            registration: customerTimelineRead,
