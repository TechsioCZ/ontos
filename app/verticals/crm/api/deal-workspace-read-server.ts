...
import { Cause, Config } from 'effect';
import { DealWorkspaceSchemaErrorMiddleware } from '../shared/apis/deal-workspace.ts';
import { crmApi } from '../shared/api.ts';
import { dealWorkspaceDetailRead, dealWorkspaceListRead } from '../src/api/deal-workspace.read.ts';
import { verifyOperationPrincipal } from './auth/action-principal.ts';

const authenticationProblem = () => ({
  _tag: 'DealWorkspaceAuthenticationProblem' as const,
...
const validationProblem = () => ({
  _tag: 'DealWorkspaceValidationProblem' as const,
...
const unavailableProblem = () => ({
  _tag: 'DealWorkspaceUnavailableProblem' as const,
...
const forbiddenProblem = () => ({
  _tag: 'DealWorkspaceForbiddenProblem' as const,
...
const notFoundProblem = () => ({
  _tag: 'DealWorkspaceNotFoundProblem' as const,
...
const policyProblem = (status: 409 | 422) =>
  status === 409
    ? {
        _tag: 'DealWorkspacePolicyConflictProblem' as const,
    ...
    : {
        _tag: 'DealWorkspacePolicyProblem' as const,
...
const internalProblem = () => ({
  _tag: 'DealWorkspaceInternalProblem' as const,
...
const dealWorkspaceSchemaErrorLive = HttpApiMiddleware.layerSchemaErrorTransform(
  DealWorkspaceSchemaErrorMiddleware,
  () => Effect.fail(validationProblem()),
);

export const dealWorkspaceReadApiLive = HttpApiBuilder.group(
  crmApi,
  'dealWorkspaceReads',
  (handlers) =>
    handlers.handle('execute', ({ payload, request }) =>
      Effect.gen(function* governedProviderRead() {
        ...
        const transport = { correlationId: request.headers['x-correlation-id'] };
        ...
        const handleCause = (cause: Cause.Cause<ReadCoreError>) => {
          const failure = Cause.findErrorOption(cause);
          if (failure._tag === 'Some') {
            return handleFailure(failure.value);
          }
          return Effect.logError('Unexpected Deal workspace transport defect', cause).pipe(
            Effect.annotateLogs({ correlationId: transport.correlationId }),
            Effect.andThen(Effect.fail(internalProblem())),
          );
        };
        // eslint-disable-next-line default-case -- The decoded request is a closed discriminated union.
          ...
          case 'list': {
            return yield* runtime
              .runRead({
                input: payload,
                principal,
                registration: dealWorkspaceListRead,
                transport,
              })
              .pipe(Effect.catchCause(handleCause));
          }
          case 'detail': {
            return yield* runtime
              .runRead({
                input: payload,
                principal,
                registration: dealWorkspaceDetailRead,
                transport,
              })
              .pipe(Effect.catchCause(handleCause));
          }
...
).pipe(Layer.provide(dealWorkspaceSchemaErrorLive));
