CREATE TABLE "inventory"."reservation_confirmation_history" (
	...
	CONSTRAINT "inventory_reservation_confirmation_history_health_ck" CHECK ("health_state" in ('VALID', 'AT_RISK', 'REVOKED', 'EXPIRED', 'UNVERIFIABLE') and "revision" >= 1)
);
--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmation_history" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
CREATE TABLE "inventory"."reservation_confirmations" (
	...
	"current_health_state" text DEFAULT 'VALID' NOT NULL,
	"current_revision" integer DEFAULT 1 NOT NULL,
	"snapshot" jsonb NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "inventory_reservation_confirmations_meaning_ck" CHECK (char_length(btrim("attempt_id")) between 1 and 300 and char_length(btrim("issuer_backend_id")) between 1 and 300 and char_length(btrim("authority_effect_id")) between 1 and 300 and char_length(btrim("owner_evidence_ref")) between 1 and 300 and "issuer_backend_kind" in ('external_business_system', 'ontos_wms') and "issued_at" < "expires_at"),
	CONSTRAINT "inventory_reservation_confirmations_health_ck" CHECK ("current_health_state" in ('VALID', 'AT_RISK', 'REVOKED', 'EXPIRED', 'UNVERIFIABLE') and "current_revision" >= 1)
);
--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmations" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
CREATE UNIQUE INDEX "inventory_reservation_confirmation_history_scope_id_uk" ON "inventory"."reservation_confirmation_history" ("tenant_id","history_id");--> statement-breakpoint
CREATE UNIQUE INDEX "inventory_reservation_confirmation_history_revision_uk" ON "inventory"."reservation_confirmation_history" ("tenant_id","confirmation_id","revision");--> statement-breakpoint
CREATE UNIQUE INDEX "inventory_reservation_confirmations_scope_id_uk" ON "inventory"."reservation_confirmations" ("tenant_id","confirmation_id");--> statement-breakpoint
CREATE UNIQUE INDEX "inventory_reservation_confirmations_reservation_attempt_uk" ON "inventory"."reservation_confirmations" ("tenant_id","reservation_id","attempt_id");--> statement-breakpoint
CREATE UNIQUE INDEX "inventory_reservation_confirmations_authority_effect_uk" ON "inventory"."reservation_confirmations" ("tenant_id","authority_effect_id");--> statement-breakpoint
CREATE INDEX "inventory_reservation_confirmations_health_rank_idx" ON "inventory"."reservation_confirmations" ("tenant_id","current_health_state","issued_at","owner_evidence_ref");--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmation_history" ADD CONSTRAINT "inventory_reservation_confirmation_history_confirmation_fk" FOREIGN KEY ("tenant_id","confirmation_id") REFERENCES "inventory"."reservation_confirmations"("tenant_id","confirmation_id") ON DELETE RESTRICT;--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmations" ADD CONSTRAINT "inventory_reservation_confirmations_reservation_fk" FOREIGN KEY ("tenant_id","reservation_id") REFERENCES "inventory"."obligations"("tenant_id","obligation_id") ON DELETE RESTRICT;--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmations" ADD CONSTRAINT "inventory_reservation_confirmations_backend_configuration_fk" FOREIGN KEY ("tenant_id","owner_configuration_id") REFERENCES "inventory"."backend_configurations"("tenant_id","configuration_id") ON DELETE RESTRICT;--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmation_history_tenant_select" ON "inventory"."reservation_confirmation_history" AS PERMISSIVE FOR SELECT TO "ontos_runtime" USING ("inventory"."reservation_confirmation_history"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmation_history_tenant_insert" ON "inventory"."reservation_confirmation_history" AS PERMISSIVE FOR INSERT TO "ontos_runtime" WITH CHECK ("inventory"."reservation_confirmation_history"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmation_history_tenant_update" ON "inventory"."reservation_confirmation_history" AS PERMISSIVE FOR UPDATE TO "ontos_runtime" USING ("inventory"."reservation_confirmation_history"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid) WITH CHECK ("inventory"."reservation_confirmation_history"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmation_history_tenant_delete" ON "inventory"."reservation_confirmation_history" AS PERMISSIVE FOR DELETE TO "ontos_runtime" USING ("inventory"."reservation_confirmation_history"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmations_tenant_select" ON "inventory"."reservation_confirmations" AS PERMISSIVE FOR SELECT TO "ontos_runtime" USING ("inventory"."reservation_confirmations"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmations_tenant_insert" ON "inventory"."reservation_confirmations" AS PERMISSIVE FOR INSERT TO "ontos_runtime" WITH CHECK ("inventory"."reservation_confirmations"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmations_tenant_update" ON "inventory"."reservation_confirmations" AS PERMISSIVE FOR UPDATE TO "ontos_runtime" USING ("inventory"."reservation_confirmations"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid) WITH CHECK ("inventory"."reservation_confirmations"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);--> statement-breakpoint
CREATE POLICY "inventory_reservation_confirmations_tenant_delete" ON "inventory"."reservation_confirmations" AS PERMISSIVE FOR DELETE TO "ontos_runtime" USING ("inventory"."reservation_confirmations"."tenant_id" = nullif(current_setting('ontos.tenant_id', true), '')::uuid);
--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmation_history" FORCE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE "inventory"."reservation_confirmations" FORCE ROW LEVEL SECURITY;
--> statement-breakpoint
CREATE FUNCTION "inventory"."enforce_reservation_confirmation_scope"() RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ...
      MESSAGE = 'Reservation Confirmation must reference one exact runtime Attempt and provisional Reservation';
  END IF;
...
$$;
--> statement-breakpoint
CREATE FUNCTION "inventory"."enforce_reservation_confirmation_lifecycle"() RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, inventory
AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      CONSTRAINT = 'inventory_reservation_confirmations_lifecycle_ck',
      MESSAGE = 'Reservation Confirmation evidence cannot be deleted';
  END IF;
  ...
  IF NEW.confirmation_id IS DISTINCT FROM OLD.confirmation_id
    ...
    OR OLD.current_health_state IN ('REVOKED', 'EXPIRED')
    OR NEW.current_revision IS DISTINCT FROM OLD.current_revision + 1
    OR (NEW.snapshot #>> '{health,observation,effectiveAt}')::timestamptz
      < (OLD.snapshot #>> '{health,observation,effectiveAt}')::timestamptz
    OR (
      NEW.current_health_state = 'VALID'
      AND OLD.current_health_state NOT IN ('AT_RISK', 'UNVERIFIABLE')
    )
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      CONSTRAINT = 'inventory_reservation_confirmations_lifecycle_ck',
      MESSAGE = 'Reservation Confirmation transition mutates immutable proof or violates monotonic health lifecycle';
  END IF;
...
$$;
--> statement-breakpoint
CREATE FUNCTION "inventory"."enforce_reservation_confirmation_history_scope"() RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, inventory
AS $$
BEGIN
  ...
  WHERE confirmation.tenant_id = NEW.tenant_id
    ...
    AND confirmation.current_revision = NEW.revision
    AND confirmation.current_health_state = NEW.health_state
    AND confirmation.snapshot IS NOT DISTINCT FROM NEW.snapshot
    AND NEW.snapshot #>> '{ref,tenantId}' = NEW.tenant_id::text
    AND NEW.snapshot #>> '{ref,resourceId}' = NEW.confirmation_id::text
    AND (NEW.snapshot ->> 'revision')::integer = NEW.revision
    AND NEW.snapshot #>> '{health,state}' = NEW.health_state
    AND (NEW.snapshot #>> '{health,observation,effectiveAt}')::timestamptz = NEW.transitioned_at
  FOR KEY SHARE;

  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      CONSTRAINT = 'inventory_reservation_confirmation_history_snapshot_ck',
      MESSAGE = 'Reservation Confirmation history must exactly snapshot the current validated revision';
  END IF;
...
$$;
--> statement-breakpoint
CREATE FUNCTION "inventory"."reject_reservation_confirmation_history_mutation"() RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, inventory
AS $$
BEGIN
  RAISE EXCEPTION USING
    ERRCODE = '23514',
    CONSTRAINT = 'inventory_reservation_confirmation_history_append_only_ck',
    MESSAGE = 'Reservation Confirmation history is append-only';
END;
$$;
--> statement-breakpoint
CREATE TRIGGER "inventory_reservation_confirmations_scope_trg"
BEFORE INSERT OR UPDATE ON "inventory"."reservation_confirmations"
FOR EACH ROW EXECUTE FUNCTION "inventory"."enforce_reservation_confirmation_scope"();
--> statement-breakpoint
CREATE TRIGGER "inventory_reservation_confirmations_lifecycle_trg"
BEFORE UPDATE OR DELETE ON "inventory"."reservation_confirmations"
FOR EACH ROW EXECUTE FUNCTION "inventory"."enforce_reservation_confirmation_lifecycle"();
--> statement-breakpoint
CREATE TRIGGER "inventory_reservation_confirmation_history_scope_trg"
BEFORE INSERT ON "inventory"."reservation_confirmation_history"
FOR EACH ROW EXECUTE FUNCTION "inventory"."enforce_reservation_confirmation_history_scope"();
--> statement-breakpoint
CREATE TRIGGER "inventory_reservation_confirmation_history_no_update_trg"
BEFORE UPDATE ON "inventory"."reservation_confirmation_history"
FOR EACH ROW EXECUTE FUNCTION "inventory"."reject_reservation_confirmation_history_mutation"();
--> statement-breakpoint
CREATE TRIGGER "inventory_reservation_confirmation_history_no_delete_trg"
BEFORE DELETE ON "inventory"."reservation_confirmation_history"
FOR EACH ROW EXECUTE FUNCTION "inventory"."reject_reservation_confirmation_history_mutation"();
--> statement-breakpoint
REVOKE ALL ON FUNCTION "inventory"."enforce_reservation_confirmation_scope"() FROM PUBLIC, "ontos_runtime";
--> statement-breakpoint
REVOKE ALL ON FUNCTION "inventory"."enforce_reservation_confirmation_lifecycle"() FROM PUBLIC, "ontos_runtime";
--> statement-breakpoint
REVOKE ALL ON FUNCTION "inventory"."enforce_reservation_confirmation_history_scope"() FROM PUBLIC, "ontos_runtime";
--> statement-breakpoint
REVOKE ALL ON FUNCTION "inventory"."reject_reservation_confirmation_history_mutation"() FROM PUBLIC, "ontos_runtime";
