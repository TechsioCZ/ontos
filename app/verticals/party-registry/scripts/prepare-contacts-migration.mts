...
const requiredConnectionStringSchema = Schema.Trim.pipe(
...
const databaseFailure = (
  message: string,
  cause: unknown
): ContactsMigrationError => new ContactsMigrationError({ cause, message });
...
const query = <Row extends QueryResultRow = QueryResultRow>(
  ...
  const executeQuery: (queryText: string) => Promise<QueryResult<Row>> =
    client.query.bind(client);
  return Effect.tryPromise({
    catch: (cause) =>
      databaseFailure(`PostgreSQL query failed: ${text}`, cause),
    ...
    Effect.timeoutOrElse({
      ...
      orElse: () =>
        Effect.fail(
          databaseFailure(`PostgreSQL query timed out: ${text}`, 'timeout')
        ),
    })
  );
};
...
const connect = Effect.fn('ContactsMigration.connect')(function* connectEffect(
  ...
  const client = yield* Effect.try({
      ...
      databaseFailure('Unable to create the PostgreSQL client', cause),
    try: () =>
      new Client({ connectionString: Redacted.value(connectionString) }),
  });
  yield* Effect.tryPromise({
    ...
    Effect.timeoutOrElse({
      ...
      orElse: () =>
        Effect.fail(
          databaseFailure('PostgreSQL connection attempt timed out', 'timeout')
        ),
    })
  );
  return client;
...
const close = (client: Client) =>
  Effect.tryPromise({
      ...
      databaseFailure('Unable to close the PostgreSQL connection', cause),
    ...
    Effect.timeoutOrElse({
      ...
      orElse: () =>
        Effect.fail(
          databaseFailure('PostgreSQL connection close timed out', 'timeout')
        ),
    })
  );
    ...
    return yield* Effect.gen(function* prepareContactsTransactionEffect() {
      ...
      if (state === 'ambiguous') {
        return yield* new ContactsMigrationError({
          cause: state,
          message:
            'Ambiguous Contacts migration state: both CRM and Contacts journals exist',
        });
      }
      if (state === 'legacy') {
        yield* query(
        ...
        Effect.tap((state) =>
          Effect.sync(() => {
            process.stdout.write(
              `Contacts migration journal preparation: ${state}\n`
            );
          })
        )
      ),
...
if (
  ...
  const MainLayer = Layer.effectDiscard(main).pipe(
    Layer.provide(RootConfigProvider)
  );
