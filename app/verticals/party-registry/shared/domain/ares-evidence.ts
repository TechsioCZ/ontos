...
export const AresSubjectLookupIcoSchema = Schema.Trim.check(
  Schema.isPattern(/^\d{8}$/u)
).pipe(Schema.brand('AresSubjectLookupIco'));
...
export const AresLegalFormCodeSchema = Schema.Trim.check(
  Schema.isPattern(/^\d{1,10}$/u)
);

export const AresRegisteredAddressSchema = Schema.Struct({
  buildingNumber: Schema.OptionFromNullOr(boundedText(30)),
  countryCode: Schema.OptionFromNullOr(
    Schema.Trim.check(Schema.isPattern(/^[A-Z]{2}$/u))
  ),
  ...
  postalCode: Schema.OptionFromNullOr(
    Schema.Trim.check(Schema.isPattern(/^\d{5}$/u))
  ),
...
export const AresSubjectEvidenceSchema = Schema.Struct({
  cacheAgeSeconds: Schema.Finite.check(
    Schema.isInt(),
