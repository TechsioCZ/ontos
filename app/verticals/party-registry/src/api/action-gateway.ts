...
import type { AddContactPointPayload, AddContactPointResult } from '../../shared/actions/add-contact-point.ts';
import type {
...
export type { AresCanonicalFactEvidence, AresCorrectionReviewHandoff } from '../../shared/domain/ares-application.ts';
...
export const { deriveAresCorrectionReviewHandoffs, prefillPartyCandidateFromAres } = AresApplication;
...
const validateSelection = (selection: AresApplySelection, partyRef: PartyRef | null) => {
  ...
  if (selection.idempotencyKey.trim().length === 0) {
    return Effect.fail(invalidSelection('Every selected Action requires its own stable idempotency key'));
  }
  if (partyRef === null) {
    ...
    const expectedFactKind = selection.fact === 'BUSINESS_NAME' ? 'DISPLAY_NAME' : 'OFFICIAL_IDENTIFIER';
    if (selection.payload.factKind !== expectedFactKind) {
      return Effect.fail(invalidSelection('Correction review must nominate the selected supported fact'));
    }
    ...
    return Effect.fail(invalidSelection('All selected facts must target the same tenant-qualified Party'));
  }
...
const validateRequest = (request: AresApplyRequest) => {
  if (!request.userConfirmed || request.correlationId.trim().length === 0) {
    return Effect.fail(invalidSelection('Explicit user confirmation and a correlation ID are required'));
  }
  ...
  return Effect.forEach(request.selections, (selection) => validateSelection(selection, request.partyRef), {
    concurrency: 1,
    discard: true,
  }).pipe(
    Effect.andThen(() =>
      Schema.decodeEffect(AresSubjectEvidenceSchema)(request.observation).pipe(Effect.mapError(invalidObservation)),
    ),
  );
};

const matchesObservation = (selection: ExecutableSelection, observation: AresSubjectEvidence): boolean => {
  ...
  return Match.value(selection).pipe(
    ...
    Match.when(
      ...
      (selected) =>
        selected.payload.identifier.identifierType === 'ICO' && selected.payload.identifier.value === subject.ico,
    ),
    Match.when(
...
const validateSelectedValues = (request: AresApplyRequest, observation: AresSubjectEvidence) =>
  request.selections.some(
    (selection) => selection.route !== 'PARTY_CORRECTION' && !matchesObservation(selection, observation),
  )
    ? Effect.fail(invalidSelection('Selected values do not match the confirmed observation'))
    : Effect.void;

const needsConfirmation = (application: AresEvidenceApplication, reasonCode: string): AresEvidenceApplication => ({
  ...application,
...
const observationIsStale = (
  ...
  const ageMillis = DateTime.toEpochMillis(decisionTime) - DateTime.toEpochMillis(observation.observedAt);
  const suppliedAgeMillis = DateTime.toEpochMillis(decisionTime) - DateTime.toEpochMillis(supplied.observedAt);
  return (
    ageMillis < 0 ||
    ageMillis > 300_000 ||
    suppliedAgeMillis < 0 ||
...
const correctionTargetChanged = (
  ...
  candidates.some(
    (candidate) =>
      candidate.fact === selection.fact && candidate.targetAssertionId !== selection.payload.targetAssertionId,
  );

const selectionRevisionChanged = (selection: ExecutableSelection, revision: number | null): boolean =>
  selection.route === 'PARTY_UPDATE' && selection.payload.expectedRevision !== revision;
...
const loadCanonicalSnapshot = Effect.fn('AresApply.loadCanonicalSnapshot')(function* loadCanonicalSnapshotEffect(
  request: AresApplyRequest,
  observation: AresSubjectEvidence,
  ...
  if (request.partyRef !== null) {
    ...
    if (!sameParty(party.partyRef, partyRef) || detail.resolution.kind === 'ALIAS') {
      return yield* invalidSelection('Choose the canonical Party explicitly before applying ARES evidence');
    }
    ...
    const canonical: AresCanonicalSnapshot = {
      ...
      factEvidence: [
        ...detail.currentFactAssertions.flatMap((assertion) =>
          ...
          (Option.isNone(assertion.validTo) || DateTime.toEpochMillis(assertion.validTo.value) > decisionEpochMillis)
            ? [
                {
      ...
      identityAmbiguous: activeIdentifiers.some(({ identifierType, normalizedValue, verification }) => {
 ...
 */
export const applyAresObservationWithActions = Effect.fn('ActionGateway.applyAresObservationWithActions')(
  function* applyAresObservationWithActionsEffect<Failure>(
    ...
    const [suppliedInput, observationInput] = yield* Effect.all(
      ...
      Effect.mapError((error) => invalidSelection('ARES observation cannot be encoded for policy evaluation', error)),
    );
    ...
    const application = yield* Effect.try({
      catch: (error) => invalidSelection('The trusted ARES evidence cannot be evaluated under the owner policy', error),
      try: () =>
    ...
    const correctionCandidates =
      canonical === null ? [] : AresApplication.deriveAresCorrectionReviewHandoffs(application, canonical);
    ...
    type PartialCompletion = Extract<AresApplyOutcome<Failure>, { readonly _tag: 'AresApplyPartiallyCompleted' }>;
      ...
      (state, selection) => {
        ...
        return invokeSelection(selection, evidence, invoker, gateway, options.gatewayContext ?? {}, {
          ...
          Effect.map((attempt): ExecutionState => {
            ...
            return {
              completed: [...state.completed, attempt.success],
              partial: null,
            };
          }),
        );
