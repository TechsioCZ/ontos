...
export const counterpartyRoleHistoryPermissionTarget = (input: Parameters<typeof counterpartyPermissionTarget>[0]) =>
...
export const counterpartyRoleHistoryRead = defineRead(
  ...
  (
    ...
    context: ReadHandlerContext<{
      ...
      ) => Effect.Effect<LookupResult<CounterpartyRoleHistoryResponse['roles']>, CounterpartyPersistenceUnavailable>;
    }>,
  ) =>
  ...
  (transaction, scope) =>
    Effect.succeed({
      list: (counterpartyId: string) =>
        listCounterpartyRoleHistory(transaction, scope.tenantId, scope.legalEntityId, counterpartyId),
    }),
