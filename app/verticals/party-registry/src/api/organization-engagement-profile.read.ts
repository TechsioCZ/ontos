...
import { OperationContextUnavailable, defineRead, defineTenantModuleEntrypoint } from '@app/core-runtime';
...
const organizationProfileUnavailable = readUnavailable('Contacts engagement persistence is temporarily unavailable');
...
export const organizationEngagementProfileRead = defineRead(
  ...
  (transaction, scope) => {
    ...
    return Effect.succeed({
      find: (profileId) => findOrganizationEngagementProfile(transaction, scope.tenantId, profileId),
    });
  },
  (input) => ({
    kind: 'resource',
