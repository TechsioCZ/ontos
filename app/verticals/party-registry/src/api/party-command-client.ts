...
import { committedCreateResult, PartyCreateRecoveryUnavailable } from '../../shared/domain/matching-contracts.ts';
import { executePartyMatchDecisionWithAuthorization } from './party-match-decision-client.ts';

type CommandGroups = typeof partyRegistryCommandsApi extends HttpApi.HttpApi<infer _Id, infer Groups> ? Groups : never;
export type PartyCommandClient = HttpApiClient.Client<Extract<CommandGroups, HttpApiGroup.Constraint>, never, never>;
...
) => providePartyCommandRequestContext(partyCommandClient.pipe(Effect.flatMap(operation)), gatewayAssertion, options);
...
export const resolvePartyCommandCommitWithAuthorization = (
  ...
  providePartyCommandRequestContext(
    partyCommandRecoveryClient.pipe(Effect.flatMap((client) => client.partyCommandRecovery.resolve({ payload }))),
    credential,
    options,
  );
