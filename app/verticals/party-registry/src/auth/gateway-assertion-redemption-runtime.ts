...
export const makeGatewayAssertionRedemption = (
  ...
  consume: Effect.fn('PartyRegistryGatewayAssertionRedemption.consume')(
    function* consumeGatewayAssertionEffect(
      input: GatewayAssertionRedemptionInput
    ) {
      ...
      const expiredBefore = DateTime.toDateUtc(
        DateTime.makeUnsafe(
          nowEpochMs - GATEWAY_ASSERTION_CLOCK_SKEW_SECONDS * 1000
        )
      );
      const expiresAt = DateTime.toDateUtc(
        DateTime.makeUnsafe(input.expiresAtEpochSeconds * 1000)
      );
      const inserted = yield* redeemAssertion(
        executor,
        input,
        expiredBefore,
        expiresAt
        ...
        Effect.catchDefect((defect) =>
          isSqlError(defect)
            ? Effect.fail(unavailableError(defect))
            : Effect.die(defect)
        ),
