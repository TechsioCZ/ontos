...
import { Cache, Cause, Context, DateTime, Effect, Exit, Layer, Option, Schedule, Schema, Semaphore } from 'effect';
...
const ARES_SUBJECT_BASE_URL = 'https://ares.gov.cz/ekonomicke-subjekty-v-be/rest/ekonomicke-subjekty/';
...
export interface AresSubjectServiceContract {
  readonly subject: (input: AresSubjectLookup) => Effect.Effect<AresSubjectEvidence, AresSubjectError>;
}
...
const classifyStatus = (status: number): AresSubjectError => (statusFailures.get(status) ?? responseInvalid)();

const AresRetryableErrorSchema = Schema.Union([AresSubjectThrottled, AresSubjectTimeout, AresSubjectUnavailable]);
  ...
  return httpClient.execute(request).pipe(
    ...
    Effect.flatMap((response) =>
      response.status === 200
        ...
        : Effect.andThen(
            Effect.annotateLogs(Effect.logError('ARES subject request returned an upstream failure'), {
...
const makeAresSubjectService = Effect.gen(function* makeAresSubjectServiceEffect() {
  ...
  const loadSubject = (ico: string) =>
    concurrency.withPermit(
      requestSubject(httpClient, ico).pipe(Effect.retry({ schedule: retrySchedule, while: isRetryable })),
    );
