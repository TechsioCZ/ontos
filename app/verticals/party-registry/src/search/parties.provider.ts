...
const mapPartyProjectionHit = Effect.fn('PartiesProvider.mapPartyProjectionHit')(
  ...
  Object.freeze({
    searchCounterparties: (input: CounterpartySearchProjectionQuery) =>
      coreSearch
        ...
        .pipe(
          Effect.flatMap((hits) =>
            Effect.forEach(hits, mapCounterpartyProjectionHit, { concurrency: 1 }),
          ),
          Effect.mapError(projectionUnavailable),
        ),
      ...
      Effect.flatMap((hits) => {
        ...
        return Match.value(normalized).pipe(
          Match.tag('SearchResults', ({ items }) =>
            Effect.succeed(items satisfies PartiesProviderResponse),
          ),
