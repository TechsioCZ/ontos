...
export interface PartyRegistryReferenceOperations {
  readonly readCounterparty: (
    ...
    PartyRegistryCounterpartyProjection,
...
export const partyRegistryReferenceOperations = ({
  ...
  readCounterparty: (ref) =>
    ref.tenantId === tenantId
      ? findCounterpartyRecord(
          ...
          Effect.flatMap((result) =>
            Match.value(result).pipe(
              Match.tag('not_found', () =>
                Effect.fail(
                  mismatch('The Counterparty reference does not exist')
                )
              ),
      ...
      : Effect.fail(
          mismatch(
            'The Counterparty reference does not belong to the trusted tenant'
          )
        ),
  readParty: (ref) =>
    ref.tenantId === tenantId
                ...
                Match.value(result).pipe(
                  ...
                  Match.tag('found', ({ value }) =>
                    Effect.succeed({
                      ...
                      requestedPartyRef: makePartyRef(
                        tenantId,
                        resolution.requestedPartyId
                      ),
                    })
                  ),
                  Match.exhaustive
      ...
      : Effect.fail(
          mismatch('The Party reference does not belong to the trusted tenant')
        ),
});
...
export const validatePartyRegistryReferences = Effect.fn(
  ...
  if (
    ...
    return yield* new EngagementProfileConflict({
      code: 'contacts_party_counterparty_mismatch',
      reason:
        'The Party response does not resolve the supplied tenant reference',
    });
  }
  if (party.partyRef.resourceId !== refs.partyRef.resourceId) {
    return yield* new EngagementProfileConflict({
      ...
      reason:
        'New engagement profiles must target the canonical survivor Party reference',
    });
  }
  if (party.archived) {
    return yield* new EngagementProfileConflict({
  ...
  const expectedType =
    ...
    (options.expectedPartyType === 'PERSON' &&
      party.partyType === 'UNRESOLVED');
  if (!expectedType) {
    return yield* new EngagementProfileConflict({
