...
export interface GuestPartyResolutionPartyObservation {
  readonly archived: boolean;
  readonly partyId: string;
  readonly partyType: PartyType;
}

export interface GuestPartyResolutionCaseObservation {
 ...
 */
export const resolveGuestPartyEvidence = Effect.fn(
  'GuestPartyResolutionService.resolveGuestPartyEvidence',
  ...
  const partyIds = [
    ...
    ...caseRows.flatMap(({ selectedPartyId }) =>
      selectedPartyId === null ? [] : [selectedPartyId],
    ),
  ];
  ...
  const partyRows =
    uniquePartyIds.length === 0
      ...
      : yield* transaction
          ...
          .where(
            and(
              eq(parties.tenantId, input.legalEntityRef.tenantId),
              inArray(parties.partyId, uniquePartyIds),
            ),
          )
          .pipe(Effect.mapError(unavailable));
