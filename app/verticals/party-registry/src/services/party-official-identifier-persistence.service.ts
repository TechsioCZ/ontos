...
import { Data, DateTime, Effect, Match, Result, Schema } from 'effect';
...
const matchingIdentifierMetadata = (
  ...
  Result.getOrThrow(
    Schema.decodeUnknownResult(MatchingOfficialIdentifierMetadataSnapshotSchema)({
      state: row.state,
...
type MatchingOfficialIdentifierRow = typeof partyOfficialIdentifiers.$inferSelect;
type MatchingOfficialIdentifierAcceptanceValue = Data.TaggedEnum<{
  added: { readonly record: MatchingOfficialIdentifierRow };
  claim_conflict: { readonly conflict?: never };
  reused: { readonly record: MatchingOfficialIdentifierRow };
  updated: {
    readonly record: MatchingOfficialIdentifierRow;
    readonly update: MatchingOfficialIdentifierUpdate;
  };
}>;
export const MatchingOfficialIdentifierAcceptance = Data.taggedEnum<MatchingOfficialIdentifierAcceptanceValue>();
...
export const acceptMatchingOfficialIdentifierRecord = Effect.fn(
  ...
  if (existing === undefined) {
    ...
    return MatchingOfficialIdentifierAcceptance.added({ record });
  }
  ...
  if (existing.verificationState !== 'UNVERIFIED' || identifier.verification !== 'VERIFIED') {
    return MatchingOfficialIdentifierAcceptance.reused({ record: existing });
  }
  ...
  if (conflict) {
    return MatchingOfficialIdentifierAcceptance.claim_conflict({});
  }
  const now = yield* DateTime.nowAsDate;
  ...
  return MatchingOfficialIdentifierAcceptance.updated({
    record: updated,
    update: {
