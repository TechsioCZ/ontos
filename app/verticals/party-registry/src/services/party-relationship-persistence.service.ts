...
const exclusionViolationCodeSchema = Schema.String.check(
  Schema.makeFilter((code) =>
    code === exclusionViolationSqlState ? undefined : 'not an exclusion violation',
  ),
);
const directOverlapFailureSchema = Schema.Struct({
  code: exclusionViolationCodeSchema,
  constraint: Schema.Literal('party_relationships_no_overlap_excl'),
});
const makeOverlapFailureSchema = (): Schema.Schema<unknown> => {
  let nested: Schema.Schema<unknown> = directOverlapFailureSchema;
  for (let depth = 0; depth < 8; depth += 1) {
    nested = Schema.Union([
      directOverlapFailureSchema,
      Schema.Struct({
        cause: nested,
      }),
    ]);
...
const overlapFailureSchema = makeOverlapFailureSchema();

const isOverlapFailure = <Failure>(error: Failure): boolean =>
  Schema.is(overlapFailureSchema)(error);
