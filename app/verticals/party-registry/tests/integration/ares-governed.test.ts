  ...
  () =>
    Effect.gen(function* aresGovernedTestEffect() {
      ...
      const app = yield* Effect.acquireRelease(
        ...
        (resource) =>
          Effect.promise(resource.dispose.bind(resource)).pipe(Effect.orDie)
      );
      ...
      assert.isOk(
        Predicate.isTagged(applied, 'AresApplyCompleted'),
        appliedMessage
      );
      ...
      expect(identifierEvidence.providerChangedOn).toBe(
        encodedObservation.providerChangedOn
      );
      expect(identifierEvidence.providerRecordRef).toBe(
        encodedObservation.providerRecordRef
      );
      expect(contactEvidence.observedAt).toBe(encodedObservation.observedAt);
      expect(contactEvidence.providerChangedOn).toBe(
        encodedObservation.providerChangedOn
      );
      expect(contactEvidence.providerRecordRef).toBe(
        encodedObservation.providerRecordRef
      );
      expect(
      ...
      assert.isOk(
        persisted.core.invocations.every((item) => item.status === 'succeeded')
      );
      ...
      const denied = yield* runHttpEffect(
        applyAresObservation(request, { gateway: deniedGateway, baseUrl }).pipe(
          Effect.result
        )
      );
      assert.isOk(
        'failure' in denied &&
          Predicate.isTagged(denied.failure, 'AresLookupForbiddenProblem')
      );

      const afterDenied = yield* state();
      expect(afterDenied.core.invocations.length).toBe(
      ...
      expect(collisionOutcome.failed.recovery).toBe(
      ...
      expect(reviewOutcome.correctionCandidates[0]?.targetAssertionId).toBe(
        assertion.assertionId
      );
      const afterReview = yield* state();
      expect(afterReview.core.invocations.length).toBe(
      ...
      expect(Option.getOrUndefined(corrected.party.displayName)).toBe(
        rawSubject.obchodniJmeno
      );
