    ...
    Effect.scoped(
      Effect.gen(function* aresGovernedTestEffect() {
        ...
        const pool = yield* Effect.acquireRelease(
          ...
          (resource) =>
            promiseEffect(endPool.bind(undefined, resource)).pipe(Effect.orDie)
        );
        ...
        const publicJwk = yield* promiseEffect(
          exportJWK.bind(undefined, publicKey)
        );
        const jwk = { ...publicJwk, alg: 'EdDSA', kid, use: 'sig' };
        const encodedPublicJwks = yield* Schema.encodeEffect(
        ...
        const provider = HttpClient.make((request, url) => {
          ...
          return Effect.succeed(
            HttpClientResponse.fromWeb(request, Response.json(rawSubject))
          );
        });
        ...
        const app = yield* Effect.acquireRelease(
          ...
          (resource) =>
            promiseEffect(resource.dispose.bind(resource)).pipe(Effect.orDie)
        );
        const inMemoryFetch: typeof fetch = (input, init) =>
          app.handler(new Request(input, init), emptyRequestContext);
        const runHttpEffect = <Success, Failure>(
          effect: Effect.Effect<Success, Failure>
        ) =>
          effect.pipe(
            Effect.provideService(FetchHttpClient.Fetch, inMemoryFetch)
          );
        ...
        const create = Effect.fn('AresGovernedTest.create')(
              ...
              resolveDuplicateCandidateCreateWithAuthorization(
                {
                  ...
                  expectedRevision: 1,
                  reason:
                    'Reviewed concrete organization without a strong identifier',
                },
                yield* authorization(),
                options()
              )
        ...
        const lookup = Effect.fn('AresGovernedTest.lookup')(
        ...
        const detail = Effect.fn('AresGovernedTest.detail')(
          function* detailEffect(partyRef: PartyRef) {
            return yield* runHttpEffect(
        ...
        const replayRejected = yield* replayLookup().pipe(
          Effect.as(false),
          Effect.catchTag('AresLookupAuthenticationProblem', () =>
            Effect.succeed(true)
          )
        );
        ...
        const encodedObservation = yield* Schema.encodeEffect(
          AresSubjectEvidenceSchema
        )(observation);
        ...
        assert.ok(
          Schema.is(Schema.TaggedStruct('AresApplySelectionInvalid', {}))(
            unconfirmed.failure
          )
        );
        ...
        const applied = yield* runHttpEffect(
          applyAresObservation(request, { gateway, baseUrl })
        );
        const appliedMessage = yield* Schema.encodeEffect(
          Schema.fromJsonString(Schema.Unknown)
        )(applied);
        assert.ok(
        ...
        assert.equal(
          identifierEvidence.observedAt,
          encodedObservation.observedAt
        );
        assert.equal(identifierEvidence.servedAt, encodedObservation.servedAt);
        assert.equal(
          identifierEvidence.providerChangedOn,
          encodedObservation.providerChangedOn
        );
        assert.equal(
          identifierEvidence.providerRecordRef,
          encodedObservation.providerRecordRef
        );
        assert.equal(contactEvidence.observedAt, encodedObservation.observedAt);
        assert.equal(
          contactEvidence.providerChangedOn,
          encodedObservation.providerChangedOn
        );
        assert.equal(
          contactEvidence.providerRecordRef,
          encodedObservation.providerRecordRef
        );
        assert.equal(
          persisted.assertions.find((item) => item.factKind === 'DISPLAY_NAME')
            ?.externalEvidence?.decidedAt,
          encodedObservation.servedAt
        );
        assert.equal(persisted.core.events.length, 4);
        assert.equal(persisted.core.outbox.length, 4);
        assert.ok(
          persisted.core.invocations.every(
            (item) => item.status === 'succeeded'
          )
        );
        const replay = yield* runHttpEffect(
          applyAresObservation(request, { gateway, baseUrl })
        );
        assert.ok(
          Schema.is(Schema.TaggedStruct('AresApplyCompleted', {}))(replay)
        );
        assert.equal(replay.completed.length, 0);
        assert.equal(replay.skipped.length, 3);
        const afterReplay = yield* state();
        assert.equal(
          afterReplay.core.events.length,
          persisted.core.events.length
        );
        ...
        assert.ok(
          Schema.is(Schema.TaggedStruct('AresLookupForbiddenProblem', {}))(
            denied.failure
          )
        );
        const afterDenied = yield* state();
        assert.equal(
          afterDenied.core.invocations.length,
          persisted.core.invocations.length
        );
        ...
        const collisionOutcome = Match.value(collision).pipe(
          ...
          Match.orElse(() =>
            assert.fail(
              `Expected partial completion, received ${collision._tag}`
            )
          )
        );
        ...
        assert.equal(
          collisionOutcome.failed.recovery,
        ...
        const reviewOutcome = Match.value(review).pipe(
          ...
          Match.orElse(() =>
            assert.fail(`Expected deferred review, received ${review._tag}`)
          )
        );
        ...
        assert.equal(
          afterReview.core.invocations.length,
          beforeReview.core.invocations.length
        );
        yield* runHttpEffect(
          correctPartyFactWithAuthorization(
            correctionPayload,
        ...
        assert.equal(
          Option.getOrUndefined(corrected.party.displayName),
          rawSubject.obchodniJmeno
        );
