...
it.layer(NodeFileSystem.layer)('api-integration-command-contract', (suite) => {
  suite.effect(
    'every generated Action has its own statically named command endpoint',
    () =>
  ...
  suite.effect(
    ...
    () =>
      Effect.gen(function* decodeContract1() {
        expect(
          yield* Schema.decodeEffect(PartyCommandHeadersSchema)({})
        ).toEqual({});
        expect(() =>
  ...
  suite.effect(
    'alias conflict preserves both canonical and submitted references',
    () =>
      Effect.gen(function* decodeContract2() {
        ...
        expect(
          Schema.is(PartyCommandAliasWriteRejectedProblemSchema)(decodedProblem)
        ).toBe(true);
        expect(Struct.omit(decodedProblem, ['_tag'])).toEqual(
          Struct.omit(input, ['_tag'])
        );
      })
  );

  suite.effect(
    'public commands and clients never import Action runtime implementations',
    () =>
      Effect.gen(function* testProgram2() {
        const sources = yield* FileSystem.FileSystem.use((fs) =>
          Effect.forEach(
            ...
            (path) =>
              fs.readFileString(fileURLToPath(new URL(path, import.meta.url))),
