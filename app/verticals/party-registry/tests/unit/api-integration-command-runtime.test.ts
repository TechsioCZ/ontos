...
const mounted = (
  ...
  const redemptionLayer = Layer.succeed(
    GatewayAssertionRedemptionService,
    redemption
  );
  ...
  const loggerLayer =
    observedLogs === undefined
      ...
      : Logger.layer([
          Logger.make((options) => {
            observedLogs.push(
              JSON.stringify(Logger.formatStructured.log(options))
            );
          }),
        ]);
...
const recoveryRequest = (invocationId: string, token?: string) => {
  ...
  return new Request(
    'https://party.ontos.test/party-registry/action-commits/resolve',
    {
...
const commandRequest = (
  ...
  return new Request(
    `https://party.ontos.test/party-registry/actions/${slug}`,
    {
...
it.live(
  ...
  () =>
    Effect.gen(function* rejectInvalidMountedCommands() {
      ...
      expect(
        Object.keys(partyRegistryApi.groups.partyCommands.endpoints).length
      ).toBe(24);
      expect(
        Object.keys(partyRegistryApi.groups.partyCommands.endpoints).toSorted()
      ).toEqual([...endpointNames].toSorted());
      expect(
      ...
      yield* forEachSequential(
        ...
        (endpoint) =>
          Effect.gen(function* rejectInvalidEndpointRequest() {
            ...
            expect(response.status === 400 || response.status === 401).toBe(
              true
            );
      ...
      const malformed = yield* handle(
        app,
        new Request(
          'https://party.ontos.test/party-registry/actions/archive-party',
          {
            body: '{not-json',
            headers: {
              'content-type': 'application/json',
      ...
      expect(malformed.headers.get('content-type') ?? '').toMatch(
        /application\/problem\+json/u
      );
      const malformedBody = yield* Effect.promise(() => malformed.json());
      expect(
        Predicate.isTagged(malformedBody, 'PartyCommandInvalidRequestProblem')
      ).toBe(true);
    ...
    Effect.gen(function* rejectInvalidAssertions() {
      ...
      yield* forEachSequential(cases, ({ assertion, token }) =>
        Effect.gen(function* rejectInvalidAssertionCase() {
          ...
          const body = yield* Effect.promise(() => response.json());
          expect(
            Predicate.isTagged(body, 'PartyCommandAuthenticationProblem')
          ).toBe(true);
          expect(body.status).toBe(401);
          expect(
            (yield* Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown))(
              body
            )).includes(assertion.token)
          ).toBe(false);
          expect(harness.snapshot().invocations.length).toBe(0);
        })
      );
      ...
      yield* forEachSequential(
        ...
        (environment) =>
          Effect.gen(function* rejectInvalidConfigurationCase() {
            ...
            const body = yield* Effect.promise(() => response.json());
            expect(
              Predicate.isTagged(body, 'PartyCommandUnavailableProblem')
            ).toBe(true);
    ...
    Effect.gen(function* authenticateGovernedReads() {
      ...
      yield* forEachSequential([undefined, 'not-a-jwt'], (token) =>
        Effect.gen(function* rejectMissingGovernedCredentials() {
          const response = yield* handle(
            app,
            decisionRequest(randomUUID(), token)
          );
          expect(response.status).toBe(401);
          expect(response.headers.get('www-authenticate')).toBe('Bearer');
          expect(response.headers.get('content-type') ?? '').toMatch(
          ...
          expect(
            Schema.is(
              Schema.TaggedStruct('PartyMatchDecisionAuthenticationProblem', {})
            )(body)
          ).toBe(true);
          expect(reads).toBe(0);
        })
      ...
      expect(
        Schema.is(
          Schema.TaggedStruct('PartyMatchDecisionUnavailableProblem', {})
        )(body)
      ).toBe(true);
      expect(body.retryable).toBe(true);
      expect(reads).toBe(1);
...
it.live(
  ...
  () =>
    Effect.gen(function* mapEveryGovernedReadFailure() {
      ...
      yield* forEachSequential(
        cases,
        ([nextFailure, expectedStatus, expectedTag]) =>
...
it.live(
  'the generated governed Read seam sanitizes unexpected runtime defects',
  () =>
    Effect.gen(function* sanitizeGovernedReadDefect() {
      ...
      const body = yield* Effect.promise(() => response.json());
      expect(
        Schema.is(Schema.TaggedStruct('PartyMatchDecisionInternalProblem', {}))(
          body
        )
      ).toBe(true);
...
it.live(
  ...
  () =>
    Effect.gen(function* rejectAssertionReplays() {
      ...
      expect(actionAssertionReadReplay.headers.get('www-authenticate')).toBe(
        'Bearer'
      );
      expect(readCount()).toBe(0);
      const firstRead = yield* handle(
        app,
        decisionRequest(randomUUID(), assertion.otherToken)
      );
      expect(firstRead.status).toBe(404);
      expect(readCount()).toBe(1);
      const replayedRead = yield* handle(
        app,
        decisionRequest(randomUUID(), assertion.otherToken)
      );
      expect(replayedRead.status).toBe(401);
      expect(replayedRead.headers.get('www-authenticate')).toBe('Bearer');
      expect(replayedRead.headers.get('content-type') ?? '').toMatch(
        /application\/problem\+json/u
      );
      expect(readCount()).toBe(1);
    })
...
it.live(
  'correlation and idempotency are mandatory before the Core Action lifecycle',
  () =>
    Effect.gen(function* validateActionTransport() {
      ...
      expect(
        Schema.is(
          Schema.TaggedStruct('PartyCommandPreconditionRequiredProblem', {})
        )(missingKeyBody)
      ).toBe(true);
      ...
      expect(missingCorrelation.status).toBe(400);
      const missingCorrelationBody = yield* Effect.promise(() =>
        missingCorrelation.json()
      );
      expect(
...
it.live(
  ...
  () =>
    Effect.gen(function* preserveSafeActionTransport() {
      ...
      expect(snapshot.committed[0]?.actionKey).toBe(
...
it.live(
  'a decoded relationship timestamp reaches the Action runtime exactly once',
  () =>
    Effect.gen(function* decodeRelationshipTimestampOnce() {
      ...
      const body = yield* Effect.promise(() => response.json());
      expect(
        Predicate.isTagged(body, 'PartyCommandInvalidRequestProblem')
      ).toBe(true);
      expect(runtimeCalls).toBe(1);
    })
);
...
it.live(
  'an unexpected runtime defect is sanitized by the governed outer HTTP seam',
  () =>
    Effect.gen(function* sanitizeActionRuntimeDefect() {
      ...
      expect(response.status).toBe(500);
      const body = yield* Effect.promise(() => response.json());
      expect(Predicate.isTagged(body, 'PartyCommandInternalProblem')).toBe(
        true
      );
      expect(body.status).toBe(500);
      expect(
        (yield* Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown))(
          body
        )).includes('private governed runner defect')
      ).toBe(false);
      expect(harness.snapshot().invocations.length).toBe(0);
...
it.live(
  'the endpoint-owned mapper preserves representative Core failure semantics',
  () =>
    Effect.gen(function* mapRepresentativeActionFailures() {
      const assertion = yield* makeAssertion();
      const cases: readonly [
        failure: ActionCoreError,
        status: number,
        tag: string,
      ][] = [
        [
      ...
      yield* forEachSequential(
        cases,
        ([failure, expectedStatus, expectedTag]) =>
          Effect.gen(function* verifyActionFailureMapping() {
            ...
            const body = yield* Effect.promise(() => response.json());
            expect(Predicate.isTagged(body, expectedTag), failure.code).toBe(
              true
            );
          })
      );
...
it.live(
  'endpoint-local mappings keep declared not-found capability distinct over HTTP',
  () =>
    Effect.gen(function* preserveEndpointNotFoundCapabilities() {
      ...
      const actionRuntime: ActionRuntimeService = {
        resolveActionCommit: () =>
          Effect.die('commit recovery is outside the fixture'),
        runAction: () => Effect.fail(failure),
      };
      const app = yield* Effect.acquireRelease(
        Effect.sync(() =>
          mountedOrganizationEngagement(assertion.environment, actionRuntime)
        ),
        (resource) =>
          Effect.promise(() => resource.dispose()).pipe(Effect.orDie)
      );
      const profileRef = {
        moduleId: 'party.registry',
      ...
      expect(Predicate.isTagged(attachBody, 'ContactsInternalProblem')).toBe(
        true
      );
      ...
      expect(Predicate.isTagged(archiveBody, 'ContactsNotFoundProblem')).toBe(
        true
      );
    })
);
...
it.live(
  'real Core permission denial is a durable 403 and does not execute the command',
  () =>
    Effect.gen(function* persistPermissionDenial() {
      ...
      const body = yield* Effect.promise(() => response.json());
      expect(Predicate.isTagged(body, 'PartyCommandForbiddenProblem')).toBe(
        true
      );
      expect(harness.snapshot().invocations.length).toBe(1);
...
it.live(
  ...
  () =>
    Effect.gen(function* rollBackDomainConflict() {
      ...
      const body = yield* Effect.promise(() => response.json());
      expect(Predicate.isTagged(body, 'PartyCommandConflictProblem')).toBe(
        true
      );
...
it.live('alias conflicts preserve only safe canonical recovery metadata', () =>
  Effect.gen(function* preserveSafeAliasRecoveryMetadata() {
          ...
          transition: () =>
            Effect.fail(
              new PartyAliasWriteRejected({
                ...
                reason:
                  'Private diagnostic must never leave the owner boundary',
              })
            ),
        }),
      ],
    ...
    const body = yield* Effect.promise(() => response.json());
    expect(
      Predicate.isTagged(body, 'PartyCommandAliasWriteRejectedProblem')
    ).toBe(true);
...
it.live(
  'committed request replay stays a terminal 409 and does not execute or emit twice',
  () =>
    Effect.gen(function* rejectCommittedCommandReplay() {
      ...
      expect(
        Predicate.isTagged(body, 'PartyCommandAlreadyCommittedProblem')
      ).toBe(true);
      expect(body.code).toBe('action_already_committed');
      expect(body.invocationId).toBe(
...
it.live(
  'semantically insufficient Party evidence is a declared 422, not a server defect',
  () =>
              ...
              Effect.sync(() => {
                ...
                return {
                  ...
                  decisionRef: {
                    ...partyRef,
                    resourceType:
                      'party.registry.party-match-decision' as const,
                  },
                };
              }),
          }),
...
it.live(
  ...
  () =>
    Effect.gen(function* validateCommitResolutionRequest() {
      ...
      const malformed = yield* handle(
        app,
        recoveryRequest('not-an-id', assertion.token)
      );
      expect(malformed.status).toBe(400);
      const malformedBody = yield* Effect.promise(() => malformed.json());
      expect(
      ...
      const absent = yield* handle(
        app,
        recoveryRequest(randomUUID(), assertion.token)
      );
...
it.live(
  'an open invocation resolves explicitly without authorizing automatic command retry',
  () =>
...
it.live(
  ...
  () =>
    Effect.gen(
      function* recoverOriginalDecisionAfterLostCommitAcknowledgement() {
        const assertion = yield* makeAssertion();
        const decisions = new Map<
          string,
          typeof PartyMatchDecisionRecordSchema.Type
        >();
        let executions = 0;
        const harness = yield* makeActionTestHarness({
          actionPermission: 'allowed',
        ...
        const deniedRead = yield* handle(
          app,
          decisionRequest(invocationId, assertion.otherToken)
        );
        expect(deniedRead.status).toBe(403);
        const recovered = yield* handle(
          app,
          decisionRequest(invocationId, assertion.token)
        );
        expect(recovered.status).toBe(200);
        expect(yield* Effect.promise(() => recovered.json())).toEqual(
          decisions.get(invocationId)
        );
        const replay = yield* handle(
          app,
          commandRequest('create-party', createPayload, assertion.token, {
        ...
        expect(
          Predicate.isTagged(replayBody, 'PartyCommandAlreadyCommittedProblem')
        ).toBe(true);
