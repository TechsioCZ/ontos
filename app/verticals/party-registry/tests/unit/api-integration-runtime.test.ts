...
const makeAssertion = () =>
  Effect.gen(function* signRuntimeAssemblyAssertion() {
    ...
    return {
      issuer,
      publicJwks: yield* Schema.encodeEffect(
...
it.effect(
  ...
  () =>
    Effect.gen(function* apiIntegrationRuntimeCase1() {
      ...
      const readiness = yield* Schema.decodeUnknownEffect(
        partyRegistryReadinessSchema
...
it.live(
  ...
  () =>
    Effect.gen(function* verifyRuntimeAssemblyAndCors() {
              ...
              if (
                ...
                return service
                  ...
                  .pipe(
                    ...
                    Effect.andThen(
                      Effect.die(
                        'Party search substitute completed unexpectedly'
                      )
                    )
                  );
              }
      ...
      const unauthenticatedReadRequest = new Request(
        'http://localhost/reads/party-detail',
        {
      ...
      expect(unauthenticatedRead.headers.get('www-authenticate')).toBe(
        'Bearer'
      );
      ...
      ).toEqual([...partyRegistryCorsAllowedMethods].toSorted());
      expect(
        commaSeparatedHeader(
          preflight.headers.get('access-control-allow-headers')
        )
      ).toEqual([...partyRegistryCorsAllowedHeaders].toSorted());
      ...
      expect(foreignPreflight.headers.get('access-control-allow-origin')).toBe(
        null
      );
    })
);
