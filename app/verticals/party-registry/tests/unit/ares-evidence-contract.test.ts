...
test('normalizes only surrounding whitespace and preserves leading zeroes in an exact IČO', () => {
  assert.equal(
    Schema.decodeUnknownSync(AresSubjectLookupIcoSchema)(' 01234567 '),
    '01234567'
  );
  assert.deepEqual(
    Schema.decodeUnknownSync(AresLookupRequestSchema)({ ico: ' 01234567 ' }),
    {
      ico: '01234567',
    }
  );
...
test('returns one bounded evidence envelope and strips unowned provider payload fields', () => {
  ...
  assert.deepEqual(
    Schema.decodeUnknownSync(AresSubjectEvidenceSchema)(encoded),
    decoded
  );
});
...
test('allows ARES evidence to route only through standard Party-owned lifecycle Actions', () => {
  ...
  for (const route of routes) {
    assert.equal(
      Schema.decodeUnknownSync(AresCanonicalRouteSchema)(route),
      route
    );
  }
