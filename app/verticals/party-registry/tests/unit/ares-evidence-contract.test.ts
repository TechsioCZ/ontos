...
it.effect(
  'returns one bounded evidence envelope and strips unowned provider payload fields',
  () =>
    Effect.gen(function* validateContract2() {
      ...
      const encoded = yield* Schema.encodeEffect(AresLookupResponseSchema)(
        decoded
      );
...
it.effect(
  'keeps observed time separate from provider change time and cache-serving time',
  () =>
    Effect.gen(function* validateContract3() {
      ...
      const encoded = yield* Schema.encodeEffect(AresSubjectEvidenceSchema)(
        cached
      );
...
it.effect(
  'allows ARES evidence to route only through standard Party-owned lifecycle Actions',
  () =>
    Effect.gen(function* validateContract4() {
      ...
      for (const forbiddenRoute of [
        ...
        expect(() =>
          Schema.decodeUnknownSync(AresCanonicalRouteSchema)(forbiddenRoute)
        ).toThrow();
      }
...
it.effect(
  'rejects unattended enrichment and mutation routes on non-applying outcomes',
  () =>
