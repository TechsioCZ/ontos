...
it.effect(
  'maps a bounded ARES observation and sends an exact credential-free JSON request',
  () =>
    Effect.gen(function* aresSubjectServiceCase1() {
      ...
      expect(Option.getOrUndefined(result.subject.businessName)).toBe(
        'J.E.S., spol. s r.o.'
      );
      ...
      expect(Option.getOrUndefined(registeredAddress.municipality)).toBe(
        'Praha'
      );
      expect(
      ...
      expect(Option.getOrUndefined(result.providerRecordRef)).toBe(
        'provider-record-123'
      );
      ...
      yield* Effect.all(
        statusCases.map(([status, tag, expectedAttempts]) =>
          Effect.gen(function* aresSubjectServiceCase6() {
            ...
            const error = yield* expectedAttempts === 3
              ? fiberProgram
              : program;
            ...
            expect(
              ...
              )(error)).includes('PRIVATE_PROVIDER_CODE')
            ).toBe(false);
            expect(
              (yield* Schema.encodeEffect(
                Schema.fromJsonString(Schema.Unknown)
              )(error)).includes('private provider detail')
            ).toBe(false);
          })
        ),
        { concurrency: 'unbounded' }
...
it.effect(
  'retries transport faults with bounded backoff without exposing diagnostics',
  () =>
    Effect.gen(function* aresSubjectServiceCase7() {
      ...
      expect(
        (yield* Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown))(
          error
        )).includes('private socket diagnostic')
      ).toBe(false);
...
it.effect(
  'bounds stalled response bodies with the same three-attempt timeout policy',
  () =>
  ...
  () =>
    Effect.gen(function* aresSubjectServiceCase10() {
      const responses: readonly ((
        ...
        (request) =>
          jsonResponse(request, 200, { obchodniJmeno: 'missing IČO' }),
...
it.effect(
  'coalesces identical requests and exposes cache age without changing observedAt',
  () =>
    Effect.gen(function* aresSubjectServiceCase12() {
      ...
      const client = clientFrom((request) => {
        ...
        return Effect.sleep('1 second').pipe(
          Effect.andThen(
            Effect.succeed(jsonResponse(request, 200, rawSubject()))
          )
        );
      });
