...
it.effect(
  'keeps a future-ended role active until its exclusive effective end',
  () =>
...
it.effect(
  'records a retrospective end as historical without deleting the role period',
  () =>
...
it.effect(
  'reads end provenance independently from the role-add provenance',
  () =>
...
it.effect(
  'rejects an alias Party create target with canonical survivor guidance',
  () =>
...
it.effect(
  ...
  () =>
    Effect.gen(function* testScenario8() {
      ...
      expect(harness.selectedTables.includes('counterparty_role_periods')).toBe(
        false
      );
    })
);
...
it.effect(
  'creates the tenant-admin snapshot atomically without creating an implicit role',
  () =>
...
it.effect(
  'adds a future role and its admin history projection in the same transaction seam',
  () =>
