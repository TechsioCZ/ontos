...
it('applies CUSTOMER and SUPPLIER evidence thresholds without waiting for first completion', () => {
  expect(roleEvidenceIsSufficient('CUSTOMER', 'ENGAGEMENT_PROSPECT')).toBe(
    false
  );
  ...
  expect(roleEvidenceIsSufficient('SUPPLIER', 'BINDING_PURCHASE_ORDER')).toBe(
    true
  );
});

it('requires explicit relationship-end evidence and rejects operational inactivity', () => {
  expect(roleEndEvidenceIsSufficient('CUSTOMER', 'ENGAGEMENT_INACTIVITY')).toBe(
    false
  );
  expect(
  ...
  expect(roleEndEvidenceIsSufficient('SUPPLIER', 'PURCHASE_SUSPENSION')).toBe(
    false
  );
  expect(
    roleEndEvidenceIsSufficient(
