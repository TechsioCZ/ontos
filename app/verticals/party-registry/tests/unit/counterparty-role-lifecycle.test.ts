...
test('applies CUSTOMER and SUPPLIER evidence thresholds without waiting for first completion', () => {
  assert.equal(
    roleEvidenceIsSufficient('CUSTOMER', 'ENGAGEMENT_PROSPECT'),
    false
  );
  assert.equal(roleEvidenceIsSufficient('CUSTOMER', 'BINDING_ORDER'), true);
  assert.equal(
    roleEvidenceIsSufficient('CUSTOMER', 'APPROVED_PURCHASING_RELATIONSHIP'),
    true
  );
  assert.equal(roleEvidenceIsSufficient('SUPPLIER', 'VENDOR_CANDIDATE'), false);
  assert.equal(
    roleEvidenceIsSufficient('SUPPLIER', 'COMPLETED_VENDOR_ONBOARDING'),
    true
  );
  assert.equal(
    roleEvidenceIsSufficient('SUPPLIER', 'BINDING_PURCHASE_ORDER'),
    true
  );
});
...
test('requires explicit relationship-end evidence and rejects operational inactivity', () => {
  assert.equal(
    roleEndEvidenceIsSufficient('CUSTOMER', 'ENGAGEMENT_INACTIVITY'),
    false
  );
  assert.equal(
    roleEndEvidenceIsSufficient('CUSTOMER', 'TRANSACTION_INACTIVITY'),
    false
  );
  assert.equal(
    roleEndEvidenceIsSufficient(
  ...
  assert.equal(
    roleEndEvidenceIsSufficient('SUPPLIER', 'TEMPORARY_PROCUREMENT_BLOCK'),
    false
  );
  assert.equal(
    roleEndEvidenceIsSufficient('SUPPLIER', 'PURCHASE_SUSPENSION'),
    false
  );
  assert.equal(
    roleEndEvidenceIsSufficient(
...
test('derives current role state from lifecycle and effective time', () => {
  ...
  assert.equal(
    rolePeriodIsCurrentAt(active, '2025-12-31T23:59:59.000Z'),
    false
  );
  assert.equal(rolePeriodIsCurrentAt(active, '2026-06-01T00:00:00.000Z'), true);
  assert.equal(
    rolePeriodIsCurrentAt(active, '2027-01-01T00:00:00.000Z'),
    false
  );
  assert.equal(
    rolePeriodIsCurrentAt(
