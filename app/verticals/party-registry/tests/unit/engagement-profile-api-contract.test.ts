...
it.effect(
  'public engagement mutations preserve owner request context at the HTTP boundary',
  () =>
    Effect.gen(function* verifyCase3() {
      ...
      const fakeFetch: typeof fetch = (input, init) => {
        ...
        if (pathname === '/auth/gateway-context') {
          return Promise.resolve(
            Response.json({ expiresAt: 1, token: 'test-gateway-token' })
          );
        }
        expect(pathname).toBe(
          '/party-registry-api/contacts/engagement/organizations/attach'
        );
        return Promise.resolve(
          Response.json({
            archivedAt: null,
      ...
      const mutationRequest = requests.find(({ url }) =>
        url.includes('/contacts/engagement/')
      );
      const gatewayRequest = requests.find(({ url }) =>
        url.endsWith('/auth/gateway-context')
      );
      ...
      expect(mutationRequest?.headers.get('x-trace-id')).toBe(
        'engagement-trace'
      );
      expect(mutationRequest?.headers.get('traceparent')).toBe(traceparent);
      expect(mutationRequest?.headers.get('x-correlation-id')).toBe(
...
it.effect(
  'public Party Registry engagement API does not expose legacy identity operations',
  () =>
    Effect.gen(function* verifyCase4() {
      const [apiSource, clientSource] = yield* Effect.all([
        Effect.promise(() =>
          readFile(
            new URL('../../shared/engagement-profile-api.ts', import.meta.url),
            'utf-8'
          )
        ),
        Effect.promise(() =>
          readFile(
            new URL(
              '../../src/api/engagement-profile-client.ts',
              import.meta.url
            ),
            'utf-8'
          )
