...
it.effect(
  'Add reuses a current same-Party identifier instead of duplicating an assertion',
  () =>
      ...
      yield* Effect.all(
        (['ORGANIZATION', 'PERSON'] as const).map((partyType) =>
          Effect.gen(function* verifyCase3() {
            ...
            expect(db.inserts[0]?.values['acceptedByPrincipalId']).toBe(
              principalId
            );
            expect(
              db.inserts.filter(
                (entry) => entry.table === partyIdentifierClaims
              ).length
            ).toBe(partyType === 'ORGANIZATION' ? 1 : 0);
          })
        )
      );
    })
...
it.effect(
  'ending an identifier preserves its fact and releases its current claim',
  () =>
...
it.effect(
  'verification collision changes neither metadata nor claim ownership',
  () =>
    ...
    Effect.gen(function* verifyCase7() {
      ...
      const found = Match.value(result).pipe(
        ...
        Match.orElse(() =>
          (() => {
            throw new Error(
              'Expected the identifier verification update to succeed'
            );
          })()
        )
      );
...
it.effect(
  'PERSON verification cannot acquire an implicit strong identifier claim',
  () =>
    ...
    Effect.gen(function* verifyCase9() {
      ...
      const found = Match.value(result).pipe(
        ...
        Match.orElse(() =>
          (() => {
            throw new Error(
              'Expected the identifier verification downgrade to succeed'
            );
          })()
        )
      );
