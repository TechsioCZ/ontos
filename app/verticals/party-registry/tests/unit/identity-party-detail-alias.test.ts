...
const canonicalParty: Party = Schema.decodeSync(PartySchema)(canonicalPartyWire);
const alias = (aliasPartyId: string, canonicalPartyId: string): PartyAliasLookupRow => ({
  aliasPartyId,
  canonicalPartyId,
  tenantId,
});
const makeServices = (aliases: readonly PartyAliasLookupRow[], party: Party | null = canonicalParty) => {
...
it.effect('Party Detail reads the final canonical Party after the complete historical alias chain', () =>
  Effect.gen(function* verifyPartyDetail1() {
    const { lookups, services } = makeServices([alias('party-b', 'party-a'), alias('party-a', 'party-c')]);
...
it.effect('Party Detail preserves archived lifecycle independently of direct resolution metadata', () =>
...
it.effect('Party Detail fails closed for cycles and broken historical chains without reading an alias Party', () =>
  Effect.gen(function* verifyPartyDetail3() {
    for (const aliases of [[alias('party-a', 'party-b'), alias('party-b', 'party-a')], [alias('party-a', 'missing')]]) {
      const { lookups, services } = makeServices(aliases);
      const error = yield* Effect.flip(readPartyDetailFromServices(partyRef('party-a'), tenantId, services));
...
it.effect('Party Detail hides a missing direct Party and a cross-tenant requested reference', () =>
  Effect.gen(function* verifyPartyDetail4() {
    ...
    for (const requested of [partyRef('missing'), { ...partyRef('party-c'), tenantId: otherTenantId }]) {
