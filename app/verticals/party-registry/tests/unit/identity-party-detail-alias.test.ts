...
it.effect(
  ...
  () =>
    Effect.gen(function* verifyPartyDetail1() {
      ...
      const result = yield* readPartyDetailFromServices(
        partyRef('party-b'),
        tenantId,
        services
      );
      ...
      const encoded = yield* Schema.encodeEffect(PartyDetailResponseSchema)(
        result
      );
...
it.effect(
  ...
  () =>
    Effect.gen(function* verifyPartyDetail2() {
      ...
      expect(result.party.archivedAt).toEqual(
...
it.effect(
  'Party Detail hides a missing direct Party and a cross-tenant requested reference',
  () =>
    Effect.gen(function* verifyPartyDetail4() {
      ...
      for (const requested of [
        ...
        const error = yield* Effect.flip(
          readPartyDetailFromServices(requested, tenantId, services)
        );
