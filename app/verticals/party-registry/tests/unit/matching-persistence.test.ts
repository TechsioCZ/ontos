...
const harness = (queues: ReadonlyMap<unknown, readonly Rows[]> = new Map()) => {
  const results = new Map(
    [...queues].map(([table, rows]) => [table, [...rows]])
  );
...
it.layer(
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(function* durablePartyMatchCommitsAnAmbiguityDecision() {
        ...
        expect(result.candidateParties.map((ref) => ref.resourceId)).toEqual([
          partyA,
          partyB,
        ]);
        ...
        expect(recordedRow(subject.inserts[2]?.values)['candidateCaseId']).toBe(
          candidateCaseId
        );
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(function* anUnchangedOpenAmbiguityReusesItsImmutable() {
        ...
        expect(
          subject.updates.length,
          'the original case snapshot is never overwritten'
        ).toBe(0);
      })
  );
    ...
    () =>
      Effect.gen(function* personIOCannotAcquireOrganizationAuto() {
        ...
        const durable = recordedRow(
          subject.inserts.find(({ table }) => table === partyMatchDecisions)
            ?.values
        );
    ...
    () =>
      Effect.gen(function* unarchiveIntentBoundary() {
        for (const resolution of ['CREATE', 'MATCH']) {
          ...
          expect(Predicate.isTagged(error, 'DuplicateCandidateConflict')).toBe(
            true
          );
  ...
  const invokeReviewedMatch = (subject: ReturnType<typeof harness>) =>
    Effect.gen(function* invokeReviewedMatchEffect() {
      ...
      const result = yield* getActionHandler(
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(
        function* reviewedMatchingPublishesTheAcceptedIdentifierThrough() {
          const subject = harness(
            new Map<unknown, readonly Rows[]>([
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(function* matchedCreateReusingAnExistingIdentifierDoes() {
        ...
        expect(
          subject.inserts.some(
            ({ table }) => table === partyOfficialIdentifiers
          )
        ).toBe(false);
      })
  );
      ...
      Effect.gen(function* reviewedMatchingWithAlreadyOwnedClaimsCreates() {
        const subject = harness(
          new Map<unknown, readonly Rows[]>([
            ...
            [
              partyIdentifierClaims,
              [[{ officialIdentifierId, partyId: partyC }]],
            ],
          ])
        );
        const { collector, result } = yield* invokeReviewedMatch(subject);
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(function* reviewedMatchingLocksAndRejectsAnArchived() {
        ...
        expect(Predicate.isTagged(failure, 'DuplicateCandidateConflict')).toBe(
          true
        );
        expect(
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(function* futureEffectiveEvidenceIsRejectedBeforeA() {
        ...
        expect(Predicate.isTagged(failure, 'PartyEvidenceInsufficient')).toBe(
          true
        );
        expect(subject.inserts).toEqual([]);
      })
  ...
  testIt.effect(
    ...
    () =>
      Effect.gen(function* weakExactCanonicalEvidenceProducesReviewRather() {
        ...
        expect(subject.inserts.some(({ table }) => table === parties)).toBe(
          false
        );
      })
  );
    ...
    () =>
      Effect.gen(function* initialNoStrongCreateReviewCapturesRelevant() {
        ...
        const links = recordedRows(
          subject.inserts.find(
            ({ table }) => table === duplicateCandidateCaseParties
          )?.values
        );
    ...
    () =>
      Effect.gen(function* aNewMaterialEvaluationCreatesALinked() {
        ...
        const insertedCase = recordedRow(
          subject.inserts.find(({ table }) => table === duplicateCandidateCases)
            ?.values
        );
        expect(insertedCase['priorCandidateCaseId']).toBe(priorId);
        expect(String(insertedCase['evaluationFingerprint'])).toMatch(
          /^[0-9a-f]{64}$/u
        );
        expect(
      ...
      Effect.forEach(
        ...
        (priorCaseTenantId) =>
          Effect.gen(function* rejectInvalidPriorCaseReference() {
            ...
            expect(
              Predicate.isTagged(failure, 'PartyEvidenceInsufficient')
            ).toBe(true);
            expect(subject.inserts).toEqual([]);
          }),
  ...
  it('equivalent Candidate property and evidence ordering has one deterministic fingerprint', () => {
    ...
    expect(candidateFingerprint(original)).toBe(
      candidateFingerprint(reordered)
    );
  });
