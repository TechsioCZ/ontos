...
const lookup = (
  overrides: Partial<PartyAliasLookup> = {}
): PartyAliasLookup => ({
    ...
    Effect.succeed(
      Option.fromNullishOr(
        new Map([
          ...
          [
            'party-a',
            { aliasPartyId: 'party-a', canonicalPartyId: 'party-c', tenantId },
          ],
        ]).get(aliasPartyId)
      )
    ),
  partyExists: (_requestedTenantId, partyId) =>
    Effect.succeed(partyId === 'party-c'),
  ...overrides,
});

it.effect(
  'central resolution service walks the complete canonical alias chain in one scoped transaction seam',
  () =>
...
it.effect(
  'central resolution fails closed for cycles, cross-tenant targets, and broken chains',
  () =>
    Effect.gen(function* aliasResolution2() {
      ...
      const cycleError = yield* Effect.flip(
        cycle.resolvePartyAlias(tenantId, 'party-a')
      );
      expect(Predicate.isTagged(cycleError, 'PartyAliasResolutionCycle')).toBe(
        true
      );
      ...
      const crossTenantError = yield* Effect.flip(
        crossTenant.resolvePartyAlias(tenantId, 'party-b')
      );
      expect(
        Predicate.isTagged(crossTenantError, 'PartyAliasResolutionCrossTenant')
      ).toBe(true);
      ...
      const brokenError = yield* Effect.flip(
        broken.resolvePartyAlias(tenantId, 'missing')
      );
      expect(
        Predicate.isTagged(brokenError, 'PartyAliasResolutionBrokenChain')
      ).toBe(true);
    })
);

it.effect(
  'central write guard returns typed canonical-survivor guidance and never forwards',
  () =>
    Effect.gen(function* aliasResolution3() {
      const service = makePartyAliasResolutionService(lookup());
      const rejection = yield* Effect.flip(
        service.requireCanonicalWriteTarget(tenantId, 'party-b')
      );
