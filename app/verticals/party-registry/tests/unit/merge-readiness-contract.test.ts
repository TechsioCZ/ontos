...
it.layer(NodeFileSystem.layer)('merge-readiness-contract', (suite) => {
  suite.effect(
    ...
    () =>
      Effect.gen(function* schemaContract1() {
        ...
        expect(
          Predicate.isTagged(rejection, 'ProductionMergeExecutionRejected')
        ).toBe(true);
      ...
      Effect.gen(function* schemaContract2() {
        ...
        expect(() =>
          Schema.decodeUnknownSync(PartyMergeSchema)({
            ...merge,
            selectionEvidenceChain: merge.selectionEvidenceChain.map(
              (step) => ({
                ...step,
        ...
        expect(partyMergeResourceDescriptor.capabilities.searchable).toBe(
          false
        );
        expect(partyAliasResourceDescriptor.capabilities.searchable).toBe(
          false
        );
      })
  );
  ...
  suite.effect(
    'has no registered Party Merge Action, event, outbox consumer, or write endpoint',
    () =>
      Effect.map(
        Effect.all([
          FileSystem.FileSystem.use((fs) =>
            fs.readFileString(
              fileURLToPath(
                new URL('../../vertical.manifest.ts', import.meta.url)
              )
            )
          ),
          FileSystem.FileSystem.use((fs) =>
        ...
        ([manifestSource, registrationSource]) => {
          ...
          const endpoints = Object.values(partyRegistryApi.groups).flatMap(
            (group) => Object.values(group.endpoints)
          );
          expect(
            Object.keys(partyRegistryApi.groups.partyCommands.endpoints)
              .length > 0
          ).toBe(true);
          expect(
            endpoints
              ...
              .map(({ path }) => path)
          ).toEqual(['/reads/party-merge-readiness']);
        }
      )
  );

  suite.effect(
    'serves the generated OntOS module contract before i18n redirects in development',
    () =>
      Effect.map(
        FileSystem.FileSystem.use((fs) =>
          fs.readFileString(
            fileURLToPath(new URL('../../modern.config.ts', import.meta.url))
          )
        ),
        (modernConfigSource) => {
  ...
  suite.effect(
    'readiness response schema cannot claim production merge is enabled',
    () =>
