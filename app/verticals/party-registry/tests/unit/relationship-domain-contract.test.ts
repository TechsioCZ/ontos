...
test('create accepts one provenance-backed PERSON to ORGANIZATION period shape', () => {
  ...
  assert.equal(
    DateTime.formatIso(Option.getOrThrow(decoded.validFrom)),
    payload.validFrom
  );
  assert.throws(() =>
    Schema.decodeUnknownSync(CreatePartyRelationshipPayloadSchema)({
...
test('relationship timestamps and nullable periods preserve their JSON encoding', () => {
  ...
  assert.deepEqual(
    Schema.encodeSync(CreatePartyRelationshipPayloadSchema)(decoded),
    {
      ...wire,
      validTo: null,
    }
  ...
  assert.deepEqual(
    Schema.encodeSync(UpdatePartyRelationshipPayloadSchema)(updated),
    {
      changeReason: 'Clarified end',
      expectedRevision: 1,
      provenance,
...
test('end retry is exact and changed historical evidence requires correction', () => {
  ...
  assert.deepEqual(
    decideRelationshipEnd(current, exact, instant('2026-09-03T00:00:00.000Z')),
    {
      _tag: 'unchanged',
    }
  );
