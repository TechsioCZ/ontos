...
it('relationship writes are idempotent tenant Actions with dedicated authority', () => {
  ...
  for (const { descriptor } of actions) {
    ...
    expect(descriptor.tenantPermission?.({} as never)).toBe(
      'manage_party_relationships'
    );
  }
  expect(
    Object.keys(createPartyRelationshipAction.descriptor.domainEvents)
  ).toEqual(['party.registry.relationship-created.v1']);
  expect(
    Object.keys(updatePartyRelationshipAction.descriptor.domainEvents)
  ).toEqual(['party.registry.relationship-updated.v1']);
  expect(
    Object.keys(endPartyRelationshipAction.descriptor.domainEvents)
  ).toEqual(['party.registry.relationship-ended.v1']);
});

it.effect(
  'relationship detail is a tenant-authorized governed read of one ResourceRef',
  () =>
    Effect.gen(function* schemaContract1() {
      expect(partyRelationshipDetailRead.descriptor.legalEntityScope).toBe(
        'optional'
      );
      expect(partyRelationshipDetailRead.descriptor.permissionTarget).toBe(
        'tenant'
      );
...
it.effect(
  'relationship detail preserves canonical and stored alias endpoint context',
  () =>
    Effect.gen(function* schemaContract2() {
      ...
      expect(detail.from.canonicalPartyRef.resourceId).toBe(
      ...
      expect(DateTime.formatIso(Option.getOrThrow(detail.validTo))).toBe(
...
it.effect(
  'outbox payloads carry stable refs and no mutable Party or authorization copy',
  () =>
    Effect.gen(function* schemaContract3() {
      ...
      expect(
        yield* Schema.encodeEffect(RelationshipCreatedOutboxSchema)(decoded)
      ).toEqual(payload);
      expect(() =>
