...
it('Party-owned search semantics expose only current approved V1 facts', () => {
  ...
  expect(PARTY_SEARCH_SEMANTICS.contactPointIdentityAuthority).toBe(
    'NON_UNIQUE'
  );
...
it('Counterparty semantics retain Legal Entity and current-period role boundaries', () => {
  expect(COUNTERPARTY_SEARCH_SEMANTICS.legalEntityScope).toBe(
    'REQUIRED_TRUSTED_CONTEXT'
  );
  expect(COUNTERPARTY_SEARCH_SEMANTICS.roleFilters).toEqual([
    'CUSTOMER',
    'SUPPLIER',
  ]);
  expect(COUNTERPARTY_SEARCH_SEMANTICS.rolePeriodSemantics).toBe(
    'CURRENT_AT_EFFECTIVE_TIME'
  );
  expect(COUNTERPARTY_SEARCH_SEMANTICS.deduplicateBy).toBe(
    'COUNTERPARTY_IDENTITY'
  );
});

it.effect(
  'Party Search accepts a bounded query and an explicit archived switch',
  () =>
...
it.effect(
  'Counterparty Search exposes only the closed current-role filter',
  () =>
    Effect.gen(function* testScenario() {
      expect(
...
it.effect(
  'Counterparty Search result distinguishes Counterparty and canonical Party',
  () =>
    Effect.gen(function* testScenario() {
