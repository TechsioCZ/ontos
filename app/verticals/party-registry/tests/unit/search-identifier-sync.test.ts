...
import { makePartySearchProjector, PartySearchProjector } from '../../src/services/party-search-projection.service.ts';
import type {
...
const assertIdentifierOutbox = (
  ...
  expect(outbox.message.payloadJson).toEqual({
...
it.effect('CreateParty MATCHED_EXISTING publishes an attached identifier and indexes it after delivery only', () =>
...
it.effect('reviewed MATCH_EXISTING publishes an attached identifier and indexes it after delivery only', () =>
...
it.effect('END_VALIDITY refreshes search only after its committed identifier message and remains replay-safe', () =>
  Effect.gen(function* endIdentifierSearchScenario() {
    ...
    const outbox = assertIdentifierOutbox(harness, 'party.registry.official-identifier-updated.v1');
