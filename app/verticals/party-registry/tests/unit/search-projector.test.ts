...
it.effect('post-commit projection makes only active permission-safe identity evidence searchable', () =>
...
it.effect('aliases collapse to canonical identity and only alias-only evidence labels the match', () =>
  Effect.gen(function* testScenario() {
...
it.effect('future-ended contact disappears at its period boundary without another lifecycle message', () =>
  Effect.gen(function* testScenario() {
    const [party] = snapshot.parties;
    expect(party).toBeTruthy();
    if (party === undefined) {
...
it.effect('Counterparty identity survives aliases, current-role expiry and canonical-party collisions', () =>
  Effect.gen(function* testScenario() {
...
it.effect('shared public contact returns multiple Parties without uniqueness or matching authority', () =>
  Effect.gen(function* testScenario() {
...
it.effect('rebuild reconciles omitted documents and preserves tombstones against stale lifecycle delivery', () =>
  Effect.gen(function* testScenario() {
    const store = makeInMemoryCoreSearchProjectionStore();
    const search = yield* createCoreSearchQueryRuntime.pipe(Effect.provideService(CoreSearchProjectionStore, store));
    let current = snapshot;
...
it.effect('source failure is sanitized and leaves previously searchable state intact for retry', () =>
  Effect.gen(function* testScenario() {
    const store = makeInMemoryCoreSearchProjectionStore();
    const search = yield* createCoreSearchQueryRuntime.pipe(Effect.provideService(CoreSearchProjectionStore, store));
    let fail = false;
    ...
    const priorHits = yield* search.search({
      includeArchived: false,
      moduleId: 'party.registry',
      query: 'ACME',
      resourceType: 'party.registry.party',
...
it.effect('zero-length cancelled periods are never searchable and do not poison projection delivery', () =>
  Effect.gen(function* testScenario() {
    const [party] = snapshot.parties;
    expect(party).toBeTruthy();
    if (party === undefined) {
    ...
    expect(result.value[0]?.temporalSearchableText?.filter((entry) => entry.value === 'cancelled')).toEqual([]);
...
it.effect('correction and identifier/contact changes replace obsolete evidence instead of accumulating history', () =>
  Effect.gen(function* testScenario() {
    const [party] = snapshot.parties;
    expect(party).toBeTruthy();
    if (party === undefined) {
...
it.effect('a complete empty rebuild also rejects delayed evidence for a never-before-indexed Party', () =>
  Effect.gen(function* testScenario() {
    const store = makeInMemoryCoreSearchProjectionStore();
    const search = yield* createCoreSearchQueryRuntime.pipe(Effect.provideService(CoreSearchProjectionStore, store));
