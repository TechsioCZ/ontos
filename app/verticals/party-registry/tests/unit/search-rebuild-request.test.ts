...
import { handleRebuildSearch, rebuildSearchWorker } from '../../src/workers/rebuild-search.worker.ts';
...
it.effect('tenant rebuild requests require Party administration and canonical idempotency', () =>
  Effect.gen(function* rebuildDescriptor() {
    ...
    expect(Object.keys(descriptor.domainEvents)).toEqual(['party.registry.search-rebuild-requested.v1']);
  }),
);

it.effect('authorized rebuild commits one linked request without reading identity or running the projector', () =>
...
it.effect('denied Party administration cannot queue a rebuild even with Action execution permission', () =>
...
it.effect('rebuild worker uses its trusted committed context, and failures remain retryable', () => {
  ...
  return Effect.gen(function* rebuildWorkerFailure() {
    ...
    expect(rebuildSearchWorker.descriptor.topic).toBe('party.registry.search-rebuild-requested.v1');
  });
});
