...
const port = Option.getOrElse(
  Result.getOrThrow(
    ...
    )(getBuildConfigEnvironment('VERTICAL_PAYMENT_TERM_CATALOG_PORT')),
  ),
  () => 4103,
);
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_PAYMENT_TERM_CATALOG');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_PAYMENT_TERM_CATALOG, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
