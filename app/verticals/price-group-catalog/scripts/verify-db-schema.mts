...
const verifyInfrastructure = (catalog: InfrastructureCatalog | undefined) =>
  ...
  catalog.force_rls_count === PRICE_GROUP_CATALOG_TABLES.length &&
  catalog.wrong_owner_count === 0 &&
  catalog.policy_count === PRICE_GROUP_CATALOG_TABLES.length * 4 &&
  ...
  !catalog.runtime_select &&
  !catalog.runtime_insert &&
  !catalog.runtime_update &&
  !catalog.runtime_delete &&
  !catalog.role_super &&
