/* jscpd:ignore-start -- Generated deployable-MicroVertical configuration intentionally mirrors the shared Modern.js deployment contract. */
...
// fallow-ignore-next-line code-duplication -- Generated Modern.js bootstrap is deliberately identical across deployable verticals.
...
const port = Option.getOrElse(
  Result.getOrThrow(
    ...
    )(getBuildConfigEnvironment('VERTICAL_PRIVACY_PORT')),
  ),
  () => 4104,
);
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
// fallow-ignore-next-line code-duplication -- Generated deployment URL and runtime configuration follows the shared vertical contract.
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_PRIVACY');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_PRIVACY, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
