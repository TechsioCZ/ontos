...
const authenticationProblem = () => ({
  _tag: 'ReadProjectAuthenticationProblem' as const,
...
const unavailableProblem = () => ({
  _tag: 'ReadProjectUnavailableProblem' as const,
...
const invalidProblem = () => ({
  _tag: 'ReadProjectInvalidProblem' as const,
...
const forbiddenProblem = () => ({
  _tag: 'ReadProjectForbiddenProblem' as const,
...
const notFoundProblem = () => ({
  _tag: 'ReadProjectNotFoundProblem' as const,
...
const policyProblem = (status: 409 | 422) =>
  status === 409
    ? {
        _tag: 'ReadProjectPolicyConflictProblem' as const,
    ...
    : {
        _tag: 'ReadProjectPolicyProblem' as const,
...
const internalProblem = () => ({
  _tag: 'ReadProjectInternalProblem' as const,
...
export const readProjectReadApiLive = HttpApiBuilder.group(projectsApi, 'reads', (handlers) =>
  handlers.handle('execute', ({ payload, request }) =>
    Effect.gen(function* governedProviderRead() {
      ...
      Effect.catchDefect((defect) =>
        Effect.annotateLogs(Effect.logError('Unexpected Project read BFF defect', defect), {
          correlationId: request.headers['x-correlation-id'] ?? 'unavailable',
          readKey: readProjectRead.descriptor.readKey,
