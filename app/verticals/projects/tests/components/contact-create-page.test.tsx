...
} from '../../src/routes/[lang]/projects/customers/[id]/contacts/new/page.tsx';
...
test('renders the empty ready form without loading the parent Customer', () => {
  ...
  expect(screen.getByRole('link', { name: 'Back to Customer' }).getAttribute('href')).toBe(
    `/en/projects/customers/${customerId}`,
  );
...
test('decodes only bounded Customer UUIDs and builds the localized parent destination', () => {
  ...
  expect(customerDetailHref('cs', customerId)).toBe(`/cs/projects/customers/${customerId}`);
});
...
test('keeps a non-writable target visible without an enabled mutation path', () => {
  ...
  expect(
    screen.getByText('Projects is currently read-only. A Contact cannot be created right now.'),
  ).toBeTruthy();
...
test('submits the exact normalized payload through the generated client and navigates to the parent', async () => {
  ...
  await waitFor(() =>
    expect(navigateMock).toHaveBeenCalledWith({ to: `/en/projects/customers/${customerId}` }),
  );
...
test('creates a new key after an uncertain intent changes', async () => {
  createContactMock.mockReturnValue(Effect.fail({ _tag: 'ProjectsUnavailableProblem' }));
  renderFeature();
  const user = await fillForm();
...
test('creates a fresh key after a definite failure without changing values', async () => {
  createContactMock.mockReturnValue(Effect.fail({ _tag: 'ProjectsForbiddenProblem' }));
  renderFeature();
  const user = await fillForm();
  await user.click(screen.getByRole('button', { name: 'Create Contact' }));
...
test.each([
  ['ProjectsInvalidRequestProblem', 'The Contact details were rejected.'],
  ['ProjectsAuthenticationProblem', 'Your session expired before the Contact could be created.'],
  ['ProjectsForbiddenProblem', 'You do not have permission to create a Contact for this Customer.'],
  ['ProjectsNotFoundProblem', 'This Customer no longer exists or is not available to you.'],
  ['ProjectsConflictProblem', 'The Contact could not be created because the request conflicts'],
  ['ProjectsInternalProblem', 'The Contact could not be created safely.'],
...
test('maps the complete typed client error union to a closed presentation vocabulary', () => {
  expect(classifyCreateContactError({ _tag: 'ProjectsInvalidRequestProblem' })).toEqual({
    state: 'invalid_form',
  });
  ...
  expect(classifyCreateContactError({ _tag: 'ProjectsForbiddenProblem' })).toEqual({
    state: 'forbidden',
  });
  expect(classifyCreateContactError({ _tag: 'ProjectsNotFoundProblem' })).toEqual({
    state: 'not_found',
  });
  expect(classifyCreateContactError({ _tag: 'ProjectsPreconditionRequiredProblem' })).toEqual({
    state: 'conflict',
  });
  expect(classifyCreateContactError({ _tag: 'ProjectsConflictProblem' })).toEqual({
    state: 'conflict',
  });
  ...
  for (const tag of ['ProjectsUnavailableProblem', 'GatewayUnavailableProblem'] as const) {
  ...
  for (const tag of ['ProjectsInternalProblem', 'GatewayInternalProblem'] as const) {
...
test('Back and Cancel use the localized parent route without invoking the mutation', async () => {
  ...
  expect(screen.getByRole('link', { name: 'Zpět k zákazníkovi' }).getAttribute('href')).toBe(
    `/cs/projects/customers/${customerId}`,
  );
  await user.click(screen.getByRole('button', { name: 'Zrušit' }));
  expect(navigateMock).toHaveBeenCalledWith({ to: `/cs/projects/customers/${customerId}` });
  expect(createContactMock).not.toHaveBeenCalled();
});
...
describe('generated boundaries and localization', () => {
  test('keeps the form private and the page behind exact owner/Shell wiring', () => {
    const pageSource = readFileSync(
      new URL(
        '../../src/routes/[lang]/projects/customers/[id]/contacts/new/page.tsx',
        import.meta.url,
      ),
      'utf-8',
    );
    ...
    expect(manifest).toContain("contributionKey: 'projects.core.page.contact-create'");
    expect(manifest).toContain("routePath: '/projects/customers/:id/contacts/new'");
    expect(manifest).not.toContain('projects.core.navigation.contact-create');
    ...
    expect(formSource).not.toMatch(/projects-client|routeParams|customerId|target\.writable/u);
  });

  test('keeps English and Czech ContactCreate locale structures in parity', () => {
    expect(flattenKeys(csCatalog.projects.pages.contactCreate)).toEqual(
      flattenKeys(enCatalog.projects.pages.contactCreate),
    );
  });
});
