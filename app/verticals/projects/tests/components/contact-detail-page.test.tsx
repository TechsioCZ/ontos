...
import { getProjectsQueryClient } from '../../src/projects-query-client.ts';
import {
  ContactDetailPage,
...
} from '../../src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/page.tsx';
...
describe('Contact detail route input', () => {
  test('accepts only bounded Projects UUIDs and builds a hierarchical query key', () => {
...
test('loads one Contact once through the typed Projects client with only the Contact ID', async () => {
...
test('renders the parent link, heading, semantic overview, communication links, lifecycle, and times', async () => {
  ...
  expect(heading.className).toContain('projects:break-words');
  expect(screen.getByRole('link', { name: 'Back' }).getAttribute('href')).toBe(
    `/en/projects/customers/${customerId}`,
  );
  expect(screen.getByRole('link', { name: 'Edit' }).getAttribute('href')).toBe(
    `/en/projects/customers/${customerId}/contacts/${contactId}/edit`,
  );
...
test('renders Czech archived data and preserves the locale and Customer ID in the parent link', async () => {
  ...
  expect(screen.getByRole('link', { name: 'Upravit' }).getAttribute('href')).toBe(
    `/cs/projects/customers/${customerId}/contacts/${contactId}/edit`,
  );
});
...
test('keeps locale parity, long-value wrapping, and the generated frontend seam', () => {
  expect(flattenKeys(csCatalog.projects.pages.contactDetail)).toEqual(
    flattenKeys(enCatalog.projects.pages.contactDetail),
  );
  const source = readFileSync(
    new URL(
      '../../src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/page.tsx',
      import.meta.url,
    ),
    'utf-8',
  );
  expect(source).toContain("from '../../../../../../../api/projects-client.ts'");
  expect(source).toContain('projects:break-words');
  expect(source).toContain('projects:break-all');
  expect(source).toContain('projects:block');
