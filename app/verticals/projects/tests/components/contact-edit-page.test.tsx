...
} from '../../src/projects-query-client.ts';
import {
  ContactEditPage,
  ContactEditFeature,
...
} from '../../src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/edit/page.tsx';
...
beforeEach(() => {
  historyCanGoBack.current = false;
  const queryClient = getProjectsQueryClient();
  queryClient.clear();
...
describe('ContactEdit route and query boundary', () => {
  test('decodes only the bounded UUID pair and keeps the established hierarchical cache identity', () => {
    ...
    expect(contactDetailHref('cs', customerId, contactId)).toBe(
      `/cs/projects/customers/${customerId}/contacts/${contactId}`,
    );
  });
  ...
  ])('rejects invalid route input %j before exposing data or calling Projects', async (options) => {
...
describe('ContactEdit presentation and mutation', () => {
  test('keeps loaded values visible but disables every mutation control when read-only', async () => {
    ...
    expect(
      await screen.findByText(
        'Projects is currently read-only. You can review this Contact, but cannot save changes.',
      ),
    ).toBeTruthy();
  ...
  test('Back and Cancel preserve locale and hierarchy without invoking the mutation', async () => {
    ...
    const user = userEvent.setup();
    renderFeature();
    const destination = `/cs/projects/customers/${customerId}/contacts/${contactId}`;
  ...
  test('submits normalized values, updates the exact detail cache, announces, and navigates', async () => {
    ...
    const destination = `/en/projects/customers/${customerId}/contacts/${contactId}`;
  ...
  test('creates a new key after the uncertain intent changes', async () => {
    editContactMock.mockReturnValue(Effect.fail({ _tag: 'ProjectsUnavailableProblem' }));
    const user = userEvent.setup();
    renderFeature();
    const name = await screen.findByRole('textbox', { name: /^Contact name/u });
    await user.click(screen.getByRole('button', { name: 'Save Contact' }));
  ...
  test('creates a new key after a definite failure with unchanged values', async () => {
    editContactMock.mockReturnValue(Effect.fail({ _tag: 'ProjectsForbiddenProblem' }));
    const user = userEvent.setup();
    renderFeature();
    await screen.findByRole('textbox', { name: /^Contact name/u });
    await user.click(screen.getByRole('button', { name: 'Save Contact' }));
  ...
  test.each([
    ...
    ['ProjectsAuthenticationProblem', 'Your session expired before the Contact could be saved.'],
    ['ProjectsForbiddenProblem', 'You do not have permission to edit this Contact.'],
    ['ProjectsNotFoundProblem', 'This Contact no longer exists or is not available to you.'],
    ['ProjectsConflictProblem', 'The Contact changed while you were editing it.'],
    ['ProjectsInternalProblem', 'The Contact could not be saved safely.'],
  ...
  test('maps every mutation failure family to a closed presentation vocabulary', () => {
    expect(classifyEditContactError({ _tag: 'ProjectsInvalidRequestProblem' })).toEqual({
      state: 'invalid_form',
    });
    expect(classifyEditContactError({ _tag: 'GatewayForbiddenProblem' })).toEqual({
      state: 'forbidden',
    });
    expect(classifyEditContactError({ _tag: 'ProjectsPreconditionRequiredProblem' })).toEqual({
      state: 'conflict',
    });
...
describe('ContactEdit generated boundaries and localization', () => {
  test('keeps ContactForm private and crosses only the generated browser-safe Projects seam', () => {
    const pageSource = readFileSync(
      new URL(
        '../../src/routes/[lang]/projects/customers/[id]/contacts/[contactId]/edit/page.tsx',
        import.meta.url,
      ),
      'utf-8',
    );
    ...
    expect(pageSource).toContain('projects:min-w-0 projects:w-full');
    expect(pageSource).not.toMatch(
      /\bfetch\s*\(|api\/index|edit-contact\.action|persistence\.service|src\/db|HttpApiEndpoint/u,
    );
    expect(manifest).toContain("contributionKey: 'projects.core.page.contact-edit'");
    expect(manifest).toContain("routePath: '/projects/customers/:id/contacts/:contactId/edit'");
    expect(manifest).not.toContain('projects.core.navigation.contact-edit');
    ...
    expect(formSource).not.toMatch(
      /projects-client|routeParams|customerId|contactId|target\.writable/u,
    );
  });

  test('keeps English and Czech ContactEdit locale structures in parity', () => {
    expect(flattenKeys(csCatalog.projects.pages.contactEdit)).toEqual(
      flattenKeys(enCatalog.projects.pages.contactEdit),
    );
  });
});
