...
} from '../../src/routes/[lang]/projects/customers/[id]/edit/page.tsx';
...
test('keeps a read-only Customer readable without an enabled mutation path', async () => {
  ...
  expect(
    await screen.findByText(
      'Projects is currently read-only. You can review this Customer, but cannot save changes.',
    ),
  ).toBeTruthy();
...
test('creates a new idempotency key after the user changes the failed intent', async () => {
  editCustomerMock.mockReturnValue(Effect.fail({ _tag: 'ProjectsUnavailableProblem' }));
  const user = userEvent.setup();
  renderFeature();
  const name = await screen.findByRole('textbox', { name: /^Customer name/u });
...
test.each([
  ...
  ['ProjectsAuthenticationProblem', 'Your session expired before the Customer could be saved.'],
  ['ProjectsForbiddenProblem', 'You do not have permission to edit this Customer.'],
  ...
  ['ProjectsInternalProblem', 'The Customer could not be saved safely. Try again.'],
...
test('maps duplicate IČO to a distinct safe and actionable warning', async () => {
  editCustomerMock.mockReturnValue(
    Effect.fail({ _tag: 'ProjectsConflictProblem', code: 'projects_customer_ico_conflict' }),
  );
  const user = userEvent.setup();
  renderFeature();
  await screen.findByRole('textbox', { name: /^Customer name/u });
...
test('maps all mutation failure families to a closed presentation vocabulary', () => {
  expect(classifyEditCustomerError({ _tag: 'ProjectsInvalidRequestProblem' })).toEqual({
    state: 'invalid',
  });
  ...
  expect(
    classifyEditCustomerError({ _tag: 'ProjectsConflictProblem', code: 'projects_conflict' }),
  ).toEqual({
    state: 'conflict',
  });
  expect(
...
describe('generated boundaries and localization', () => {
  test('keeps the form private and the page behind generated owner/Shell wiring', () => {
    const pageSource = readFileSync(
      new URL('../../src/routes/[lang]/projects/customers/[id]/edit/page.tsx', import.meta.url),
      'utf-8',
    );
    ...
    expect(manifest).toContain("contributionKey: 'projects.core.page.customer-edit'");
    expect(manifest).toContain("routePath: '/projects/customers/:id/edit'");
    expect(manifest).not.toContain('projects.core.navigation.customer-edit');
    expect(registration).toContain("'page-customer-edit': () =>");
    expect(registration).toContain(
      "import('./src/routes/[lang]/projects/customers/[id]/edit/page.tsx')",
    );
  ...
  test('keeps English and Czech CustomerEdit locale structures in parity', () => {
    expect(flattenKeys(csCatalog.projects.pages.customerEdit)).toEqual(
      flattenKeys(enCatalog.projects.pages.customerEdit),
    );
  });
});
