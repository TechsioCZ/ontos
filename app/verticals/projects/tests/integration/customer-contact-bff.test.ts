...
import { makeProjectsApiRuntime } from '../../api/index.ts';
...
import {
  ...
  unarchiveContact,
  unarchiveCustomer,
} from '../../src/api/projects-client.ts';
...
test('runs every Projects client operation through the real governed BFF boundary', async () => {
  const previousIssuer = process.env['ONTOS_GATEWAY_ISSUER'];
  const previousJwks = process.env['ONTOS_GATEWAY_PUBLIC_JWKS'];
  const issuer = 'https://shell.projects-bff.test';
  const { privateKey, publicKey } = await generateKeyPair('Ed25519');
  const publicJwk = await exportJWK(publicKey);
  const now = Math.floor(Date.now() / 1000);
  const assertion = await new SignJWT({ principal, ver: 1 })
    .setProtectedHeader({ alg: 'EdDSA', kid: 'projects-bff', typ: 'JWT' })
    .setIssuer(issuer)
    .setAudience('projects')
  ...
  process.env['ONTOS_GATEWAY_PUBLIC_JWKS'] = JSON.stringify({
    keys: [{ ...publicJwk, alg: 'EdDSA', kid: 'projects-bff', use: 'sig' }],
  });
  ...
  const actionRuntime: ActionRuntimeService = {
    ...
    runAction: (input) => {
      ...
      if (payload['name'] === 'trigger-ico-conflict') {
        return failDomain(
          new ProjectsCustomerIcoConflict({
      ...
      if (payload['name'] === 'trigger-unavailable') {
        return failDomain(
          new ProjectsPersistenceUnavailable({
            ...
            reason: 'Projects persistence is temporarily unavailable',
          }),
        );
      }
  ...
  const runtime = makeProjectsApiRuntime(
  ...
  try {
    const base = {
      ...
      correlationId: 'projects-bff-correlation',
      gateway: { baseUrl: server.baseUrl },
      traceId: 'projects-bff-trace',
    } as const;
    const mutation = { ...base, idempotencyKey: 'projects-bff-idempotency' };
    ...
    const malformedBase = {
      ...
      correlationId: 'projects-bff-malformed-correlation',
    ...
    const transportFailure = await Effect.runPromise(
      Effect.flip(
        getCustomerDetail(
          ...
          {
            baseUrl: malformedServer.baseUrl,
            correlationId: 'projects-bff-transport-correlation',
            gateway: { baseUrl: server.baseUrl },
          },
        ),
      ),
    ...
    assert.equal(parseJsonObject(defectFailure)['_tag'], 'ProjectsInternalProblem');
    ...
    assert.equal(parseJsonObject(forbiddenFailure)['_tag'], 'ProjectsForbiddenProblem');
    ...
    assert.equal(parseJsonObject(notFoundFailure)['_tag'], 'ProjectsNotFoundProblem');
    const conflictFailure = await Effect.runPromise(
      Effect.flip(archiveCustomer({ customerId: conflictId }, mutation)),
    );
    assert.equal(parseJsonObject(conflictFailure)['_tag'], 'ProjectsConflictProblem');

    const invalidMutation = await Effect.runPromise(
      Effect.flip(createCustomer(customerPayload('trigger-invalid'), mutation)),
    );
    assert.equal(parseJsonObject(invalidMutation)['_tag'], 'ProjectsInvalidRequestProblem');
    ...
    assert.equal(icoConflictProblem['_tag'], 'ProjectsConflictProblem');
    assert.equal(icoConflictProblem['code'], 'projects_customer_ico_conflict');
    assert.equal(JSON.stringify(icoConflict).includes('projects_customers_tenant_ico_uk'), false);
    ...
    assert.equal(editIcoConflictProblem['_tag'], 'ProjectsConflictProblem');
    assert.equal(editIcoConflictProblem['code'], 'projects_customer_ico_conflict');
    assert.equal(
      JSON.stringify(editIcoConflict).includes('projects_customers_tenant_ico_uk'),
      false,
    );
    ...
    assert.equal(unavailableProblem['_tag'], 'ProjectsUnavailableProblem');
    ...
    const missingAssertion = await handler.handler(
      new Request('https://projects.test/projects/customers/create', {
