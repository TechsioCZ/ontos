...
const principal = {
  ...
  authContextRef: 'better-auth-session:projects-integration',
...
const otherPrincipal = {
  authBindingId: randomUUID(),
  authContextRef: 'better-auth-session:projects-integration-other',
  authMethod: 'session' as const,
    ...
    executeTransaction((transaction) => {
      ...
      transaction.insert = (table) => {
        if (table === dataAccessEvents) {
          throw new Error('Injected Projects evidence persistence failure');
        }
        return insert(table);
      };
...
test('runs Projects writes and reads through the governed runtimes with durable evidence', async () => {
  ...
  try {
    await adminPool.query(
      `insert into core.tenants (tenant_id, slug, name, status, default_locale)
       values ($1, $2, 'Projects operations integration', 'active', 'en')`,
      [tenantId, `projects-operations-${tenantId}`],
    );
    await adminPool.query(
      `insert into core.legal_entities
         (legal_entity_id, tenant_id, legal_name, registration_country, registration_number, status)
       values ($1, $2, 'Projects operations entity', 'CZ', $3, 'active')`,
      [legalEntityId, tenantId, `projects-operations-${legalEntityId}`],
    );
    await adminPool.query(
      `insert into core.principals (principal_id, tenant_id, kind, display_name, status)
       values ($1, $2, 'human', 'Projects operations principal', 'active')`,
    ...
    await adminPool.query(
      ...
      [principal.authBindingId, tenantId, principalId, `projects-operations-${principalId}`],
    );
    await adminPool.query(
      `insert into core.tenant_module_states (tenant_id, module_key, state)
       values ($1, 'projects.core', 'active')`,
      [tenantId],
    );
    await adminPool.query(
      `insert into core.tenants (tenant_id, slug, name, status, default_locale)
       values ($1, $2, 'Projects operations other tenant', 'active', 'en')`,
      [otherTenantId, `projects-operations-${otherTenantId}`],
    );
    await adminPool.query(
      `insert into core.principals (principal_id, tenant_id, kind, display_name, status)
       values ($1, $2, 'human', 'Projects operations other principal', 'active')`,
    ...
    await adminPool.query(
      ...
      [
        ...
        otherTenantId,
        otherPrincipalId,
        `projects-operations-${otherPrincipalId}`,
      ],
    );
    await adminPool.query(
      `insert into core.tenant_module_states (tenant_id, module_key, state)
        ...
        Effect.gen(function* governedOperations() {
                ...
                Effect.succeed({
                  create: (payload: CreateCustomerPayload) =>
                    Effect.tryPromise({
                        ...
                        new ProjectsPersistenceUnavailable({
                          code: 'projects_persistence_unavailable',
          ...
          const verifyReadEvidence = <Success, Failure, Requirements>(
            ...
            Effect.gen(function* verifyOneReadEvidence() {
              ...
              assert.deepEqual(row, {
                ...
                serving_module_key: 'projects.core',
          ...
          const customersBeforeInvalidPayload = yield* Effect.promise(() =>
            adminPool.query<{ count: string }>(
              'select count(*) from projects.customers where tenant_id = $1',
              [tenantId],
            ),
          );
          ...
          assert.equal(duplicateCreate._tag, 'ProjectsCustomerIcoConflict');
          ...
          assert.equal(rollbackFailure._tag, 'ProjectsPersistenceUnavailable');
          ...
          const contactsBeforeMissingParent = yield* Effect.promise(() =>
            adminPool.query<{ count: string }>(
              'select count(*) from projects.contacts where tenant_id = $1',
              [tenantId],
            ),
          );
          ...
          assert.equal(missingParent._tag, 'ProjectsCustomerNotFound');
          ...
          const firstPage = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('projects.core.api.customer-list.evidence.v1', 'list'),
          ...
          const detail = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('projects.core.api.customer-detail.evidence.v1', 'read'),
              resultCount: () => 1,
            },
          );
          ...
          const contactPage = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('projects.core.api.contact-list.evidence.v1', 'list'),
              resultCount: (result) => result.items.length,
            },
          );
          assert.deepEqual(
          ...
          assert.equal(repeatedContactArchive._tag, 'ProjectsLifecycleConflict');
          ...
          assert.equal(duplicateArchivedCreate._tag, 'ProjectsCustomerIcoConflict');
          ...
          assert.equal(repeatedCustomerArchive._tag, 'ProjectsLifecycleConflict');
          const nonCascadedDetail = yield* verifyReadEvidence(
            ...
            {
              ...ownRead('projects.core.api.contact-detail.evidence.v1', 'read'),
              resultCount: () => 1,
            },
          );
          ...
          assert.equal(duplicateEdit._tag, 'ProjectsCustomerIcoConflict');
          const duplicateTargetAfterConflict = yield* Effect.promise(() =>
            adminPool.query<{ ico: string | null; name: string }>(
              'select ico, name from projects.customers where tenant_id = $1 and customer_id = $2',
          ...
          for (const state of ['read_only', 'deprecated'] as const) {
            yield* Effect.promise(() =>
              adminPool.query(
                "update core.tenant_module_states set state = $2 where tenant_id = $1 and module_key = 'projects.core'",
                [tenantId, state],
              ),
            );
          ...
          yield* Effect.promise(() =>
            adminPool.query(
              "update core.tenant_module_states set state = 'active' where tenant_id = $1 and module_key = 'projects.core'",
              [tenantId],
            ),
          );

          const evidenceFailingDatabase = failingEvidenceDatabase(
            yield* makeCoreDatabase(configuration),
          );
          ...
          assert.ok(
            standalone.every(
              (row) =>
                ...
                row.evidencePolicyKey.startsWith('projects.core.'),
            ),
          );
          ...
          const persistedAresCustomer = yield* Effect.promise(() =>
            adminPool.query<{ record: JsonObject }>(
              `select to_jsonb(customer) as record
               from projects.customers as customer
    ...
    for (const cleanupTenantId of [tenantId, otherTenantId]) {
      await adminPool.query('delete from projects.contacts where tenant_id = $1', [
        cleanupTenantId,
      ]);
      await adminPool.query('delete from projects.customers where tenant_id = $1', [
        cleanupTenantId,
      ]);
