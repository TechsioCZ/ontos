...
import { Pool } from 'pg';
import { contacts, projectsDatabaseSchema, customers } from '../../src/db/schema.ts';
...
test('fresh install owns only the Core, Auth, and Projects schemas and journals', async () => {
  ...
  try {
    const schemas = await adminPool.query<{ schema_name: string }>(`
    ...
    `);
    const journals = await adminPool.query<{ table_name: string }>(`
    ...
    assert.deepEqual(
      schemas.rows.map(({ schema_name: schemaName }) => schemaName),
      ['auth', 'core', 'projects'],
    );
    assert.deepEqual(
      journals.rows.map(({ table_name: tableName }) => tableName),
      ['__drizzle_migrations_auth', '__drizzle_migrations_core', '__drizzle_migrations_projects'],
    );
    assert.equal(
      schemas.rows.some(({ schema_name: schemaName }) => schemaName === 'crm'),
      false,
    );
    assert.equal(
      journals.rows.some(({ table_name: tableName }) => tableName.endsWith('_crm')),
      false,
    );
  } finally {
    await adminPool.end();
...
test('enforces Projects constraints, tenant RLS, parent integrity, and durable archiving', async () => {
  const connections = await Effect.runPromise(loadDatabaseConnectionPair());
  const adminPool = new Pool({ connectionString: connections.admin.connectionString });
  ...
  const admin = drizzle({ client: adminPool, schema: projectsDatabaseSchema });
  const runtime = drizzle({ client: runtimePool, schema: projectsDatabaseSchema });
  const cleanup = async () => {
