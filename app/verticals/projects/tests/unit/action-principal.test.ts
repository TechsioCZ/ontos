...
const makeAssertion = async (
  ...
  const publicJwk = {
    ...
    x5u: 'https://shell.ontos.test/certificates/projects-test.pem',
  };
  ...
  const token = await new SignJWT({ principal, ver: 1 })
    .setProtectedHeader({ alg: 'EdDSA', kid: 'projects-test', typ: 'JWT' })
    .setIssuer(issuer)
...
test('verifies a signed Projects-audience assertion into trusted principal context', async () => {
...
test('acquires a fresh Projects assertion for every gateway invocation', async () => {
  let issued = 0;
