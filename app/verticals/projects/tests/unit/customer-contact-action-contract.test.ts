...
const expectedActionKeys = [
  'projects.core.archive-contact',
  'projects.core.archive-customer',
  'projects.core.create-contact',
  'projects.core.create-customer',
  'projects.core.edit-contact',
  'projects.core.edit-customer',
  'projects.core.unarchive-contact',
  'projects.core.unarchive-customer',
] as const;
...
test('registers the exact generated Projects write contracts', async () => {
  assert.deepEqual(
    actions.map(({ descriptor }) => descriptor.actionKey).toSorted(),
    expectedActionKeys,
  );
  for (const { descriptor } of actions) {
    assert.equal(descriptor.owningModuleKey, 'projects.core');
...
test('governs complete Customer mutation payloads and keeps generated Action identities stable', () => {
  ...
  assert.equal(createCustomerAction.descriptor.actionKey, 'projects.core.create-customer');
  assert.equal(editCustomerAction.descriptor.actionKey, 'projects.core.edit-customer');
  ...
  assert.equal(createError._tag, 'ProjectsCustomerIcoConflict');
  ...
  assert.equal(persistenceError._tag, 'ProjectsPersistenceUnavailable');
});

test('defines exact flat Customer business-field result schemas', () => {
  assert.equal(Schema.decodeUnknownSync(ProjectsIcoSchema)('00123456'), '00123456');
  assert.equal(Schema.decodeUnknownSync(ProjectsDicSchema)('  CZ00123456  '), 'CZ00123456');
  assert.equal(Schema.decodeUnknownSync(ProjectsLegalFormCodeSchema)('112'), '112');
  assert.equal(Schema.decodeUnknownSync(ProjectsDateOnlySchema)('2024-02-29'), '2024-02-29');
  for (const invalidIco of ['1234567', '123456789', '1234A678']) {
    assert.throws(() => Schema.decodeUnknownSync(ProjectsIcoSchema)(invalidIco));
  }
  assert.throws(() => Schema.decodeUnknownSync(ProjectsDicSchema)('   '));
  assert.throws(() => Schema.decodeUnknownSync(ProjectsDicSchema)('x'.repeat(21)));
  assert.throws(() => Schema.decodeUnknownSync(ProjectsLegalFormCodeSchema)('12A'));
  for (const invalidDate of ['2024-2-09', '2023-02-29', '2024-04-31']) {
    assert.throws(() => Schema.decodeUnknownSync(ProjectsDateOnlySchema)(invalidDate));
  }
...
test('registers four governed reads with exact tenant-scoped evidence contracts', () => {
  ...
  assert.deepEqual(reads.map(({ descriptor }) => descriptor.readKey).toSorted(), [
    'projects.core.api.contact-detail',
    'projects.core.api.contact-list',
    'projects.core.api.customer-detail',
    'projects.core.api.customer-list',
  ]);
