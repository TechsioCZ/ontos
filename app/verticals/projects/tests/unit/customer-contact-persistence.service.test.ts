...
test('maps only the named tenant/IČO uniqueness constraint to the typed conflict', async () => {
  ...
  const conflict = await Effect.runPromise(
    Effect.flip(
      createCustomerRecord(
        rejectingTransaction('projects_customers_tenant_ico_uk'),
        baseRow.tenantId,
        payload,
      ),
    ),
  );
  assert.equal(conflict._tag, 'ProjectsCustomerIcoConflict');
  assert.equal(JSON.stringify(conflict).includes('projects_customers_tenant_ico_uk'), false);
  ...
  assert.equal(unrelated._tag, 'ProjectsPersistenceUnavailable');
});
