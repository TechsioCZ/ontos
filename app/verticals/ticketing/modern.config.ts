...
const appId = 'ticketing';
const cloudflareWorkerName = 'app-ticketing';
const port = Number(process.env['VERTICAL_TICKETING_PORT'] ?? 4101);
const envValue = (name: string) => {
  const value = process.env[name]?.trim();
  return value !== undefined && value.length > 0 ? value : undefined;
};
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_TICKETING');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_TICKETING, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
  ...
  presetUltramodern(
    {
      ...
      tools: {
        ...
        bundlerChain: (chain) => {
          chain.output
            .uniqueName('verticalTicketing')
            .chunkLoadingGlobal('__ULTRAMODERN_VERTICAL_TICKETING_LOADED_CHUNKS__');
        },
        devServer: {
          headers: {
