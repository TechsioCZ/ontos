...
import {
  createDatePropertyDefinitionActionKey,
  createDatePropertyDefinitionActionPayloadSchema,
  createDatePropertyDefinitionActionResponseSchema,
} from '../../shared/actions/create-date-property-definition.ts';
import type {
...
interface DatePropertyDefinitionRow {
...
const datePropertyDefinitionEvidence = (
  input: CreateDatePropertyDefinitionActionPayload,
  response: CreateDatePropertyDefinitionActionResponse,
) => ({
...
const createDatePropertyDefinitionAuditEvent = {
  evidence: datePropertyDefinitionEvidence,
...
const createDatePropertyDefinitionDomainEvent = {
  eventType: 'ticketing.taskPropertyDefinition.created',
  payload: datePropertyDefinitionEvidence,
...
const createDatePropertyDefinitionActionHandler: ActionHandler<
  CreateDatePropertyDefinitionActionPayload,
  CreateDatePropertyDefinitionActionResponse
> = async (input, services) => {
  ...
  if (name.length === 0) {
    throw rejectAction({
      code: 'ticketing.createDatePropertyDefinition.name_required',
  ...
  const result = await services.tx.execute(sql`
    ...
    returning
      ...
      revision
  `);
  const definition = rowsFromResult<DatePropertyDefinitionRow>(result).at(0);

  if (definition === undefined) {
    throw rejectAction({
      code: 'ticketing.createDatePropertyDefinition.not_created',
...
export const createDatePropertyDefinitionActionRegistration: ActionRegistration<
  CreateDatePropertyDefinitionActionPayload,
  CreateDatePropertyDefinitionActionResponse
> = {
  descriptor: {
    actionKey: createDatePropertyDefinitionActionKey,
    auditEvent: createDatePropertyDefinitionAuditEvent,
    ...
    domainEvent: createDatePropertyDefinitionDomainEvent,
    gatewayAudience: 'ticketing',
    idempotency: 'required',
    moduleStateAccess: 'mutate',
    transportRequestSchema: createDatePropertyDefinitionActionPayloadSchema,
    transportResponseSchema: createDatePropertyDefinitionActionResponseSchema,
  },
  handler: createDatePropertyDefinitionActionHandler,
};
