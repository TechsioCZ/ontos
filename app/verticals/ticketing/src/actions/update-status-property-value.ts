...
import {
  updateStatusPropertyValueActionKey,
  updateStatusPropertyValueActionPayloadSchema,
  updateStatusPropertyValueActionResponseSchema,
} from '../../shared/actions/update-status-property-value.ts';
import type {
...
const evidence = (
  input: UpdateStatusPropertyValueActionPayload,
  response: UpdateStatusPropertyValueActionResponse,
) => ({
  changedComponents: ['statusValue'],
...
> = async (input, services) => {
  ...
  if (current === undefined || (current.revision ?? 0) !== input.expectedRevision) {
    throw rejectAction({
      code: 'ticketing.updateStatusPropertyValue.stale_or_missing',
      message: 'The Status value changed elsewhere or is no longer available.',
    });
  }
  if (input.optionId !== undefined) {
    ...
    if (rowsFromResult(optionResult).length !== 1) {
      throw rejectAction({
        code: 'ticketing.updateStatusPropertyValue.option_not_found',
        message: 'The selected option is not available for this Status property.',
      });
    }
  }
  ...
  const result = await services.tx.execute(sql`
    with changed_value as (
      ...
      set option_id = excluded.option_id,
          revision = task_status_values.revision + 1
      where task_status_values.revision = ${input.expectedRevision}
      ...
      select updated_task.last_edited_at, ${services.context.principalId},
             'status_value_changed', updated_task.revision, updated_task.task_id,
...
export const updateStatusPropertyValueActionRegistration: ActionRegistration<
  UpdateStatusPropertyValueActionPayload,
  UpdateStatusPropertyValueActionResponse
> = {
  descriptor: {
    actionKey: updateStatusPropertyValueActionKey,
    auditEvent,
    ...
    transportRequestSchema: updateStatusPropertyValueActionPayloadSchema,
    transportResponseSchema: updateStatusPropertyValueActionResponseSchema,
  },
  handler,
};
