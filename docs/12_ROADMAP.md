...
The PoC should validate the stack and the architectural boundaries. It is expected to be disposable. It should prove UltraModern.js MicroVertical structure, design-system integration, BetterAuth session/API-key mapping to OntOS principals, SpiceDB permission checks, ResourceRef-based cross-module references, audit recording, outbox basics, tenant-level module activation/deactivation, internal dogfood slice, and property/rental stubs. Neo4j can be tested as an optional projection spike, but the PoC should not make V0 depend on it.
...
June should convert PoC learnings into decisions. Key outputs are a refined glossary, accepted/rejected ADRs, V0 scope lock, OntOS Module Manifest shape, Core/module boundaries, authz approach, ResourceRef model, outbox model, tenant-level module activation model, and customer process clarification.
...
July should establish the production skeleton, tenant/legal entity model, principal model, BetterAuth integration, Principal Auth Bindings, SpiceDB checks, tenant-level module state, action invocation recording, ResourceRef conventions, audit, timeline basics, Organization Registry V0 group views, and the first `property.registry` slice.
...
August should build the property registry, contacts/CRM basics, media/document center basics, ResourceRef linking UX, search basics, module unavailable states, read-only/quarantine behavior, audit hardening, and permission tests. Internal dogfooding should begin to produce real feedback.

## September 2026 — long-term and short-term rental MVPs

September should deliver pilotable long-term rental and short-term reservation slices. Long-term rental should include lease contracts, tenant/contact links, unit links, deposits/basic payment schedules, terms, attachments, and invoice source links. Short-term rental should include reservations, guests/contacts, reservation state, check-in/check-out basics, cleaning tasks, cancellation/change basics, and invoice source links.

## October 2026 — billing and accounting workflow

October should deliver billing basics and accounting workflow/export. This includes invoices with draft/issued lifecycle, line-level source allocations, numbering series, legal-entity billing identity, receivables/payment state basics, client/company accounting workspace, document inbox basics, cost/supplier invoice basics, checklist/status workflow, and accounting export/import baseline.
