...
const cloudflareWorkerName = 'mvp-property-registry';
const port = Number(process.env['VERTICAL_PROPERTY_REGISTRY_PORT'] ?? 4101);
const envValue = (name: string) => {
  const value = process.env[name]?.trim();
  return value !== undefined && value.length > 0 ? value : undefined;
};
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_PROPERTY_REGISTRY');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_PROPERTY_REGISTRY, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
