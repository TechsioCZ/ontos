...
CREATE TABLE "core"."outbox_messages" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "core_outbox_messages_status_ck" CHECK ("core"."outbox_messages"."status" in ('pending', 'processing', 'done', 'dead'))
);
--> statement-breakpoint
...
ALTER TABLE "core"."outbox_attempts" ADD CONSTRAINT "outbox_attempts_outbox_message_id_outbox_messages_outbox_message_id_fk" FOREIGN KEY ("outbox_message_id") REFERENCES "core"."outbox_messages"("outbox_message_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "core_outbox_messages_pending_idx" ON "core"."outbox_messages" USING btree ("status","available_at");--> statement-breakpoint
...
CREATE UNIQUE INDEX "core_tenants_slug_uk" ON "core"."tenants" USING btree ("slug");