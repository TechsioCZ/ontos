...
const chunkLoadingGlobal = '__ULTRAMODERN_VERTICAL_ACCOUNTING_LOADED_CHUNKS__';
...
const appId = 'accounting';
const cloudflareWorkerName = 'mvp2-accounting';
const port = Number(process.env['VERTICAL_ACCOUNTING_PORT'] ?? 4102);
const envValue = (name: string) => {
  const value = process.env[name]?.trim();
  return value !== undefined && value.length > 0 ? value : undefined;
};
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_ACCOUNTING');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_ACCOUNTING, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
  ...
  presetUltramodern(
    {
      ...
      tools: {
        ...
        bundlerChain: (chain) => {
          chain.output.uniqueName('verticalAccounting').chunkLoadingGlobal(chunkLoadingGlobal);
