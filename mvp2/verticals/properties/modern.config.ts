...
const chunkLoadingGlobal = '__ULTRAMODERN_VERTICAL_PROPERTIES_LOADED_CHUNKS__';
...
const cloudflareWorkerName = 'mvp2-properties';
const port = Number(process.env['VERTICAL_PROPERTIES_PORT'] ?? 4101);
const envValue = (name: string) => {
  const value = process.env[name]?.trim();
  return value !== undefined && value.length > 0 ? value : undefined;
};
const configuredSiteUrl = envValue('MODERN_PUBLIC_SITE_URL');
const configuredCloudflareUrl = envValue('ULTRAMODERN_PUBLIC_URL_PROPERTIES');
...
if (
  ...
  throw new Error(
    `Cloudflare deploy for ${appId} needs ULTRAMODERN_PUBLIC_URL_PROPERTIES, MODERN_PUBLIC_SITE_URL, or ULTRAMODERN_CLOUDFLARE_WORKERS_DEV_SUBDOMAIN.`,
  );
}
  ...
  presetUltramodern(
    {
      ...
      source: {
        globalVars: {
          ULTRAMODERN_PROPERTIES_API_URL: apiBaseUrl,
      ...
      tools: {
        ...
        bundlerChain: (chain) => {
          chain.output.uniqueName('verticalProperties').chunkLoadingGlobal(chunkLoadingGlobal);
