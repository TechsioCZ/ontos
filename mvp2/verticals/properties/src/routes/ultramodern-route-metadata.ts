// @generated by @modern-js/create.
// Author route metadata in colocated src/routes/**/route.meta.ts files.
// This compatibility manifest is regenerated from route-owned metadata.

export const ultramodernRouteNamespace = 'properties' as const;

export const ultramodernRouteMetadata = [
  {
    canonicalPath: '/',
    descriptionKey: 'properties.seo.description',
    id: 'properties-home',
    indexable: false,
    localisedPaths: {
      cs: '/',
      en: '/',
    },
    mfBoundaryId: 'verticalProperties',
    namespace: 'properties',
    ownerAppId: 'properties',
    public: false,
    publicSurface: 'private-app-screen',
    titleKey: 'properties.title',
  },
  {
    canonicalPath: '/properties/units',
    descriptionKey: 'properties.units.description',
    id: 'properties-units',
    indexable: false,
    localisedPaths: {
      cs: '/properties/units',
      en: '/properties/units',
    },
    mfBoundaryId: 'verticalProperties',
    namespace: 'properties',
    ownerAppId: 'properties',
    public: false,
    publicSurface: 'private-app-screen',
    titleKey: 'properties.units.title',
  },
] as const;

export const ultramodernLocalisedUrls = {} as const;

export const ultramodernPublicRoutes = [] as const;

export const ultramodernRouteConfig = {
  authoring: 'colocated-route-meta',
  generatedManifest: true,
  localisedUrls: ultramodernLocalisedUrls,
  namespace: ultramodernRouteNamespace,
  publicRoutes: ultramodernPublicRoutes,
  routes: ultramodernRouteMetadata,
  source: 'route-owned',
} as const;
